...
const equipmentRecipeCoverage = equipmentIds.filter((itemId) => Object.values(RECIPES).filter((recipe) => recipe.output.itemId === itemId && recipe.category === 'equipment' && recipe.output.quantity === 1).length === 1).length
...
for (const [dungeonId, ids] of Object.entries(EQUIPMENT_BY_DUNGEON)) { addDoc('Items/Equipment_' + dungeonFileName(dungeonId) + '.md', block('# ' + DUNGEONS[dungeonId].name + ' equipment', '', '## Stats & Effects', '', table([...equipmentStatColumns], ids.map(buildEquipmentStatRow)), '', '## Crafting', '', '> Comparison view — edit recipe costs in `Transmutation/Recipes.md`.', '', table(equipmentCraftingColumns, ids.map(buildEquipmentCraftingRow))), ids, ['src/game/content/items/items.ts', 'src/game/content/equipment/equipmentSets.ts', 'src/game/content/recipes/recipes.ts']) }
const recipeForEquipment = (itemId: ItemId) => Object.values(RECIPES).find((recipe) => recipe.output.itemId === itemId && recipe.category === 'equipment')
...
addDoc('Items/Boss_Relics.md', block('# Boss-signature equipment', '', 'Identity grouping only — these Equipment items are crafted through Transmutation from boss/signature materials and never drop as finished items.', '', table(['Item', 'Associated Boss', 'Signature Material', 'Material Drop Rate', 'Material Required', 'Recipe Time', 'Slot', 'Special Effect'], EQUIPMENT_BOSS_RELIC_IDS.map((id) => { const item = ITEMS[id]; const recipe = recipeForEquipment(id); const bossId = bossForRecipe(recipe, id); const signatureMaterial = signatureMaterialForRecipe(recipe); const materialDrop = signatureMaterial && bossId ? dropRowsForItem(signatureMaterial.itemId).find((drop) => drop.monsterId === bossId) : undefined; return [itemWithId(id), bossId ? monsterWithId(bossId) : dash, signatureMaterial ? itemWithId(signatureMaterial.itemId) : dash, materialDrop ? formatPercent(materialDrop.chance) : dash, signatureMaterial ? formatNumber(signatureMaterial.quantity) : dash, recipe ? formatDuration(recipe.baseDurationMs) : dash, readableSlot(item.equipmentSlot), formatCompactEquipmentSpecial(item) || dash] }))), EQUIPMENT_BOSS_RELIC_IDS, ['src/game/content/items/items.ts', 'src/game/content/equipment/equipmentSets.ts', 'src/game/content/recipes/recipes.ts', 'src/game/content/monsters'])

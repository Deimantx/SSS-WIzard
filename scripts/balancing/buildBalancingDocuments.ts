...
addDoc('Artificing/Recipes.md', block('# Artificing recipes', '', 'Manual Equipment crafting: one click commits one recipe cost and starts one 5-second craft. Output is granted once on completion. No Mana, Echoes, queue, repeat, or offline production.', '', ...DUNGEON_ORDER.flatMap(dungeonId => ['## ' + DUNGEONS[dungeonId].name, '', table(['Recipe', 'Slot', 'Dungeon', ...equipmentCraftingColumns.slice(1)], ARTIFICING_RECIPE_ORDER.filter(id => ARTIFICING_RECIPES[id].sourceDungeonId === dungeonId).map(id => { const recipe = ARTIFICING_RECIPES[id]; return [nameWithId(recipe.name, id), readableSlot(ITEMS[recipe.output.itemId].equipmentSlot), DUNGEONS[dungeonId].name, ...ingredientCells(recipe), formatRecipeUnlock(recipe.unlock)] })), ''])), ARTIFICING_RECIPE_ORDER, ['src/game/content/recipes/artificingRecipes.ts'])
...
for (const domain of ['Transmutation', 'Artificing'] as const) {
  ...
  })), '', 'Est. Enemy Kills uses the best authored normal-monster expected quantity for each monster material and takes the largest ingredient estimate. It does not estimate target farm time.', '', domain === 'Artificing' ? 'Crafting is manual with a 5-second completion timer per current recipe. Material sourcing remains the primary acquisition pacing factor; there is no queue or offline production.' : 'Continuous production is Mana funded and Echo driven.'), ids, ['src/game/content/recipes/' + (domain === 'Artificing' ? 'artificingRecipes.ts' : 'transmutationRecipes.ts'), 'src/game/content/monsters', 'src/game/content/items/items.ts'])
}

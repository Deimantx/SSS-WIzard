...
const equipmentCrafting = (itemId: string) => RECIPES[itemId as keyof typeof RECIPES]
...
export const buildEquipmentCraftingRow = (itemId: string) => {
  ...
  return recipe ? [itemWithId(itemId), formatDuration(recipe.baseDurationMs), formatNumber(recipe.manaCost), ...ingredientCells(recipe), formatRecipeUnlock(recipe.unlock)] : [itemWithId(itemId), dash, dash, ...ingredientCells({ ingredients: [] }), dash]
}
const directDropRow = (itemId: string) => {
  const item = ITEMS[itemId as ItemId]
  const drop = dropRowsForItem(item.id).find((entry) => entry.role === 'boss')
  return [itemWithId(itemId), drop ? monsterWithId(drop.monsterId) : dash, drop ? formatPercent(drop.chance) : dash, readableSlot(item.equipmentSlot), formatCompactEquipmentSpecial(item) || dash, sellValue(item)]
}
...
const recipeRows = (recipeIds: readonly string[]) => recipeIds.map((id) => { const recipe = RECIPES[id as keyof typeof RECIPES]; return [nameWithId(recipe.name, recipe.id), formatNumber(recipe.output.quantity), formatDuration(recipe.baseDurationMs), formatNumber(recipe.manaCost), ...ingredientCells(recipe), formatRecipeUnlock(recipe.unlock)] })
...
const recipeDungeon = (recipeId: string) => Object.entries(EQUIPMENT_BY_DUNGEON).find(([, itemIdsForDungeon]) => itemIdsForDungeon.includes(recipeId as ItemId))?.[0]
const groupedRecipeIds = (group: 'elemental' | 'whispering-woods' | 'howling-den' | 'abandoned-catacombs') => RECIPE_ORDER.filter((id) => group === 'elemental' ? RECIPES[id].category !== 'equipment' : recipeDungeon(id) === group)
...
const equipmentCraftingColumns = ['Item', 'Time', 'Mana', 'Ingredient 1', 'Qty 1', 'Ingredient 2', 'Qty 2', 'Ingredient 3', 'Qty 3', 'Ingredient 4', 'Qty 4', 'Ingredient 5', 'Qty 5', 'Unlock']
for (const [dungeonId, ids] of Object.entries(EQUIPMENT_BY_DUNGEON)) { const directDrops = ids.filter((id) => !equipmentCrafting(id)); addDoc('Items/Equipment_' + dungeonFileName(dungeonId) + '.md', block('# ' + DUNGEONS[dungeonId].name + ' equipment', '', '## Stats & Effects', '', table([...equipmentStatColumns], ids.map(buildEquipmentStatRow)), '', '## Crafting', '', '> Comparison view — edit recipe costs in `Transmutation/Recipes.md`.', '', table(equipmentCraftingColumns, ids.filter((id) => equipmentCrafting(id)).map(buildEquipmentCraftingRow)), ...(directDrops.length ? ['', '## Direct Drops', '', table(['Item', 'Boss', 'Chance', 'Slot', 'Special Effect', 'Sell'], directDrops.map(directDropRow))] : [])), ids, ['src/game/content/items/items.ts', 'src/game/content/equipment/equipmentSets.ts', 'src/game/content/recipes/recipes.ts']) }
addDoc('Items/Boss_Relics.md', block('# Boss relics', '', table(['Item', 'Boss', 'Slot', 'HP', 'Mana', 'Focus', 'Spell Power', 'Resistances', 'Other Stats', 'Special Effect', 'Drop Chance', 'Sell'], EQUIPMENT_BOSS_RELIC_IDS.map((id) => { const item = ITEMS[id]; const origin = Object.entries(EQUIPMENT_BY_DUNGEON).find(([, ids]) => ids.includes(id))?.[0]; const drop = dropRowsForItem(id).find((entry) => entry.role === 'boss'); const stats = item.stats ?? {}; const otherStats = Object.entries(stats).filter(([key]) => !['maxHealth', 'maxMana', 'maxFocus', 'spellPower', 'resistances'].includes(key)).map(([key, value]) => `${formatReadableId(key)} ${typeof value === 'number' && key.endsWith('Pct') ? formatSignedPercent(value) : formatFlatBonus(Number(value))}`).join('; ') || dash; const resistances = Object.entries(stats.resistances ?? {}).map(([type, value]) => `${formatReadableId(type)} ${formatSignedPercent(value)}`).join('; ') || dash; return [itemWithId(id), drop ? monsterWithId(drop.monsterId) : origin ? monsterWithId(DUNGEONS[origin].boss) : dash, readableSlot(item.equipmentSlot), formatFlatBonus(stats.maxHealth), formatFlatBonus(stats.maxMana), formatFlatBonus(stats.maxFocus), formatFlatBonus(stats.spellPower), resistances, otherStats, formatCompactEquipmentSpecial(item) || dash, drop ? formatPercent(drop.chance) : dash, sellValue(item)] }))), EQUIPMENT_BOSS_RELIC_IDS, ['src/game/content/items/items.ts', 'src/game/content/equipment/equipmentSets.ts', 'src/game/content/monsters'])
...
addDoc('Loot/Boss_Drops.md', block('# Boss drops', '', '> Comparison view — edit Min, Max, and Chance in the owning dungeon enemy Loot sheet.', '', table(['Dungeon', 'Enemy', 'Boss?', 'Item', 'Min', 'Max', 'Chance', 'Avg Drop', 'Est. Qty / Kill', 'Relic?', 'Progression Item?'], globalLootRows(DUNGEON_ORDER.map((id) => DUNGEONS[id].boss)).map((row) => [...row, EQUIPMENT_BOSS_RELIC_IDS.some((id) => String(row[3]).includes(id)) ? 'Yes' : 'No', String(row[3]).includes('heartseed') ? 'Yes' : 'No']))), DUNGEON_ORDER.map((id) => DUNGEONS[id].boss), ['src/game/content/monsters', 'src/game/content/dungeons/dungeons.ts'])

addDoc('Transmutation/Recipes.md', block('# Transmutation recipes', '', 'Canonical recipe sheet. Ingredient names and quantities are separate cells for direct editing.', '', ...(['elemental', 'whispering-woods', 'howling-den', 'abandoned-catacombs'] as const).flatMap((group) => [`## ${group === 'elemental' ? 'Elemental' : DUNGEONS[group].name}`, '', table(recipeColumns, recipeRows(groupedRecipeIds(group))), '']), 'Transmutation is the single item-creation system. Dev Tools grants use the normal acquisition path.'), RECIPE_ORDER, ['src/game/content/recipes/recipes.ts'])
...
addDoc('Transmutation/Crafting_Economy.md', block('# Crafting economy', '', 'Derived comparison values are marked Est. and do not add runtime values.', '', table(['Recipe', 'Dungeon', 'Monster Mats', 'Elemental Mats', 'Boss Mats', 'Total Items', 'Time', 'Est. Enemy Kills', 'Sell Value'], RECIPE_ORDER.map((id) => { const recipe = RECIPES[id]; const counts = recipeMaterialCounts(recipe); const dungeon = recipeDungeon(id); return [nameWithId(recipe.name, id), dungeon ? DUNGEONS[dungeon].name : formatReadableId(recipe.category), formatNumber(counts.monster), formatNumber(counts.elemental), formatNumber(counts.boss), formatNumber(recipe.ingredients.reduce((sum, ingredient) => sum + ingredient.quantity, 0)), formatDuration(recipe.baseDurationMs), estimatedRecipeKills(recipe), sellValue(ITEMS[recipe.output.itemId])] })), '', 'Est. Enemy Kills uses the best authored normal-monster expected quantity for each monster material and takes the largest ingredient estimate. It does not estimate target farm time.'), RECIPE_ORDER, ['src/game/content/recipes/recipes.ts', 'src/game/content/monsters', 'src/game/content/items/items.ts'])
...
const schoolLevels = Array.from({ length: 20 }, (_, index) => index + 1)
addDoc('Progression/Magic_School_XP.md', block('# Magic school XP', '', '## Level Curve', '', table(['Level', 'XP to Next', 'Total XP'], schoolLevels.map((level) => [level, formatNumber(SCHOOL_LEVEL_XP(level)), formatNumber(schoolLevels.slice(0, level).reduce((sum, current) => sum + SCHOOL_LEVEL_XP(current), 0))])), '', table(['Setting', 'Value'], [['Starting School cap', formatNumber(BALANCE.schoolProgression.startingCap)], ['Tutorial-complete School cap', formatNumber(BALANCE.schoolProgression.tutorialCompleteCap)]]), '', '## School Sources', '', table(['School', 'XP Sources', 'Base XP Rate', 'Notes'], Object.values(SCHOOLS).map((school) => [school.name, 'Research and spell progression', dash, school.tagline]))), ['SCHOOL_LEVEL_XP'].concat(Object.keys(BALANCE.schoolProgression).map((key) => 'schoolProgression.' + key)), ['src/game/core/balance/balance.ts', 'src/game/systems/spells'])

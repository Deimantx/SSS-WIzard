...
export function GameShell() {
  ...
  return <div className="game-shell">
    ...
    <main className={`main-area ${editor.isEditing ? 'editor-open' : ''}`}>
      <header className="topbar topbar-v2">
        ...
        <div className="topbar-metrics">
          <div className="topbar-primary"><ResourceChip tone="hp" icon="♥" label="HP" value={`${formatNumber(player.health)} / ${formatNumber(player.maxHealth)}`} percent={player.health / player.maxHealth * 100} /><ResourceChip tone="mana" icon="✦" label="MANA" value={`${formatNumber(player.mana)} / ${formatNumber(player.maxMana)}`} percent={player.mana / player.maxMana * 100} /><div className="resource-chip focus resource-chip-focus" title={`${usedFocus} / ${player.maxFocus} Focus reserved`}><span className="resource-icon">◈</span><div><small>FOCUS</small><strong>{formatNumber(freeFocus)} FREE</strong><em>{usedFocus} / {player.maxFocus} reserved</em></div><div className="mini-bar"><i style={{ width: `${usedFocus / player.maxFocus * 100}%` }} /></div></div></div>
          <div className="topbar-secondary"><div className="secondary-metric" title="Mana regeneration"><small>REGEN</small><strong>↗ +{manaRegen}/s</strong></div><div className="secondary-metric" title="Current player status"><small>STATUS</small><strong>⚔ {combatStatus}</strong></div><div className="secondary-metric" title={`Offline Bank · Banked time: ${Math.max(0, Math.floor(offlineBankMs / 1000))} seconds`}><small>OFFLINE</small><strong>◷ {formatOfflineBank(offlineBankMs)}</strong></div></div>
        </div>
    ...
    <div className="toast-stack">{notifications.map((note) => <div className={`toast ${note.tone}`} key={note.id}><span>{note.tone === 'success' ? '✦' : note.tone === 'warning' ? '!' : '·'}</span><div>{note.text}</div><button onClick={() => dismissNotification(note.id)} aria-label="Dismiss notification"><X size={13} /></button></div>)}</div>
  </div>
}

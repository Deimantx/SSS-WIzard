...
import { Anvil, BookOpen, CircleDotDashed, FlaskConical, Gem, Home, Library, Package, PawPrint, Settings, Shield, Swords, Target, WandSparkles, Zap } from 'lucide-react'
...
export interface NavigationItem { id: ScreenId; label: string; icon: LucideIcon; hint: string; visibility?: { type: 'story-event-triggered'; eventId: StoryEventId } }
...
export const navigationGroups: NavigationGroup[] = [
  ...
  { id: 'combat', label: 'Combat', breadcrumb: 'Combat', items: [{ id: 'combat', label: 'Combat', icon: Swords, hint: 'Campaign progression' }] },
  ...
  { id: 'tower', label: 'Wizard Tower', breadcrumb: 'Wizard Tower', items: [{ id: 'tower-channeling', label: 'Channeling', icon: Zap, hint: 'Draw Mana into the tower' }, { id: 'tower-focus', label: 'Focus', icon: Target, hint: 'Review Focus reservations' }, { id: 'tower-transmutation', label: 'Transmutation', icon: WandSparkles, hint: 'Shape elemental materials' }, { id: 'tower-artificing', label: 'Artificing', icon: Anvil, hint: 'Forge magical equipment' }, { id: 'tower-research', label: 'Research', icon: FlaskConical, hint: 'Deepen a Magic School' }, { id: 'tower-dark-portal', label: 'Dark Portal', icon: CircleDotDashed, hint: 'Enter the chamber awakened by the Black Portal Shard', visibility: { type: 'story-event-triggered', eventId: 'edrin-dark-portal-discovery' } }] },
...
export const isNavigationItemVisible = (item: NavigationItem, state: Pick<GameState, 'storyProgress'>) => !item.visibility || item.visibility.type !== 'story-event-triggered' || isStoryEventTriggered(state, item.visibility.eventId)

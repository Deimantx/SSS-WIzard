    ...
    <nav className="nav-list" aria-label="Main navigation">
      {navigationGroups.map((group) => {
        ...
        return <section className={`nav-group ${collapsed ? 'collapsed' : ''}`} key={group.id}>
          ...
          <div className="nav-group-items" aria-hidden={collapsed} inert={collapsed}>{group.items.map(({ id, label, icon: Icon, hint }) => { const destination = id === 'inventory' || id === 'collection' || id === 'bestiary' || id === 'schools' || id === 'tower-transmutation' ? id === 'tower-transmutation' ? 'transmutation' : id : null; const hasNew = destination ? hasUnseenAttention(attention, destination) : false; return <GameTooltip block key={id} content={hasNew ? `${hint} · New discoveries available` : hint}><button key={id} className={`nav-item ${screen === id ? 'active' : ''}`} onClick={() => setScreen(id)} aria-label={`${label}${hasNew ? ', new discoveries available' : ''}`}><Icon size={16} /><span>{label}</span>{hasNew && <i className="nav-attention-dot" aria-hidden="true" />}{screen === id && <ChevronRight className="nav-chevron" size={14} />}</button></GameTooltip> })}</div>
        </section>
      })}
    </nav>

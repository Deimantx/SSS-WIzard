...
export function SpellCardTooltip({ presentation }: { presentation: SpellDetailPresentation }) {
  ...
  return <div className="game-tooltip-content game-tooltip-rich spell-card-tooltip" data-detail-mode={advanced ? 'advanced' : 'compact'}>
    ...
    <section className="spell-card-tooltip-section"><span className="spell-card-tooltip-section-label">CORE CASTING</span><div className="spell-card-tooltip-core"><div className="spell-card-tooltip-metric"><small>MANA</small><strong className="ui-mana">{presentation.manaCost}</strong></div><div className="spell-card-tooltip-metric"><small>COOLDOWN</small><strong className="ui-time">{presentation.cooldownLabel}</strong></div><div className="spell-card-tooltip-metric"><small>AUTO-CAST FOCUS</small><strong className="ui-focus">{presentation.autoCastFocus} Focus</strong>{presentation.autoCastActive && <em className="spell-card-tooltip-active">ACTIVE</em>}</div></div></section>

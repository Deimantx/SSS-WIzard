...
import { getInspectorInlineEffectRows, getSpellbookTooltipRows } from '../../game/presentation/spells/spellDetailPresentation'
...
export function SpellEffectDetailBlock({ model, density, showSource = false }: { model: SpellEffectTooltipModel; density: SpellEffectDetailDensity; showSource?: boolean }) {
  ...
  return <section className={`spell-effect-detail-block is-${density} effect-${model.categoryKey}`} data-school={model.school} style={style}><div className="spell-effect-detail-heading"><span className="spell-effect-detail-category">{model.category}</span><strong>{model.title}</strong><p>{model.description}</p></div><div className="spell-effect-detail-grid">{rows.map((row, index) => <EffectDetailRow row={row} key={`${row.label}-${index}`} />)}</div></section>
}

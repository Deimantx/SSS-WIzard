...
export function Button({ children, onClick, variant = 'primary', disabled = false, className = '', tooltip, ariaLabel, ariaPressed, icon = false, uiSound, dataStaticMotion = false, dataNoDrag = false }: { children: ReactNode; onClick?: () => void; variant?: 'primary' | 'secondary' | 'ghost' | 'danger' | 'success'; disabled?: boolean; className?: string; tooltip?: TooltipNode; ariaLabel?: string; ariaPressed?: boolean; icon?: boolean; uiSound?: 'click' | 'confirm' | 'none'; dataStaticMotion?: boolean; dataNoDrag?: boolean }) {
  const button = <button aria-label={ariaLabel} aria-pressed={ariaPressed} data-ui-sound={uiSound} data-static-motion={dataStaticMotion ? 'true' : undefined} data-no-drag={dataNoDrag ? 'true' : undefined} className={`button ${variant} ${icon ? 'icon' : ''} ${className}`} onClick={onClick} disabled={disabled}>{children}</button>

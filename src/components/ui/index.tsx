...
export function Button({ children, onClick, variant = 'primary', disabled = false, className = '', tooltip, ariaLabel, ariaPressed, icon = false, uiSound }: { children: ReactNode; onClick?: () => void; variant?: 'primary' | 'secondary' | 'ghost' | 'danger' | 'success'; disabled?: boolean; className?: string; tooltip?: TooltipNode; ariaLabel?: string; ariaPressed?: boolean; icon?: boolean; uiSound?: 'click' | 'confirm' | 'none' }) {
  const button = <button aria-label={ariaLabel} aria-pressed={ariaPressed} data-ui-sound={uiSound} className={`button ${variant} ${icon ? 'icon' : ''} ${className}`} onClick={onClick} disabled={disabled}>{children}</button>

...
export const setDeveloperToolsGeometry = (next: Partial<DeveloperToolsGeometry>, persist = true) => update(clampDeveloperToolsGeometry({ x: current.x, y: current.y, width: current.width, height: current.height, minimized: current.minimized, minimizedX: current.minimizedX, minimizedY: current.minimizedY, ...next }), persist)
...
export const setDeveloperToolsMinimizedPosition = (x: number, y: number, persist = true) => setDeveloperToolsGeometry({ minimizedX: x, minimizedY: y }, persist)
export const minimizeDeveloperTools = () => update(clampDeveloperToolsGeometry({ ...current, minimized: true, minimizedX: current.x, minimizedY: current.y }), true)

...
describe('Developer Artifacts tab', () => {
  ...
  it('provides artifact selection, progression controls, overrides, and batch actions', () => {
    ...
    fireEvent.change(selector, { target: { value: 'ember-staff' } })
    expect(screen.getByText('Ember Staff')).toBeTruthy()
    fireEvent.click(screen.getByRole('checkbox', { name: /Free upgrade/ }))
    act(() => useGameStore.getState().debugSetArtifactLevel('ember-staff', 1))
    act(() => expect(useGameStore.getState().upgradeArtifact('ember-staff')).toBe(true))
    ...
    expect(useGameStore.getState().artifactProgress['ember-staff']?.level).toBe(1)
    ...
    expect(useGameStore.getState().artifactProgress['ember-staff']?.level).toBe(10)

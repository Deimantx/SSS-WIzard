...
const slots: readonly FilterOption<'all' | EquipmentItemSlot>[] = [{ value: 'all', label: 'ALL' }, ...EQUIPMENT_ITEM_SLOTS.map(value => ({ value, label: EQUIPMENT_ITEM_SLOT_LABELS[value].toUpperCase() }))]
...
export function DeveloperArtificing() {
  ...
  return <Card title="Artificing Artifact browser" className="developer-browser-card">
    <div className="developer-filter-stack"><SearchInput value={query} onChange={setQuery} placeholder="Search Artifact recipes..." /><div className="developer-filter-label">SLOT<FilterBar options={slots} value={filters.slotFilter} onChange={value => setFilters(current => ({ ...current, slotFilter: value }))} ariaLabel="Developer Artificing slot" /></div><div className="developer-filter-label">OWNERSHIP<FilterBar options={ownership} value={filters.ownershipFilter} onChange={value => setFilters(current => ({ ...current, ownershipFilter: value }))} ariaLabel="Developer Artificing ownership" /></div><div className="developer-filter-label">UNLOCK STATUS<FilterBar options={statuses} value={unlockFilter} onChange={setUnlockFilter} ariaLabel="Developer Artificing unlock status" /></div><div className="developer-filter-label">CRAFTABILITY<FilterBar options={craftability} value={craftFilter} onChange={setCraftFilter} ariaLabel="Developer Artificing craftability" /></div><div className="developer-debug-toggle"><Button variant={state.debug.showLockedArtificingRecipes ? 'secondary' : 'ghost'} tooltip="Reveal locked Artificing recipes on the game screen for inspection. Normal crafting still requires the authored unlock." onClick={() => state.setDebugShowLockedArtificingRecipes(!state.debug.showLockedArtificingRecipes)}>{state.debug.showLockedArtificingRecipes ? 'HIDE LOCKED ARTIFACT RECIPES' : 'SHOW LOCKED ARTIFACT RECIPES'}</Button><small>This does not unlock normal crafting. The tester browser always lists all authored Artifacts.</small></div></div>
    <DeveloperBrowserLayout browser={<><div className="developer-browser-heading"><strong>{visible.length} / {ARTIFICING_RECIPE_ORDER.length} Artifact recipes</strong></div><DeveloperBrowser items={visible.map(entry => ({ id: entry.id, label: entry.name, icon: ITEMS[entry.output.itemId].icon, meta: 'STARTER ARTIFACT', status: <Status tone={isRecipeUnlocked(state, entry) ? 'neutral' : 'locked'}>{isRecipeUnlocked(state, entry) ? 'UNLOCKED' : 'LOCKED'}</Status> }))} selectedId={selected} onSelect={id => setSelected(id as ArtificingRecipeId)} emptyMessage="No matching Artifacts." /></>} inspector={selectedRecipe ? <>
      <div className="developer-inspector-title"><ItemIcon itemId={recipe.output.itemId} size="tile" /><div><h2>{recipe.name}</h2><small>{getArtificingProfile(recipe)} · STARTER ARTIFACT</small></div><Status tone={unlocked ? 'neutral' : 'locked'}>{unlocked ? 'UNLOCKED' : 'LOCKED'}</Status></div>
    ...
    </> : <div className="developer-browser-empty"><strong>No matching Artifacts</strong><small>Change the search or filters.</small></div>} />
  </Card>
}

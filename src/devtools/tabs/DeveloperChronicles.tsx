...
import { getChronicleChapterProgress, getChronicleMainObjective, getChronicleObjectiveStatus, type ChronicleObjectiveStatus } from '../../game/systems/chronicles/chronicleRuntime'
...
export function DeveloperChronicles() {
  ...
  return <div className="developer-tab-stack developer-chronicles-tab">
    <Card title="Chronicles · tester workspace">
      ...
      <div className="developer-button-grid"><Button variant="secondary" onClick={() => run('Reconciled authored evidence.', state.debugReconcileChronicles)}>Reconcile Now</Button><Button variant="ghost" onClick={() => run('Skipped the current main objective.', state.debugSkipCurrentChronicleObjective)}>Skip Current</Button><Button variant="ghost" disabled={!selected} onClick={() => selected && run('Completed the selected objective.', () => state.debugCompleteChronicleObjective(selected.id))}>Complete Objective</Button><Button variant="ghost" disabled={!selected} onClick={() => selected && run('Completed prerequisites for the selected objective.', () => state.debugCompleteChroniclePrerequisites(selected.id))}>Complete Prerequisites</Button><Button variant="secondary" onClick={() => run('Chapter unlocked for testing.', () => state.debugUnlockChronicleChapter(chapterId))}>Unlock Chapter</Button><Button variant="secondary" onClick={() => run('Required objectives completed.', () => state.debugCompleteChronicleRequiredObjectives(chapterId))}>Complete Required</Button><Button variant="ghost" onClick={() => run('Optional objectives completed.', () => state.debugCompleteChronicleOptionalObjectives(chapterId))}>Complete Optional</Button></div>

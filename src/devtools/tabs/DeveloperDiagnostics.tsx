...
export function DeveloperDiagnostics({ copy }: { copy: (label: string, value: unknown) => Promise<void> }) {
  ...
  return <div className="developer-tab-grid">
    <Card title="Runtime diagnostics"><div className="developer-summary-grid"><Summary label="Save version" value={`v${state.saveVersion}`} /><Summary label="Debug state" value={debugActive ? 'Active' : 'Clean'} /><Summary label="Mana" value={`${formatResourceAmount(state.player.mana)} / ${formatResourceAmount(state.player.maxMana)}`} /><Summary label="Mana regen" value={`+${formatResourceAmount(regen.total)}/s`} /><Summary label="Capacity order" value={`${capacity.preAmplification} × ${capacity.astralExpansionMultiplier}`} /><Summary label="Focus" value={`${used} used / ${rawFree} available`} /><Summary label="Notifications" value={state.notifications.length} /><Summary label="Combat" value={state.combat.enemyId ?? 'None'} /></div><div className="developer-diagnostics"><span>Finite numeric state <b>{hasInvalidNumber ? 'FAILED' : 'OK'}</b></span><span>Serialized debug overrides <b>excluded</b></span><span>Developer regen source <b>+{formatResourceAmount(regen.developerBonus)}/s</b></span><span>Developer capacity source <b>+{capacity.developerCapacityBonus}</b></span></div></Card>

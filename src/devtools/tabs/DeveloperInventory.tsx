...
type InventorySourceFilter = 'all' | 'transmutation' | DungeonId | 'monster-drops' | 'boss-drops'
    ...
    <Card title="Inventory browser" className="developer-browser-card">
      <div className="developer-filter-stack">
        ...
        inspector={selectedItem && selected ? <>
          ...
          {selectedItem.kind === 'equipment' && <DeveloperSection title="Equipment actions"><div className="button-row"><Button onClick={addSelected}>Add</Button><Button variant="secondary" onClick={addAndEquipSelected}>Add &amp; Equip</Button><Button variant="secondary" onClick={() => equipItem(selected, getDefaultEquipmentPosition(selectedItem.equipmentSlot))}>Equip</Button></div>{equippedPositions.length > 0 && <div className="button-row">{equippedPositions.map((position) => <Button key={position} variant="ghost" onClick={() => unequipItem(position)}>Unequip {EQUIPMENT_POSITION_LABELS[position]}</Button>)}</div>}</DeveloperSection>}

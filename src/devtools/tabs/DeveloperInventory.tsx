...
import { ARTIFACT_EQUIPMENT_IDS, BOSS_SIGNATURE_EQUIPMENT_IDS, getDungeonRegularEquipment, isBossSignatureEquipment } from '../../game/content/equipment/equipmentSets'
...
const countLoadoutItems = (slots: DeveloperEquipmentLoadout['slots']) => Object.values(slots).reduce<Partial<Record<ItemId, number>>>((counts, itemId) => {
...
export function DeveloperInventory({ initialView = 'all' }: { initialView?: InventoryCategoryFilter } = {}) {
  ...
  return <div className="developer-tab-stack">
    <Card title="Inventory browser" className="developer-browser-card">
      <div className="developer-filter-stack">
        ...
        inspector={selectedItem && selected ? <>
          ...
          <DeveloperAdvancedSection title="Advanced item details"><span>Item identifier: <code>{selected}</code></span>{selectedRecipe && <span>Recipe identifier: <code>{selectedRecipe.id}</code></span>}</DeveloperAdvancedSection>
          ...
          {selectedItem.kind === 'equipment' && <DeveloperSection title="Equipment actions"><div className="button-row"><Button onClick={addSelected}>Add</Button><Button variant="secondary" onClick={addAndEquipSelected}>Add &amp; Equip</Button>{selectedItem.equipmentSlot === 'ring' ? <><Button variant="secondary" onClick={() => equipItem(selected, 'ring1')}>Equip Ring 1</Button><Button variant="secondary" onClick={() => equipItem(selected, 'ring2')}>Equip Ring 2</Button></> : selectedItem.equipmentSlot === 'earring' ? <><Button variant="secondary" onClick={() => equipItem(selected, 'earring1')}>Equip Earring 1</Button><Button variant="secondary" onClick={() => equipItem(selected, 'earring2')}>Equip Earring 2</Button></> : <Button variant="secondary" onClick={() => equipItem(selected, getDefaultEquipmentPosition(selectedItem.equipmentSlot))}>Equip</Button>}</div>{equippedPositions.length > 0 && <div className="button-row">{equippedPositions.map((position) => <Button key={position} variant="ghost" onClick={() => unequipItem(position)}>Unequip {EQUIPMENT_POSITION_LABELS[position]}</Button>)}</div>}</DeveloperSection>}
    ...
    <div className="developer-tab-grid">
      <Card title="Quick groups"><p className="muted">All grants use normal acquisition. Removal respects protected and equipped item rules.</p><div className="developer-button-grid"><Button variant="secondary" onClick={() => addGroup(MATERIAL_IDS.filter((id) => ITEMS[id].category === 'elemental'), 10)}>Add elemental fragments ×10</Button><Button variant="secondary" onClick={() => addGroup(DUNGEON_MATERIAL_IDS, 100)}>Add Artifact Essence ×100</Button><Button variant="secondary" onClick={() => addGroup(ARTIFACT_EQUIPMENT_IDS, 1)}>Add all Artifacts</Button><Button variant="secondary" onClick={() => addGroup(EQUIPMENT_IDS, 1)}>Add 1 of every equipment</Button>{DUNGEON_ORDER.map((id) => <Button key={id} variant="secondary" onClick={() => addGroup(getDungeonRegularEquipment(id), 1)}>Add {DUNGEONS[id].name} equipment</Button>)}<Button variant="secondary" onClick={() => addGroup(BOSS_SIGNATURE_EQUIPMENT_IDS, 1)}>Add boss signature equipment</Button><Button variant="ghost" onClick={() => DUNGEON_MATERIAL_IDS.forEach((id) => { const current = useGameStore.getState(); const amount = current.inventory[id] ?? 0; if (amount > 0) removeItem(id, amount) })}>Clear Artifact Essence</Button><Button variant="ghost" onClick={clearEquipmentInventory}>Clear unprotected equipment</Button></div><div className="developer-owned-list">{ITEM_IDS.filter((id) => (state.inventory[id] ?? 0) > 0).map((id) => <span key={id}>{ITEMS[id].name}<strong>{state.inventory[id]}</strong></span>)}</div></Card>

...
import { formatActionPattern, formatCombatEffect, formatDuration, formatPercent, formatReadableId } from '../../game/content/presentation/balanceFormatters'
...
function MonsterInspector({ monsterId, healthPercent, setHealthPercent, barrier, setBarrierValue, combat, spawn, setHealth, setBarrier, setThreat, kill, clearStatuses, forceAction, startAction, resolveAction, advanceAction, setPattern, next, jumpBoss }: { monsterId: MonsterId; healthPercent: number; setHealthPercent: (value: number) => void; barrier: number; setBarrierValue: (value: number) => void; combat: ReturnType<typeof useGameStore.getState>['combat']; spawn: ReturnType<typeof useGameStore.getState>['spawnDebugEnemy']; setHealth: ReturnType<typeof useGameStore.getState>['setEnemyHealthPercent']; setBarrier: ReturnType<typeof useGameStore.getState>['setEnemyBarrier']; setThreat: ReturnType<typeof useGameStore.getState>['setThreat']; kill: ReturnType<typeof useGameStore.getState>['killCurrentEnemy']; clearStatuses: ReturnType<typeof useGameStore.getState>['clearEnemyStatuses']; forceAction: ReturnType<typeof useGameStore.getState>['forceEnemyAction']; startAction: ReturnType<typeof useGameStore.getState>['startEnemyAction']; resolveAction: ReturnType<typeof useGameStore.getState>['resolveCurrentEnemyAction']; advanceAction: ReturnType<typeof useGameStore.getState>['advanceEnemyAction']; setPattern: ReturnType<typeof useGameStore.getState>['setEnemyActionPattern']; next: ReturnType<typeof useGameStore.getState>['fastResolveDebugEnemies']; jumpBoss: ReturnType<typeof useGameStore.getState>['jumpDebugToBoss'] }) {
  ...
  const defense = monster.defense ?? DEFAULT_ENEMY_DEFENSE
  const critChance = monster.critChance ?? DEFAULT_ENEMY_CRIT_CHANCE
  const critDamage = monster.critDamage ?? DEFAULT_ENEMY_CRIT_DAMAGE_MULTIPLIER
  return <>
    <div className="developer-inspector-title"><span className="developer-browser-icon" style={{ color: monster.color }}>{monster.ui?.portraitIcon ?? '◆'}</span><div><h2>{monster.name}</h2><small className="muted">{dungeon?.dungeonName ?? 'Unassigned'} · {isBossMonster(monster) ? 'Boss' : 'Normal enemy'}</small></div><Status tone={isBossMonster(monster) ? 'warning' : 'neutral'}>{isBossMonster(monster) ? 'BOSS' : 'NORMAL'}</Status></div>
    <DeveloperSection title="Combat stats"><div className="developer-detail-grid"><span>MAX HEALTH<strong>{monster.maxHealth}</strong></span><span>BASIC DAMAGE<strong>{monster.basicAttackDamage}</strong></span><span>BASIC ATTACK<strong>{formatDuration(monster.basicAttackTimeMs)}</strong></span><span>DEFENSE<strong>{defense}</strong></span><span>CRITICAL CHANCE<strong>{formatPercent(critChance)}</strong></span><span>CRITICAL DAMAGE<strong>{formatPercent(critDamage)}</strong></span><span>BLOCK CHANCE<strong>{formatPercent(monster.blockChance ?? 0)}</strong></span><span>NORMAL KILLS<strong>{useGameStore.getState().progress.lifetimeKillsByMonster[monsterId] ?? 0}</strong></span><span>BOSS KILLS<strong>{useGameStore.getState().progress.bossKillsByBoss[monsterId] ?? 0}</strong></span></div><p className="muted">{monster.subtitle}</p></DeveloperSection>
    ...
    <DeveloperSection title="Traits and specials"><div className="developer-relation-list">{monster.traitIds.map((id) => { const trait = getTraitDefinition(id); return <span key={id}><strong>{trait?.name ?? formatReadableId(id)}</strong><small>{trait?.description ?? 'No description available.'}</small>{trait?.modifiers?.map((modifier) => <small key={`${id}-${modifier.key}`}>{formatReadableId(modifier.key)}: {modifier.value}</small>)}{trait?.rules?.map((rule) => <small key={rule.id}>{rule.effects.map(formatCombatEffect).join('; ')}</small>)}</span> })}</div></DeveloperSection>
    <DeveloperSection title="Encounter actions"><div className="developer-relation-list">{Object.values(monster.actions).map((action) => <span key={action.id}><strong>{action.name}</strong><small>{action.description} · action time: {formatDuration(action.actionTimeMs)}</small><small>{action.effects.map(formatCombatEffect).join('; ') || 'No direct effect.'}</small></span>)}</div><div className="developer-relation-list">{Object.values(monster.actionPatterns).map((pattern) => <span key={pattern.id}><strong>{formatReadableId(pattern.id)} pattern</strong><small>{formatActionPattern(pattern, monster.actions)}</small></span>)}</div></DeveloperSection>

...
function MonsterInspector({ monsterId, healthPercent, setHealthPercent, barrier, setBarrierValue, combat, spawn, setHealth, setBarrier, setThreat, kill, clearStatuses, forceAction, startAction, resolveAction, advanceAction, setPattern, next, jumpBoss }: { monsterId: MonsterId; healthPercent: number; setHealthPercent: (value: number) => void; barrier: number; setBarrierValue: (value: number) => void; combat: ReturnType<typeof useGameStore.getState>['combat']; spawn: ReturnType<typeof useGameStore.getState>['spawnDebugEnemy']; setHealth: ReturnType<typeof useGameStore.getState>['setEnemyHealthPercent']; setBarrier: ReturnType<typeof useGameStore.getState>['setEnemyBarrier']; setThreat: ReturnType<typeof useGameStore.getState>['setThreat']; kill: ReturnType<typeof useGameStore.getState>['killCurrentEnemy']; clearStatuses: ReturnType<typeof useGameStore.getState>['clearEnemyStatuses']; forceAction: ReturnType<typeof useGameStore.getState>['forceEnemyAction']; startAction: ReturnType<typeof useGameStore.getState>['startEnemyAction']; resolveAction: ReturnType<typeof useGameStore.getState>['resolveCurrentEnemyAction']; advanceAction: ReturnType<typeof useGameStore.getState>['advanceEnemyAction']; setPattern: ReturnType<typeof useGameStore.getState>['setEnemyActionPattern']; next: ReturnType<typeof useGameStore.getState>['fastResolveDebugEnemies']; jumpBoss: ReturnType<typeof useGameStore.getState>['jumpDebugToBoss'] }) {
  ...
  const lifeEssenceReward = resolveLifeEssenceRewardRange(monsterId, state.worldTier.current)
  return <>
    ...
    <DeveloperSection title="Resonance Harvest"><div className="developer-detail-grid"><span>BASE PROFILE<strong>{getNonZeroResonanceEntries(resonanceReward.baseYield).length ? getNonZeroResonanceEntries(resonanceReward.baseYield).map(({ label, amount }) => `${label} ${formatResonanceAmount(amount)}`).join(' · ') : 'No Phase 1 Resonance profile authored.'}</strong></span><span>WORLD TIER<strong>WT{resonanceReward.worldTier}</strong></span><span>WT / GLOBAL<strong>{resonanceReward.worldTierRewardMultiplier.toFixed(2)}× / {resonanceReward.globalRewardMultiplier.toFixed(2)}×</strong></span><span>EFFECTIVE MULTIPLIER<strong>{resonanceReward.rewardMultiplier.toFixed(2)}×</strong></span><span>FINAL REWARD<strong>{getNonZeroResonanceEntries(resonanceReward.finalYield).length ? getNonZeroResonanceEntries(resonanceReward.finalYield).map(({ label, amount }) => `${label} ${formatResonanceAmount(amount)}`).join(' · ') : '0'}</strong></span></div></DeveloperSection>
    ...
    <DeveloperSection title="Loot"><div className="developer-relation-list">{monster.loot.map((drop) => <span key={drop.itemId}><strong>{ITEMS[drop.itemId]?.name ?? formatReadableId(drop.itemId)}</strong><small>{drop.min}–{drop.max} items · {formatPercent(drop.chance)} chance</small></span>)}<span key="life-essence"><strong>{ITEMS['life-essence'].name}</strong><small>{lifeEssenceReward.finalMin}–{lifeEssenceReward.finalMax} items · 100% guaranteed at WT{lifeEssenceReward.worldTier}</small></span></div></DeveloperSection>

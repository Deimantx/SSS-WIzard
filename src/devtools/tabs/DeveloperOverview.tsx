...
export function DeveloperOverview() {
  ...
  return <div className="developer-tab-grid"><Card title="Runtime summary"><div className="developer-summary-grid"><Summary label="Screen" value={state.ui.screen} /><Summary label="Save version" value={`v${state.saveVersion}`} /><Summary label="HP" value={`${Math.floor(state.player.health)} / ${state.player.maxHealth}`} /><Summary label="Mana" value={`${Math.floor(state.player.mana)} / ${state.player.maxMana}`} /><Summary label="Focus" value={`${free} free  -  ${used} reserved`} /><Summary label="Magic cap" value={state.progress.magicLevelCap} /><Summary label="Combat" value={state.combat.active ? state.combat.enemyId ? `Fighting ${state.combat.enemyId}` : 'Recovery' : 'Inactive'} /><Summary label="Threat" value={state.combat.threatCleared} /><Summary label="Guild" value={`${state.progress.guildRank}  -  ${state.progress.guildReputation} rep`} /><Summary label="Offline" value={formatOfflineBank(state.offlineBankMs)} /><Summary label="Activities" value={reservations.length} /><Summary label="Viewport" value={typeof window === 'undefined' ? '-' : `${window.innerWidth} x ${window.innerHeight}`} /></div></Card><Card title="Quick presets"><p className="muted">Presets reset gameplay state and prepare a focused test scenario.</p><div className="developer-button-grid">{presets.map((item) => <Button key={item.id} variant="secondary" onClick={() => preset(item.id)}>{item.label}</Button>)}</div></Card></div>
}

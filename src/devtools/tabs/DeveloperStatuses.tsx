...
import { formatCombatEffect, formatCombatModifier, formatCombatRule, formatDuration, formatReadableId } from '../../game/content/presentation/balanceFormatters'
...
function StatusInspector({ statusId, playerStatuses, enemyStatuses, playerCount, enemyCount, applyPlayer, applyEnemy, applyPlayerThree, applyEnemyThree, removePlayer, removeEnemy, clearPlayer, clearEnemy }: { statusId: StatusId; playerStatuses: ActiveStatus[]; enemyStatuses: ActiveStatus[]; playerCount: number; enemyCount: number; applyPlayer: (id: StatusId) => void; applyEnemy: (id: StatusId) => void; applyPlayerThree: () => void; applyEnemyThree: () => void; removePlayer: (id: StatusId) => void; removeEnemy: (id: StatusId) => void; clearPlayer: () => void; clearEnemy: () => void }) {
  ...
  return <>
    <div className="developer-inspector-title"><div><h2>{status.name}</h2><small className="muted">{status.classification} · {formatDuration(status.defaultDurationMs)}</small></div><Status tone={status.classification === 'debuff' ? 'warning' : 'success'}>{status.classification.toUpperCase()}</Status></div>
    <DeveloperSection title="Status details"><p className="muted">{status.description}</p><div className="developer-detail-grid"><span>DURATION<strong>{formatDuration(status.defaultDurationMs)}</strong></span><span>STACKING<strong>{formatReadableId(status.stacking.mode)}{status.stacking.maxStacks ? ` · up to ${status.stacking.maxStacks}` : ''}</strong></span><span>CLEANSE<strong>{status.cleanseable ? 'Allowed' : 'Not allowed'}</strong></span><span>DISPEL<strong>{status.dispellable ? 'Allowed' : 'Not allowed'}</strong></span><span>PLAYER INSTANCES<strong>{playerCount}</strong></span><span>ENEMY INSTANCES<strong>{enemyCount}</strong></span></div><div className="developer-relation-list">{status.tags.map((tag) => <span key={tag}><strong>{formatReadableId(tag)}</strong><small>Status tag</small></span>)}</div></DeveloperSection>
    {status.modifiers && <DeveloperSection title="Effects"><div className="developer-relation-list">{status.modifiers.map((modifier) => <span key={modifier.key}><strong>Modifier</strong><small>{formatCombatModifier(modifier)}</small></span>)}</div></DeveloperSection>}
    {status.periodic && <DeveloperSection title="Ongoing effect"><div className="developer-detail-grid"><span>TICK INTERVAL<strong>{formatDuration(status.periodic.intervalMs)}</strong></span><span>EFFECTS<strong>{status.periodic.effects.length}</strong></span></div><div className="developer-relation-list">{status.periodic.effects.map((effect, index) => <span key={`periodic-${index}`}><strong>Tick {index + 1}</strong><small>{formatCombatEffect(effect, { statusHolder: true })}</small></span>)}</div></DeveloperSection>}
    {status.triggers && <DeveloperSection title="Special triggers"><div className="developer-relation-list">{status.triggers.map((rule) => <span key={rule.id}><strong>{formatReadableId(rule.event.replace(/^on-/, ''))}</strong><small>{formatCombatRule(rule)}</small></span>)}</div></DeveloperSection>}

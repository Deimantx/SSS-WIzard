...
import { getRecipeConsumableRequirements, getRecipeCurrentEffectiveDuration, getRecipeManaDemandPerSecond, getRecipeOutputPerHour, getRecipeStatus, getTransmutationEchoCapacity, getTransmutationEchoesAssigned } from '../../game/systems/transmutation/transmutationSelectors'
import { TRANSMUTATION_ARRAYS, TRANSMUTATION_ARRAY_IDS } from '../../game/content/transmutation/transmutationArrays'
import { getEffectiveTransmutationManaCost, getTransmutationArrayBonuses } from '../../game/systems/transmutation/transmutationArrays'
  ...
  return <div className="developer-tab-stack">
    <Card title="Transmutation recipe browser" className="developer-browser-card">
      ...
      <DeveloperBrowserLayout browser={<><div className="developer-browser-heading"><strong>{recipeOptions.length} authored recipes</strong><small>Inspect outputs, ingredients, unlock requirements, and test actions.</small></div><DeveloperBrowser items={recipeOptions.map((id) => { const entry = RECIPES[id]; return { id, label: entry.name, icon: ITEMS[entry.output.itemId].icon, accent: ITEMS[entry.output.itemId].color, meta: `${formatReadableId(entry.category)} · ${entry.output.quantity} ${ITEMS[entry.output.itemId].name}`, status: <Status tone={getRecipeStatus(state, entry) === 'locked' ? 'locked' : getRecipeStatus(state, entry) === 'active' ? 'success' : 'neutral'}>{formatReadableId(getRecipeStatus(state, entry))}</Status> } })} selectedId={selected} onSelect={(id) => setSelected(id as TransmutationRecipeId)} emptyMessage="No matching recipes." /></>} inspector={recipeOptions.length > 0 ? <>
        ...
        <DeveloperSection title="Recipe definition"><p className="muted">{recipe.description}</p><div className="developer-detail-grid"><span>OUTPUT<strong>{ITEMS[recipe.output.itemId].name} × {recipe.output.quantity}</strong></span><span>CATEGORY<strong>{formatReadableId(recipe.category)}</strong></span><span>BASE CRAFT TIME<strong>{formatDuration(recipe.baseDurationMs)}</strong></span><span>MANA / CYCLE<strong>{formatResourceAmount(getEffectiveTransmutationManaCost(state, recipe))}</strong></span><span>ECHOES<strong>{job?.echoesAssigned ?? 0} / {capacity}</strong></span><span>EXPECTED OUTPUT / H<strong>{getRecipeOutputPerHour(recipe, job?.echoesAssigned ?? 0, state).toFixed(2)}</strong></span></div></DeveloperSection>
        ...
        <DeveloperSection title="Unlock and activity state"><p className="developer-relation-line">{formatRecipeUnlock(recipe.unlock)}</p><div className="developer-detail-grid"><span>RUNNING JOB<strong>{job ? 'Yes' : 'No'}</strong></span><span>PROGRESS<strong>{job ? `${formatDuration(job.progressMs)} / ${formatDuration(recipe.baseDurationMs)}` : 'Not started'}</strong></span><span>EFFECTIVE TIME<strong>{job ? formatDuration(getRecipeCurrentEffectiveDuration(recipe, job.echoesAssigned, state) ?? 0) : 'Paused'}</strong></span><span>MANA / SECOND<strong>{formatResourceRate(getRecipeManaDemandPerSecond(recipe, job?.echoesAssigned ?? 0, state))}</strong></span><span>MISSING FOR ONE<strong>{missingForOne}</strong></span></div></DeveloperSection>
      <DeveloperSection title="Recipe test actions"><div className="button-row"><Button onClick={() => grantMissing(1)}>Grant missing ingredients</Button><Button variant="secondary" onClick={() => grantMissing(10)}>Grant 10× missing</Button><Button variant="ghost" onClick={removeIngredients} disabled={requirements.length === 0}>Remove ingredients</Button><Button variant="secondary" onClick={() => setEchoes(selected, 1)}>Assign one Echo</Button><Button variant="ghost" onClick={() => setEchoes(selected, 0)}>Clear Echoes</Button><Button variant="success" onClick={() => state.completeTransmutationCycle(selected)}>Complete one cycle</Button></div></DeveloperSection>

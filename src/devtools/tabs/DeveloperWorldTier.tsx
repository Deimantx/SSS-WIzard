...
export function DeveloperWorldTier() {
  ...
  return <div className="developer-tab-stack developer-world-tier-tab">
    <Card title="WORLD TIER · Overview" action={<Status tone={state.worldTier.highestUnlocked > 1 ? 'success' : 'neutral'}>{state.worldTier.highestUnlocked > 1 ? 'WT2 UNLOCKED' : 'WT1 ONLY'}</Status>}>
      <div className="developer-summary-grid"><Summary label="Current tier" value={`WT${state.worldTier.current}`} /><Summary label="Highest unlocked" value={`WT${state.worldTier.highestUnlocked}`} /><Summary label="Active encounter" value={activeTier ? `WT${activeTier} snapshot` : 'None'} /><Summary label="Combat state" value={state.combat.active ? 'Active' : 'Idle'} /></div>
      ...
      <div className="developer-button-grid"><Button variant="secondary" onClick={() => state.debugSetWorldTier(1)}>SET CURRENT WT1</Button><Button variant="secondary" onClick={() => state.debugSetWorldTier(2)}>SET CURRENT WT2</Button><Button variant="success" onClick={() => state.debugUnlockWorldTier(2)}>UNLOCK WT2</Button><Button variant="danger" onClick={state.debugResetWorldTier}>RESET UNLOCKS TO WT1</Button></div>
    ...
    <Card title="WORLD TIER · Authored definitions">
      <div className="developer-world-tier-definitions">{WORLD_TIER_IDS.map((tier) => { const definition = WORLD_TIERS[tier]; return <div className="developer-world-tier-definition" key={tier}><div className="developer-world-tier-definition-head"><strong>WT{tier} · {definition.name}</strong><Status tone={tier <= state.worldTier.highestUnlocked ? 'success' : 'locked'}>{tier <= state.worldTier.highestUnlocked ? 'AVAILABLE' : 'LOCKED'}</Status></div><div className="developer-detail-grid"><span>ENEMY HEALTH<strong>{formatMultiplier(definition.enemyHealthMultiplier)}</strong></span><span>ENEMY DAMAGE<strong>{formatMultiplier(definition.enemyDamageMultiplier)}</strong></span><span>ENEMY DEFENSE<strong>{formatMultiplier(definition.enemyDefenseMultiplier)}</strong></span><span>RESONANCE<strong>{formatMultiplier(definition.resonanceRewardMultiplier)}</strong></span></div>{tier === 2 && <small className="muted">Unlocks on the first defeat of Archmage Edrin Shade. Unlocking does not auto-select WT2.</small>}</div> })}</div>
    </Card>
    ...
    <Card title="WORLD TIER · Canonical enemy preview">
      ...
      <div className="developer-world-tier-preview-grid">{profiles.map(({ tier, profile }) => <GameTooltip key={tier} block content={<TooltipContent title={`WT${tier} preview`} description="Pure authored preview. This does not mutate the profile or active encounter." />}><div className={`developer-world-tier-preview${activeTier === tier ? ' is-active' : ''}`}><div className="developer-world-tier-definition-head"><strong>WT{tier}</strong>{activeTier === tier && <Status tone="active">SNAPSHOT</Status>}</div><div className="developer-detail-grid"><span>HP<strong>{profile.maxHealth.toLocaleString()} <small>from {profile.baseMaxHealth.toLocaleString()}</small></strong></span><span>BASIC DAMAGE<strong>{profile.basicAttackDamage.toFixed(1)} <small>from {profile.baseBasicAttackDamage.toFixed(1)}</small></strong></span><span>DEFENSE<strong>{profile.defense.toFixed(1)} <small>from {profile.baseDefense.toFixed(1)}</small></strong></span><span>RESONANCE<strong>{formatResonanceBundle(profile.resonanceYield)}</strong></span></div></div></GameTooltip>)}</div>
      <p className="developer-debug-note">Preview uses the same profile resolver as encounter spawn and combat stat reads. Resistances, speed, action patterns, loot, Arcane Points, and threat are unchanged.</p>
    </Card>
  </div>
}

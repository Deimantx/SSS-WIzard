...
import { setCombatRenderIsolation, useCombatRenderIsolation } from '../../../devtools/combatRenderIsolationStore'
...
export function DeveloperCombatLive() {
  ...
  return <div className="developer-tab-grid combat-live-grid">
    ...
    {import.meta.env.DEV && <Card title="Render isolation"><p className="developer-debug-note">UI-only diagnostic switches. They do not pause or alter combat simulation.</p><div className="developer-toggle-list"><Toggle label="Render Combat Analytics" checked={renderIsolation.renderCombatAnalytics} onChange={(value) => setCombatRenderIsolation({ renderCombatAnalytics: value })} /><Toggle label="Render Combat Details" checked={renderIsolation.renderCombatDetails} onChange={(value) => setCombatRenderIsolation({ renderCombatDetails: value })} /></div></Card>}
  </div>
}

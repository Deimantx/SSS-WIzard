...
function RawStatusInstances({ statuses }: { statuses: Array<{ actor: 'player' | 'enemy'; status: ActiveStatus }> }) {
  ...
  return <div className="developer-status-instance-list">{statuses.map(({ actor, status }) => {
    ...
    return <div className="developer-status-instance" key={`${actor}:${status.statusId}:${status.instanceKey}`}><strong>{definition?.name ?? status.statusId}</strong><span>Actor: {actor} · Status ID: {status.statusId}</span><span>Instance Key: {status.instanceKey}</span><span>Source: {resolveCombatSourceLabel(status.source)} · Origin: {status.source.originSourceKind ?? '-'} / {status.source.originSourceId ?? status.source.sourceId ?? '-'}</span><span>Origin tags: {status.source.originTags?.join(', ') ?? status.source.tags?.join(', ') ?? '-'} · School: {status.source.originSchool ?? status.source.school ?? '-'}</span><span>Remaining: {status.remainingMs === null ? '∞' : `${Math.max(0, Math.floor(status.remainingMs))}ms`} · Initial: {status.initialDurationMs === null ? '∞' : `${Math.max(0, Math.floor(status.initialDurationMs ?? 0))}ms`} · Next tick: {status.nextTickMs === undefined ? '-' : `${Math.max(0, Math.floor(status.nextTickMs))}ms`}</span><span>Stacks: {status.stacks} · Potency: {status.modifierOverrides ? JSON.stringify(status.modifierOverrides) : '-'} · Periodic: {periodicLabel}</span></div>
  })}</div>
}

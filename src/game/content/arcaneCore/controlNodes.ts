...
const ring1 = createRing('control', 1, [
  ...
  perk('control-r1-debilitating-presence', 'Debilitating Presence', 'Control-tagged enemies deal -0.5% Damage per rank.', rankedModifier('damage-dealt-percent', -0.005, selfControlled, 'enemy')),
...
const ring3 = createRing('control', 3, [
  ...
  perk('control-r3-deep-suppression', 'Deep Suppression', 'Enemies with 2+ negative Statuses deal -0.75% Damage per rank.', rankedModifier('damage-dealt-percent', -0.0075, selfNegativeStatuses(2), 'enemy')),
  ...
  perk('control-r3-controlled-flow', 'Controlled Flow', 'A control Status restores 1–3 Mana.', (rank) => ({ rules: [rule('control-controlled-flow', 'on-status-applied', [{ type: 'restore-resource', target: 'self', resource: 'mana', magnitude: { type: 'flat', value: rankValues(rank, [1, 1, 2, 2, 3]) } }], controlStatus, { cooldownMs: 2000 })] })),
  ...
  major('control-r3-dominion', 'Dominion', 'Enemies with 3+ negative Statuses take +10% and deal -5% Damage.', fixedEffects({ modifiers: [modifier('damage-taken-percent', 0.1, selfNegativeStatuses(3), 'enemy'), modifier('damage-dealt-percent', -0.05, selfNegativeStatuses(3), 'enemy')] })),
])
const ring4 = createRing('control', 4, [
  ...
  perk('control-r4-absolute-suppression', 'Absolute Suppression', 'Enemies with 3+ negative Statuses deal -1% Damage per rank.', rankedModifier('damage-dealt-percent', -0.01, selfNegativeStatuses(3), 'enemy')),
  minor('control-r4-perfect-rhythm', 'Perfect Rhythm', '+1% Action Speed per rank.', rankedModifier('action-speed-percent', 0.01)),
  perk('control-r4-disruption-mastery', 'Disruption Mastery', 'Control Statuses delay enemy actions by 25–125 ms.', (rank) => ({ rules: [rule('control-disruption-mastery', 'on-status-applied', [{ type: 'modify-action-timer', target: 'opponent', amountMs: rankValues(rank, [25, 50, 75, 100, 125]), action: 'current' }], controlStatus, { cooldownMs: 1000 })] })),

...
const cooldownPulse = (id: string, values: readonly number[]) => (rank: number) => ({ rules: [rule(id, 'on-spell-hit', [{ type: 'modify-cooldown', target: 'self', amountMs: -rankValues(rank, values) }], directSpellCrit, { cooldownMs: 500 })] })
...
export const powerNodes = [ring1, ring2, ring3, ring4]

...
describe('Equipment origin groups', () => {
  it('separates starter Artifacts from direct dungeon Equipment', () => {
    ...
    expect(DUNGEON_EQUIPMENT_BY_DUNGEON['fractured-approach']).toHaveLength(5)
    expect(DUNGEON_EQUIPMENT_BY_DUNGEON['flooded-reliquary']).toHaveLength(4)
    expect(DUNGEON_EQUIPMENT_BY_DUNGEON['black-gate']).toHaveLength(5)
    expect(new Set(Object.values(DUNGEON_EQUIPMENT_BY_DUNGEON).flat()).size).toBe(65)

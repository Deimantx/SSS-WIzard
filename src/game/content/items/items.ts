...
type AuthoredEquipmentDefinition = Omit<AuthoredItemDefinition, 'kind' | 'category' | 'inventoryCategory' | 'source' | 'sourceNavigation'> & Pick<ItemDefinition, 'equipmentSlot'>
/**
...
const authoredItems: Record<ItemId, AuthoredItemDefinition> = {
  ...
  'ember-staff': equipment({ id: 'ember-staff', name: 'Ember Staff', description: 'A two-handed staff that makes Fire spells burn brighter.', icon: '⚒', color: '#ff956f', equipmentSlot: 'weapon', weaponHands: 2, stats: { basicDamage: 6, maxMana: 10, spellPower: 24 }, combat: { modifiers: [{ key: 'spell-damage-percent', value: 0.2, sourceKinds: ['spell'], damageTypes: ['fire'] }] } }),
  'wispwood-wand': equipment({ id: 'wispwood-wand', name: 'Wispwood Wand', description: 'A light one-handed wand made for flexible caster loadouts.', icon: '|', color: '#c3a7ff', equipmentSlot: 'weapon', weaponHands: 1, stats: { basicDamage: 4, maxMana: 5, spellPower: 12 } }),
  'tide-focus': equipment({ id: 'tide-focus', name: 'Tide Focus', description: 'A fluid focus that deepens Water barriers.', icon: '◈', color: '#64b7ff', equipmentSlot: 'offhand', equipmentPresentation: 'focus', stats: { maxMana: 15, spellPower: 12 }, combat: { modifiers: [{ key: 'barrier-power-percent', value: 0.2, sourceKinds: ['spell'], damageTypes: ['water'] }] } }),
  'stoneweave-robe': equipment({ id: 'stoneweave-robe', name: 'Stoneweave Robe', description: 'A heavy robe that turns barriers into shelter.', icon: '◇', color: '#d5a36b', equipmentSlot: 'armor', stats: { maxHealth: 30, defense: 6 }, combat: { modifiers: [{ key: 'barrier-received-flat', value: 10 }] } }),
  'windthread-charm': equipment({ id: 'windthread-charm', name: 'Windthread Charm', description: 'A charm that leaves room for one more automation.', icon: '~', color: '#b9d8d0', equipmentSlot: 'amulet', stats: { maxMana: 10, maxFocus: 10, spellPower: 8 }, combat: { modifiers: [{ key: 'spell-damage-percent', value: 0.1, sourceKinds: ['spell'], damageTypes: ['air'] }] } }),
  'wispveil-hood': equipment({ id: 'wispveil-hood', name: 'Wispveil Hood', description: 'A soft hood threaded with a wisp’s steady rhythm.', icon: '◇', color: '#b8a8e8', equipmentSlot: 'helmet', stats: { maxHealth: 15, defense: 4, maxMana: 15, manaRegen: 1 } }),
  'grovekeeper-mantle': equipment({ id: 'grovekeeper-mantle', name: 'Grovekeeper Mantle', description: 'A mantle carrying the quiet resilience of the inner grove.', icon: '▼', color: '#9eaa75', equipmentSlot: 'cape', stats: { maxHealth: 15, defense: 3, resistances: { physical: 0.03 } } }),
  'wispbound-ring': equipment({ id: 'wispbound-ring', name: 'Wispbound Ring', description: 'A small ring that keeps Mana flowing between spells.', icon: 'O', color: '#c3a7ff', equipmentSlot: 'ring', stats: { maxMana: 10, manaRegen: 1, spellPower: 5 } }),
  'heartseed-necklace': equipment({ id: 'heartseed-necklace', name: 'Heartseed Necklace', description: 'A living seed that answers a moment of mortal danger.', icon: '✤', color: '#f4c46e', equipmentSlot: 'amulet', stats: { maxHealth: 25, defense: 5 }, combat: { modifiers: [{ key: 'healing-done-percent', value: 0.05 }], rules: [{ id: 'living-seed', event: 'on-hp-threshold', condition: { type: 'self-hp-below-percent', percent: 30 }, oncePerEncounter: true, effects: [{ type: 'gain-barrier', target: 'self', magnitude: { type: 'flat', value: 20 } }], ui: { name: 'Living Seed' } }] } }),
  ...
  'fangbound-dagger': equipment({ id: 'fangbound-dagger', name: 'Fangbound Dagger', description: 'A quick blade that rewards a steady Basic Attack rhythm.', icon: '/', color: '#d49b75', equipmentSlot: 'weapon', weaponHands: 1, stats: { basicDamage: 10, spellPower: 8, basicAttackSpeedPct: 0.08, critChance: 0.05 } }),
  'fangbound-buckler': equipment({ id: 'fangbound-buckler', name: 'Fangbound Buckler', description: 'A predator’s buckler that turns momentum aside.', icon: 'O', color: '#bd8c6e', equipmentSlot: 'offhand', equipmentPresentation: 'shield', stats: { maxHealth: 35, defense: 8, blockChance: 0.15, resistances: { physical: 0.03 } } }),
  'corrupted-howlstaff': equipment({ id: 'corrupted-howlstaff', name: 'Corrupted Howlstaff', description: 'A warped staff that accelerates spells and stretches their statuses.', icon: 'Y', color: '#7e6c9f', equipmentSlot: 'weapon', weaponHands: 2, stats: { basicDamage: 8, spellPower: 36, cooldownRecoveryPct: 0.1, statusDurationPct: 0.1 } }),
  'razorclaw-circlet': equipment({ id: 'razorclaw-circlet', name: 'Razorclaw Circlet', description: 'A circlet honed for critical strikes and swift attacks.', icon: '^', color: '#c18b73', equipmentSlot: 'helmet', stats: { maxHealth: 20, defense: 5, critChance: 0.02, critDamage: 0.15, basicAttackSpeedPct: 0.05 } }),
  'predator-hide-mantle': equipment({ id: 'predator-hide-mantle', name: 'Predator-Hide Mantle', description: 'A mantle that blunts physical blows and shakes off hostile magic.', icon: '▼', color: '#8f7469', equipmentSlot: 'cape', stats: { maxHealth: 20, defense: 5, resistances: { physical: 0.05 } }, combat: { modifiers: [{ key: 'status-duration-received-percent', value: -0.1, statusTags: ['debuff'] }] } }),
  'greatbear-vestment': equipment({ id: 'greatbear-vestment', name: 'Greatbear Vestment', description: 'A massive vestment built for endurance.', icon: '◇', color: '#806b69', equipmentSlot: 'armor', stats: { maxHealth: 45, defense: 12, resistances: { physical: 0.1 } } }),
  'howling-signet': equipment({ id: 'howling-signet', name: 'Howling Signet', description: 'A signet that feeds the hunter after every kill.', icon: 'O', color: '#c18b73', equipmentSlot: 'ring', stats: { maxHealth: 15, maxMana: 20 }, combat: { rules: [{ id: 'predators-feast', event: 'on-kill', effects: [{ type: 'heal', target: 'self', magnitude: { type: 'flat', value: 25 } }], ui: { name: "Predator's Feast" } }] } }),
  'greatbear-heartstone': equipment({ id: 'greatbear-heartstone', name: 'Greatbear Heartstone', description: 'A corrupted heartstone that refuses to yield.', icon: 'O', color: '#806b69', equipmentSlot: 'amulet', stats: { maxHealth: 30, healthRegen: 1, defense: 10, resistances: { fire: 0.05, water: 0.05, earth: 0.05, air: 0.05 } }, combat: { rules: [{ id: 'unyielding', event: 'on-hp-threshold', condition: { type: 'self-hp-below-percent', percent: 35 }, oncePerEncounter: true, effects: [{ type: 'gain-barrier', target: 'self', magnitude: { type: 'flat', value: 40 } }], ui: { name: 'Unyielding' } }] } }),
  ...
  'graveglass-wand': equipment({ id: 'graveglass-wand', name: 'Graveglass Wand', description: 'A one-handed wand that makes every spell more economical.', icon: '|', color: '#8d9dc9', equipmentSlot: 'weapon', weaponHands: 1, stats: { basicDamage: 8, spellPower: 32, cooldownRecoveryPct: 0.1, manaCostReductionPct: 0.1 } }),
  'edrins-remnant-staff': equipment({ id: 'edrins-remnant-staff', name: "Edrin's Remnant Staff", description: 'A staff carrying forbidden arcane memory into every spell.', icon: 'Y', color: '#70619b', equipmentSlot: 'weapon', weaponHands: 2, stats: { basicDamage: 14, spellPower: 50, cooldownRecoveryPct: 0.1, statusDurationPct: 0.15 }, combat: { modifiers: [{ key: 'damage-dealt-percent', value: 0.1, sourceKinds: ['spell'], condition: { type: 'target-has-status-tag', tag: 'debuff' } }] } }),
  'soulward-focus': equipment({ id: 'soulward-focus', name: 'Soulward Focus', description: 'A focus that releases stored Mana when your Barrier breaks.', icon: '◈', color: '#9b7eaa', equipmentSlot: 'offhand', equipmentPresentation: 'focus', stats: { maxMana: 25, manaRegen: 5, spellPower: 20, barrierPowerPct: 0.15 }, combat: { rules: [{ id: 'soul-release-mana', event: 'on-barrier-broken', effects: [{ type: 'restore-resource', target: 'self', resource: 'mana', magnitude: { type: 'flat', value: 15 } }], ui: { name: 'Soul Release' } }] } }),
  'soulward-shield': equipment({ id: 'soulward-shield', name: 'Soulward Shield', description: 'A shield that answers a broken Barrier with a physical backlash.', icon: 'O', color: '#9b7eaa', equipmentSlot: 'offhand', equipmentPresentation: 'shield', stats: { maxHealth: 35, defense: 10, manaRegen: 1, blockChance: 0.15, barrierPowerPct: 0.1, resistances: { fire: 0.1, water: 0.1, earth: 0.1, air: 0.1 } }, combat: { rules: [{ id: 'soul-release-damage', event: 'on-barrier-broken', effects: [{ type: 'deal-damage', target: 'opponent', components: [{ damageType: 'physical', magnitude: { type: 'flat', value: 20 } }], tags: ['equipment', 'direct', 'physical'] }], ui: { name: 'Soul Release' } }] } }),
  'acolyte-vestments': equipment({ id: 'acolyte-vestments', name: 'Acolyte Vestments', description: 'Protective vestments strengthened by elemental wards.', icon: '◇', color: '#9b7eaa', equipmentSlot: 'armor', stats: { maxHealth: 50, defense: 12, maxMana: 20, healingDonePct: 0.1, resistances: { fire: 0.1, water: 0.1, earth: 0.1, air: 0.1 } } }),
  'wraithveil-hood': equipment({ id: 'wraithveil-hood', name: 'Wraithveil Hood', description: 'A hood that extends your statuses while shortening hostile ones.', icon: '◇', color: '#8d9dc9', equipmentSlot: 'helmet', stats: { maxHealth: 25, defense: 7, spellPower: 25, statusDurationPct: 0.15 }, combat: { modifiers: [{ key: 'status-duration-received-percent', value: -0.1, statusTags: ['debuff'] }] } }),
  'ossuary-mantle': equipment({ id: 'ossuary-mantle', name: 'Ossuary Mantle', description: 'A broad mantle layered with elemental-resistant bone.', icon: '▼', color: '#c9c3ae', equipmentSlot: 'cape', stats: { maxHealth: 30, defense: 8, maxMana: 20, resistances: { fire: 0.1, water: 0.1, earth: 0.1, air: 0.1 } } }),
  'soulglass-amulet': equipment({ id: 'soulglass-amulet', name: 'Soulglass Amulet', description: 'An amulet that sharpens damage over time and status magic.', icon: '✤', color: '#8d9dc9', equipmentSlot: 'amulet', stats: { maxHealth: 20, spellPower: 15, statusDurationPct: 0.15, damageOverTimePct: 0.15 } }),
  'gravebinder-ring': equipment({ id: 'gravebinder-ring', name: 'Gravebinder Ring', description: 'A ring that exploits every negative status on the enemy.', icon: 'O', color: '#70619b', equipmentSlot: 'ring', stats: { maxMana: 15, spellPower: 15 }, combat: { modifiers: [{ key: 'damage-dealt-percent', value: 0.1, condition: { type: 'target-has-status-tag', tag: 'debuff' } }] } }),
  'edrins-signet': equipment({ id: 'edrins-signet', name: "Edrin's Signet", description: 'A remnant signet that turns hostile status magic into a ward.', icon: 'O', color: '#70619b', equipmentSlot: 'ring', stats: { maxHealth: 20, maxMana: 20, manaRegen: 3, spellPower: 20, manaCostReductionPct: 0.1 }, combat: { rules: [{ id: 'arcane-remnant', event: 'on-status-applied', condition: { type: 'all', conditions: [{ type: 'source-is-opponent' }, { type: 'event-target-is-self' }, { type: 'event-status-has-tag', tag: 'debuff' }] }, cooldownMs: 30_000, effects: [{ type: 'gain-barrier', target: 'self', magnitude: { type: 'flat', value: 20 } }], ui: { name: 'Arcane Remnant' } }] } }),
}

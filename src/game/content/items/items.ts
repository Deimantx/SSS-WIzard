...
const directEquipmentSources: Partial<Record<ItemId, string>> = {
  ...
  'galeglass-earring': 'Combat → Fractured Approach', 'riftwind-ring': 'Combat → Fractured Approach', 'waystone-pendant': 'Combat → Fractured Approach', 'fractured-ward-mantle': 'Combat → Fractured Approach', 'gatekeeper-sigil': 'Combat → Corrupted Elemental Gatekeeper',
}
/**
...
const ACT1_EQUIPMENT_TIER_BY_SOURCE: Record<string, number> = {
  'Flooded Reliquary': 1.8, 'Drowned Keeper': 1.8,
  'Ashen Watch': 1.8, 'Flamebound Revenant': 1.8,
  'Rootscar Hollow': 1.8, 'Rootscar Ancient': 1.8,
  'Crossroads of Ruin': 2, 'Crossroads Keeper': 2,
  'Graveglass Hollow': 2.2, 'Graveglass Behemoth': 2.2,
  'Stormvault Gallery': 2.2, 'Storm Archivist': 2.2,
  'Starfallen Observatory': 2.2, 'Fallen Astromancer': 2.2,
  'Broken Meridian': 2.5, 'Meridian Splitter': 2.5,
  'Hall of Unbound Names': 2.7, 'Unspoken Prelate': 2.7,
  'Vault of the Black Sigil': 2.7, 'Sigil Warden': 2.7,
  'Black Gate': 3, 'Black Gatekeeper': 3,
}

const act1Equipment = (id: ItemId, name: string, slot: AuthoredEquipmentDefinition['equipmentSlot'], dungeon: string, boss = false): AuthoredItemDefinition => {
  const stats = slot === 'cape' ? { maxHealth: boss ? 90 : 48, defense: boss ? 24 : 12 } : slot === 'earring' ? { maxMana: boss ? 55 : 30, spellPower: boss ? 28 : 16 } : slot === 'ring' ? { maxMana: boss ? 55 : 24, manaRegen: boss ? 5 : 2 } : { maxHealth: boss ? 95 : 36, maxMana: boss ? 50 : 18 }
  return equipment({ id, name, description: `${name} carries the signature of ${dungeon}.`, icon: slot === 'cape' ? '▼' : slot === 'earring' ? '◍' : slot === 'ring' ? 'O' : '◇', color: boss ? '#d39bff' : '#9eb9ed', equipmentTier: ACT1_EQUIPMENT_TIER_BY_SOURCE[dungeon] ?? (boss ? 2.2 : 2), buildTags: slot === 'cape' ? ['defense', 'sustain'] : ['spell', 'mana'], equipmentBudgetProfile: boss ? 'boss' : 'standard', equipmentSlot: slot, stats, ...(boss ? { combat: { modifiers: [{ key: 'damage-taken-percent' as const, value: -0.04 }] } } : {}) }, `Combat → ${dungeon}`)
}

/* Legacy draft names retained only in this comment for migration review.
const LEGACY_ACT1_EQUIPMENT = Object.fromEntries([
  ['reliquary-mantle', act1Equipment('reliquary-mantle', 'Reliquary Mantle', 'cape', 'Flooded Reliquary')], ['drowned-choir-earring', act1Equipment('drowned-choir-earring', 'Drowned Choir Earring', 'earring', 'Flooded Reliquary')], ['keeper-of-tides-ring', act1Equipment('keeper-of-tides-ring', 'Keeper of Tides Ring', 'ring', 'Flooded Reliquary')], ['floodglass-pendant', act1Equipment('floodglass-pendant', 'Floodglass Pendant', 'amulet', 'Flooded Reliquary')], ['drowned-keepers-seal', act1Equipment('drowned-keepers-seal', 'Drowned Keeper’s Seal', 'amulet', 'Drowned Keeper', true)],
  ['cinderplate-cape', act1Equipment('cinderplate-cape', 'Cinderplate Cape', 'cape', 'Ashen Watch')], ['ashbound-earring', act1Equipment('ashbound-earring', 'Ashbound Earring', 'earring', 'Ashen Watch')], ['emberwatch-ring', act1Equipment('emberwatch-ring', 'Emberwatch Ring', 'ring', 'Ashen Watch')], ['pyreglass-amulet', act1Equipment('pyreglass-amulet', 'Pyreglass Amulet', 'amulet', 'Ashen Watch')], ['flamebound-relic', act1Equipment('flamebound-relic', 'Flamebound Relic', 'amulet', 'Flamebound Revenant', true)],
  ['rootscar-mantle', act1Equipment('rootscar-mantle', 'Rootscar Mantle', 'cape', 'Rootscar Hollow')], ['thornbloom-earring', act1Equipment('thornbloom-earring', 'Thornbloom Earring', 'earring', 'Rootscar Hollow')], ['rootbound-ring', act1Equipment('rootbound-ring', 'Rootbound Ring', 'ring', 'Rootscar Hollow')], ['briarheart-pendant', act1Equipment('briarheart-pendant', 'Briarheart Pendant', 'amulet', 'Rootscar Hollow')], ['ancient-root-seal', act1Equipment('ancient-root-seal', 'Ancient Root Seal', 'amulet', 'Rootscar Ancient', true)],
  ['crossroads-mantle', act1Equipment('crossroads-mantle', 'Crossroads Mantle', 'cape', 'Crossroads of Ruin')], ['riftmark-earring', act1Equipment('riftmark-earring', 'Riftmark Earring', 'earring', 'Crossroads of Ruin')], ['wayfinder-ring', act1Equipment('wayfinder-ring', 'Wayfinder Ring', 'ring', 'Crossroads of Ruin')], ['confluence-pendant', act1Equipment('confluence-pendant', 'Confluence Pendant', 'amulet', 'Crossroads of Ruin')], ['crossroads-keepers-seal', act1Equipment('crossroads-keepers-seal', 'Crossroads Keeper’s Seal', 'amulet', 'Crossroads Keeper', true)],
  ['graveglass-mantle', act1Equipment('graveglass-mantle', 'Graveglass Mantle', 'cape', 'Graveglass Hollow')], ['mourning-shard-earring', act1Equipment('mourning-shard-earring', 'Mourning Shard Earring', 'earring', 'Graveglass Hollow')], ['gravebind-ring', act1Equipment('gravebind-ring', 'Gravebind Ring', 'ring', 'Graveglass Hollow')], ['deathglass-pendant', act1Equipment('deathglass-pendant', 'Deathglass Pendant', 'amulet', 'Graveglass Hollow')], ['graveglass-behemoth-seal', act1Equipment('graveglass-behemoth-seal', 'Graveglass Behemoth Seal', 'amulet', 'Graveglass Behemoth', true)],
  ['stormvault-mantle', act1Equipment('stormvault-mantle', 'Stormvault Mantle', 'cape', 'Stormvault Gallery')], ['voltglass-earring', act1Equipment('voltglass-earring', 'Voltglass Earring', 'earring', 'Stormvault Gallery')], ['static-ring', act1Equipment('static-ring', 'Static Ring', 'ring', 'Stormvault Gallery')], ['stormscript-pendant', act1Equipment('stormscript-pendant', 'Stormscript Pendant', 'amulet', 'Stormvault Gallery')], ['storm-archivists-seal', act1Equipment('storm-archivists-seal', 'Storm Archivist’s Seal', 'amulet', 'Storm Archivist', true)],
  ['starfallen-mantle', act1Equipment('starfallen-mantle', 'Starfallen Mantle', 'cape', 'Starfallen Observatory')], ['astral-eye-earring', act1Equipment('astral-eye-earring', 'Astral Eye Earring', 'earring', 'Starfallen Observatory')], ['orbit-ring', act1Equipment('orbit-ring', 'Orbit Ring', 'ring', 'Starfallen Observatory')], ['starshard-pendant', act1Equipment('starshard-pendant', 'Starshard Pendant', 'amulet', 'Starfallen Observatory')], ['fallen-astromancers-seal', act1Equipment('fallen-astromancers-seal', 'Fallen Astromancer’s Seal', 'amulet', 'Fallen Astromancer', true)],
  ['meridian-mantle', act1Equipment('meridian-mantle', 'Meridian Mantle', 'cape', 'Broken Meridian')], ['fractureline-earring', act1Equipment('fractureline-earring', 'Fractureline Earring', 'earring', 'Broken Meridian')], ['splitter-ring', act1Equipment('splitter-ring', 'Splitter Ring', 'ring', 'Broken Meridian')], ['meridian-pendant', act1Equipment('meridian-pendant', 'Meridian Pendant', 'amulet', 'Broken Meridian')], ['meridian-splitters-seal', act1Equipment('meridian-splitters-seal', 'Meridian Splitter’s Seal', 'amulet', 'Meridian Splitter', true)],
  ['unbound-mantle', act1Equipment('unbound-mantle', 'Unbound Mantle', 'cape', 'Hall of Unbound Names')], ['nameless-earring', act1Equipment('nameless-earring', 'Nameless Earring', 'earring', 'Hall of Unbound Names')], ['unspoken-ring', act1Equipment('unspoken-ring', 'Unspoken Ring', 'ring', 'Hall of Unbound Names')], ['litany-pendant', act1Equipment('litany-pendant', 'Litany Pendant', 'amulet', 'Hall of Unbound Names')], ['unspoken-prelates-seal', act1Equipment('unspoken-prelates-seal', 'Unspoken Prelate’s Seal', 'amulet', 'Unspoken Prelate', true)],
  ['black-sigil-mantle', act1Equipment('black-sigil-mantle', 'Black Sigil Mantle', 'cape', 'Vault of the Black Sigil')], ['inkbound-earring', act1Equipment('inkbound-earring', 'Inkbound Earring', 'earring', 'Vault of the Black Sigil')], ['forbidden-ring', act1Equipment('forbidden-ring', 'Forbidden Ring', 'ring', 'Vault of the Black Sigil')], ['black-sigil-pendant', act1Equipment('black-sigil-pendant', 'Black Sigil Pendant', 'amulet', 'Vault of the Black Sigil')], ['sigil-wardens-seal', act1Equipment('sigil-wardens-seal', 'Sigil Warden’s Seal', 'amulet', 'Sigil Warden', true)],
  ['black-gate-mantle', act1Equipment('black-gate-mantle', 'Black Gate Mantle', 'cape', 'Black Gate')], ['abyssal-earring', act1Equipment('abyssal-earring', 'Abyssal Earring', 'earring', 'Black Gate')], ['riftbound-ring', act1Equipment('riftbound-ring', 'Riftbound Ring', 'ring', 'Black Gate')], ['gatefall-pendant', act1Equipment('gatefall-pendant', 'Gatefall Pendant', 'amulet', 'Black Gate')], ['black-gatekeepers-seal', act1Equipment('black-gatekeepers-seal', 'Black Gatekeeper’s Seal', 'amulet', 'Black Gatekeeper', true)],
] as const) as Record<string, AuthoredItemDefinition> */

const ACT1_EQUIPMENT = Object.fromEntries([
  ['mistglass-earring', act1Equipment('mistglass-earring', 'Mistglass Earring', 'earring', 'Flooded Reliquary')], ['reliquary-ring', act1Equipment('reliquary-ring', 'Reliquary Ring', 'ring', 'Flooded Reliquary')], ['drowned-chain-pendant', act1Equipment('drowned-chain-pendant', 'Drowned Chain Pendant', 'amulet', 'Flooded Reliquary')], ['keepers-tide-seal', act1Equipment('keepers-tide-seal', "Keeper's Tide Seal", 'amulet', 'Drowned Keeper', true)],
  ['cinderwire-earring', act1Equipment('cinderwire-earring', 'Cinderwire Earring', 'earring', 'Ashen Watch')], ['ashbrand-ring', act1Equipment('ashbrand-ring', 'Ashbrand Ring', 'ring', 'Ashen Watch')], ['emberwatch-mantle', act1Equipment('emberwatch-mantle', 'Emberwatch Mantle', 'cape', 'Ashen Watch')], ['revenant-emberstone', act1Equipment('revenant-emberstone', 'Revenant Emberstone', 'amulet', 'Flamebound Revenant', true)],
  ['briar-earring', act1Equipment('briar-earring', 'Briar Earring', 'earring', 'Rootscar Hollow')], ['rootbound-ring', act1Equipment('rootbound-ring', 'Rootbound Ring', 'ring', 'Rootscar Hollow')], ['mossguard-mantle', act1Equipment('mossguard-mantle', 'Mossguard Mantle', 'cape', 'Rootscar Hollow')], ['ancient-heart-knot', act1Equipment('ancient-heart-knot', 'Ancient Heart Knot', 'amulet', 'Rootscar Ancient', true)],
  ['wayfarer-earring', act1Equipment('wayfarer-earring', 'Wayfarer Earring', 'earring', 'Crossroads of Ruin')], ['crossroads-signet', act1Equipment('crossroads-signet', 'Crossroads Signet', 'ring', 'Crossroads of Ruin')], ['confluence-pendant', act1Equipment('confluence-pendant', 'Confluence Pendant', 'amulet', 'Crossroads of Ruin')], ['keepers-roadseal', act1Equipment('keepers-roadseal', "Keeper's Roadseal", 'amulet', 'Crossroads Keeper', true)],
  ['graveglass-earring', act1Equipment('graveglass-earring', 'Graveglass Earring', 'earring', 'Graveglass Hollow')], ['shardbone-ring', act1Equipment('shardbone-ring', 'Shardbone Ring', 'ring', 'Graveglass Hollow')], ['mourner-veil-mantle', act1Equipment('mourner-veil-mantle', 'Mourner Veil Mantle', 'cape', 'Graveglass Hollow')], ['behemoth-heartshard', act1Equipment('behemoth-heartshard', 'Behemoth Heartshard', 'amulet', 'Graveglass Behemoth', true)],
  ['voltglass-earring', act1Equipment('voltglass-earring', 'Voltglass Earring', 'earring', 'Stormvault Gallery')], ['stormcoil-ring', act1Equipment('stormcoil-ring', 'Stormcoil Ring', 'ring', 'Stormvault Gallery')], ['gale-scribe-pendant', act1Equipment('gale-scribe-pendant', 'Gale Scribe Pendant', 'amulet', 'Stormvault Gallery')], ['archivists-conductor', act1Equipment('archivists-conductor', "Archivist's Conductor", 'amulet', 'Storm Archivist', true)],
  ['starfall-ring', act1Equipment('starfall-ring', 'Starfall Ring', 'ring', 'Starfallen Observatory')], ['lenskeeper-earring', act1Equipment('lenskeeper-earring', 'Lenskeeper Earring', 'earring', 'Starfallen Observatory')], ['astral-pendant', act1Equipment('astral-pendant', 'Astral Pendant', 'amulet', 'Starfallen Observatory')], ['fallen-astromancer-lens', act1Equipment('fallen-astromancer-lens', 'Fallen Astromancer Lens', 'amulet', 'Fallen Astromancer', true)],
  ['meridian-ring', act1Equipment('meridian-ring', 'Meridian Ring', 'ring', 'Broken Meridian')], ['linebreaker-earring', act1Equipment('linebreaker-earring', 'Linebreaker Earring', 'earring', 'Broken Meridian')], ['fractured-conduit-pendant', act1Equipment('fractured-conduit-pendant', 'Fractured Conduit Pendant', 'amulet', 'Broken Meridian')], ['leyline-mantle', act1Equipment('leyline-mantle', 'Leyline Mantle', 'cape', 'Broken Meridian')], ['splitters-meridian-core', act1Equipment('splitters-meridian-core', "Splitter's Meridian Core", 'amulet', 'Meridian Splitter', true)],
  ['nameless-ring', act1Equipment('nameless-ring', 'Nameless Ring', 'ring', 'Hall of Unbound Names')], ['whisper-earring', act1Equipment('whisper-earring', 'Whisper Earring', 'earring', 'Hall of Unbound Names')], ['unbound-seal-pendant', act1Equipment('unbound-seal-pendant', 'Unbound Seal Pendant', 'amulet', 'Hall of Unbound Names')], ['prelates-unspoken-seal', act1Equipment('prelates-unspoken-seal', "Prelate's Unspoken Seal", 'amulet', 'Unspoken Prelate', true)],
  ['black-sigil-ring', act1Equipment('black-sigil-ring', 'Black Sigil Ring', 'ring', 'Vault of the Black Sigil')], ['inkbound-earring', act1Equipment('inkbound-earring', 'Inkbound Earring', 'earring', 'Vault of the Black Sigil')], ['vaultseal-mantle', act1Equipment('vaultseal-mantle', 'Vaultseal Mantle', 'cape', 'Vault of the Black Sigil')], ['wardens-black-sigil', act1Equipment('wardens-black-sigil', "Warden's Black Sigil", 'amulet', 'Sigil Warden', true)],
  ['gatebound-ring', act1Equipment('gatebound-ring', 'Gatebound Ring', 'ring', 'Black Gate')], ['portal-echo-earring', act1Equipment('portal-echo-earring', 'Portal Echo Earring', 'earring', 'Black Gate')], ['blackgate-pendant', act1Equipment('blackgate-pendant', 'Blackgate Pendant', 'amulet', 'Black Gate')], ['voidward-mantle', act1Equipment('voidward-mantle', 'Voidward Mantle', 'cape', 'Black Gate')], ['black-gatekeepers-seal', act1Equipment('black-gatekeepers-seal', "Black Gatekeeper's Seal", 'amulet', 'Black Gatekeeper', true)],
] as const) as Record<string, AuthoredItemDefinition>

/** One authoritative item registry for materials, loot, and all authored equipment. */
const authoredItems: Record<string, AuthoredItemDefinition> = {
  ...ACT1_EQUIPMENT,
  ...
  'galeshard-staff': equipment({ id: 'galeshard-staff', name: 'Galeshard Staff', description: 'A frontier staff cut from wind-scoured crystal, carrying the sharp rhythm of unstable Air.', icon: '✦', color: '#a9d6e5', equipmentTier: 1.7, buildTags: ['spell', 'air', 'crit', 'mana'], equipmentBudgetProfile: 'signature', equipmentSlot: 'weapon' }),
  'reliquary-scepter': equipment({ id: 'reliquary-scepter', name: 'Reliquary Scepter', description: 'A drowned scepter that turns cold currents into deliberate spellcraft.', icon: '◇', color: '#83c9e8', equipmentTier: 1.8, buildTags: ['spell', 'water', 'barrier'], equipmentBudgetProfile: 'signature', equipmentSlot: 'weapon' }),
  'pyrebound-staff': equipment({ id: 'pyrebound-staff', name: 'Pyrebound Staff', description: 'A staff that keeps a watchfire burning through every incantation.', icon: '⚒', color: '#ff956f', equipmentTier: 1.8, buildTags: ['spell', 'fire', 'dot'], equipmentBudgetProfile: 'signature', equipmentSlot: 'weapon' }),
  'rootheart-scepter': equipment({ id: 'rootheart-scepter', name: 'Rootheart Scepter', description: 'A living scepter that answers each spell with a patient pulse.', icon: '⬟', color: '#9eaa75', equipmentTier: 1.8, buildTags: ['spell', 'earth', 'defense'], equipmentBudgetProfile: 'signature', equipmentSlot: 'weapon' }),
  ...
  'galeglass-earring': equipment({ id: 'galeglass-earring', name: 'Galeglass Earring', description: 'A clear shard of frontier glass that catches stray wind and turns it into a sharper rhythm for every spell.', icon: '≈', color: '#b9d8d0', equipmentTier: 1.6, buildTags: ['spell', 'air', 'mana'], equipmentBudgetProfile: 'standard', equipmentSlot: 'earring', stats: { maxMana: 18, spellPower: 10, cooldownRecoveryPct: 0.04 } }),
  'riftwind-ring': equipment({ id: 'riftwind-ring', name: 'Riftwind Ring', description: 'A fractured ring that pulls unstable air into a narrow current, rewarding precise and rapid casting.', icon: 'O', color: '#8cc9cf', equipmentTier: 1.6, buildTags: ['spell', 'air', 'crit', 'basic-attack'], equipmentBudgetProfile: 'standard', equipmentSlot: 'ring', stats: { maxMana: 12, spellPower: 9, critChance: 0.03, critDamage: 0.10, cooldownRecoveryPct: 0.03 } }),
  'waystone-pendant': equipment({ id: 'waystone-pendant', name: 'Waystone Pendant', description: 'A waystone fragment steadies the wearer between broken roads, expanding both Mana reserves and Air spell pressure.', icon: '◇', color: '#9eb9ed', equipmentTier: 1.6, buildTags: ['spell', 'air', 'mana', 'focus'], equipmentBudgetProfile: 'signature', equipmentSlot: 'amulet', stats: { maxMana: 20, maxFocus: 10, spellPower: 8 }, combat: { modifiers: [{ key: 'spell-damage-percent', value: 0.08, originSourceKinds: ['spell'], damageTypes: ['air'] }] } }),
  'fractured-ward-mantle': equipment({ id: 'fractured-ward-mantle', name: 'Fractured Ward Mantle', description: 'Broken ward-pylons are stitched into a mantle that turns the frontier\'s unstable magic into a thin protective shell.', icon: '▼', color: '#9a9fba', equipmentTier: 1.6, buildTags: ['defense', 'air'], equipmentBudgetProfile: 'standard', equipmentSlot: 'cape', stats: { maxHealth: 25, defense: 6, resistances: { air: 0.04, arcane: 0.04 } } }),
  'gatekeeper-sigil': equipment({ id: 'gatekeeper-sigil', name: 'Gatekeeper Sigil', description: 'The Gatekeeper\'s fractured seal still resists the collapse of the frontier, lending its bearer a measured ward against incoming force.', icon: '✤', color: '#d276a3', equipmentTier: 1.7, buildTags: ['defense', 'air', 'sustain'], equipmentBudgetProfile: 'boss', equipmentSlot: 'amulet', stats: { maxHealth: 35, maxMana: 25, defense: 8, spellPower: 10, resistances: { fire: 0.03, water: 0.03, earth: 0.03, air: 0.03 } }, combat: { modifiers: [{ key: 'damage-taken-percent', value: -0.03 }] } }),
}
...
const sellValues: Partial<Record<ItemId, number | null>> = {
  ...
  'galeglass-earring': 140, 'riftwind-ring': 140, 'waystone-pendant': 140, 'fractured-ward-mantle': 140, 'gatekeeper-sigil': null,
  'galeshard-staff': null,
}
...
export const ITEMS: Record<ItemId, ItemDefinition> = Object.fromEntries(Object.entries(authoredItems).map(([id, item]) => {
  ...
  return [id, { ...item, inventoryCategory, ...(inventoryCategory === 'material' ? { materialSubtype: item.materialSubtype ?? (item.category === 'elemental' ? 'elemental' : 'creature') } : {}), sourceNavigation: item.sourceNavigation ?? sourceNavigationByItem[itemId], sellValue: isArtifact ? null : item.sellValue !== undefined ? item.sellValue : sellValues[itemId] ?? (item.kind === 'equipment' ? 180 : null), canDestroy: isArtifact ? false : item.canDestroy ?? destroyability[itemId] ?? true, ...(isArtifact || item.actionRestrictionReason || actionRestrictionReasons[itemId] ? { actionRestrictionReason: isArtifact ? 'Artifact Equipment cannot be sold or destroyed.' : item.actionRestrictionReason ?? actionRestrictionReasons[itemId] } : {}) }]

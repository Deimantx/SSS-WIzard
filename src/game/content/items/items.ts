...
const authoredItems: Record<ItemId, AuthoredItemDefinition> = {
  ...
  'greatbear-heartstone': equipment({ id: 'greatbear-heartstone', name: 'Greatbear Heartstone', description: 'A corrupted heartstone that refuses to yield.', icon: 'O', color: '#806b69', source: 'Corrupted Greatbear direct drop', equipmentSlot: 'amulet', stats: { maxHealth: 25, resistances: { fire: 0.05, water: 0.05, earth: 0.05, air: 0.05 } }, combat: { rules: [{ id: 'unyielding', event: 'on-hp-threshold', condition: { type: 'self-hp-below-percent', percent: 35 }, oncePerEncounter: true, effects: [{ type: 'gain-barrier', target: 'self', magnitude: { type: 'flat', value: 40 } }], ui: { name: 'Unyielding' } }] } }),
  ...
  'edrins-signet': equipment({ id: 'edrins-signet', name: "Edrin's Signet", description: 'A remnant signet that turns hostile status magic into a ward.', icon: 'O', color: '#70619b', source: 'Archmage Edrin’s Shade direct drop', equipmentSlot: 'ring', stats: { maxMana: 25, manaCostReductionPct: 0.1 }, combat: { rules: [{ id: 'arcane-remnant', event: 'on-status-applied', condition: { type: 'all', conditions: [{ type: 'source-is-opponent' }, { type: 'event-target-is-self' }, { type: 'event-status-has-tag', tag: 'debuff' }] }, cooldownMs: 30_000, effects: [{ type: 'gain-barrier', target: 'self', magnitude: { type: 'flat', value: 20 } }], ui: { name: 'Arcane Remnant' } }] } }),
}

...
import { RECIPES, RECIPE_ORDER, type RecipeDefinition, validateRecipeDefinitions } from './recipes/recipes'
import type { ItemId, RecipeId } from '../types'

describe('material-only loot and Transmutation-only Equipment', () => {
  ...
  it('defines one material-only Transmutation recipe for every Equipment item', () => {
    const equipment = Object.values(ITEMS).filter((item) => item.kind === 'equipment')
    const equipmentOutputs = Object.values(RECIPES).filter((recipe) => recipe.category === 'equipment' && ITEMS[recipe.output.itemId]?.kind === 'equipment')
  ...
  it('rejects missing, duplicate, and wrongly categorized Equipment recipes', () => {
    const withoutHeartseed = { ...RECIPES } as Record<string, RecipeDefinition>
    delete withoutHeartseed['heartseed-necklace']
    expect(validateRecipeDefinitions(withoutHeartseed, RECIPE_ORDER.filter((id) => id !== 'heartseed-necklace'))).toContain('heartseed-necklace: Equipment must have exactly one Transmutation recipe (found 0)')

    const duplicate = {
      ...RECIPES,
      'duplicate-heartseed': { ...RECIPES['heartseed-necklace'], id: 'duplicate-heartseed' as RecipeId },
    } as Record<string, RecipeDefinition>
    expect(validateRecipeDefinitions(duplicate, [...RECIPE_ORDER, 'duplicate-heartseed'])).toContain('heartseed-necklace: Equipment must have exactly one Transmutation recipe (found 2)')

    const wrongCategory = { ...RECIPES, 'heartseed-necklace': { ...RECIPES['heartseed-necklace'], category: 'material' } } as Record<string, RecipeDefinition>
    expect(validateRecipeDefinitions(wrongCategory, RECIPE_ORDER)).toContain('heartseed-necklace: Equipment output must use category equipment')
  })

  it('uses the Transmutation source and no direct drop relation for signature Equipment', () => {
    const signatureIds = ['heartseed-necklace', 'greatbear-heartstone', 'edrins-signet'] as const
    signatureIds.forEach((itemId) => {
      expect(ITEMS[itemId].source).toBe('Transmutation')
      expect(ITEMS[itemId].sourceNavigation).toBe('tower-transmutation')

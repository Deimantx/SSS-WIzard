...
const damage = (school: 'fire' | 'water' | 'earth' | 'air', value: number, levelScaling = false): CombatEffect => ({
  type: 'deal-damage', target: 'opponent', damageType: school, school, magnitude: levelScaling ? { type: 'school-level', base: value, perLevel: 2, school } : { type: 'flat', value }, tags: ['spell', 'magic', 'direct', school],
})
const barrier = (value: number): CombatEffect => ({ type: 'gain-barrier', target: 'self', magnitude: { type: 'flat', value }, tags: ['spell', 'magic', 'barrier', 'water'] })
const heal = (value: number): CombatEffect => ({ type: 'heal', target: 'self', magnitude: { type: 'flat', value }, tags: ['spell', 'magic', 'hot', 'water'] })
...
export const SPELLS: Record<SpellId, SpellDefinition> = {
  ...
  ignite: { id: 'ignite', name: 'Ignite', school: 'fire', description: 'A spark that burns after it lands.', unlockLevel: 4, manaCost: 18, cooldownMs: 9000, autoCastFocus: 20, type: 'dot', effects: [damage('fire', 10), { type: 'apply-status', target: 'opponent', statusId: 'burning', tags: ['spell', 'magic', 'debuff', 'fire'] }], autoCondition: { type: 'always' } },
  ...
  stoneguard: { id: 'stoneguard', name: 'Stoneguard', school: 'earth', description: 'A durable shell of living stone.', unlockLevel: 4, manaCost: 20, cooldownMs: 12000, autoCastFocus: 25, type: 'barrier', effects: [{ type: 'gain-barrier', target: 'self', magnitude: { type: 'flat', value: 45 }, tags: ['spell', 'magic', 'barrier', 'earth'] }], autoCondition: { type: 'barrier-below', value: 10 } },
  quickening: { id: 'quickening', name: 'Quickening', school: 'air', description: 'A gust that accelerates every Basic Attack.', unlockLevel: 4, manaCost: 16, cooldownMs: 12000, autoCastFocus: 20, type: 'buff', effects: [{ type: 'apply-status', target: 'self', statusId: 'quickening', tags: ['spell', 'magic', 'buff', 'air'] }], autoCondition: { type: 'always' } },
}

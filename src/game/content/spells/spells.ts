...
export const SPELLS: Record<SpellId, SpellDefinition> = {
  ...
  ignite: { id: 'ignite', name: 'Ignite', school: 'fire', description: 'A spark that burns after it lands.', unlockLevel: 8, manaCost: 25, cooldownMs: 9000, type: 'dot', effects: [damage('fire', 0.1), periodicDamageStatus({ statusId: 'burning', durationMs: 6000, totalMagnitude: { type: 'spell-power', coefficient: 0.70 }, damageType: 'fire' })], autoCondition: { type: 'always' } },
  ...
  'earth-spike': { id: 'earth-spike', name: 'Earth Spike', school: 'earth', description: 'A heavy spike that strikes with earthen force.', unlockLevel: 2, manaCost: 28, cooldownMs: 7000, type: 'damage', effects: [damage('earth', 0.55)], autoCondition: { type: 'always' } },
  ...
  'air-lance': { id: 'air-lance', name: 'Air Lance', school: 'air', description: 'A focused lance of compressed air that strikes the enemy.', unlockLevel: 2, manaCost: 24, cooldownMs: 8000, type: 'damage', effects: [damage('air', 0.5)], autoCondition: { type: 'always' } },
  ...
  'shock-spark': { id: 'shock-spark', name: 'Shock Spark', school: 'air', description: 'A spark that leaves the enemy vulnerable to subsequent Air damage.', unlockLevel: 16, manaCost: 48, cooldownMs: 10000, type: 'damage', effects: [damage('air', 0.5), { type: 'apply-status', target: 'opponent', statusId: 'shock', stacks: 1, tags: ['debuff'] }], autoCondition: { type: 'always' } },
}

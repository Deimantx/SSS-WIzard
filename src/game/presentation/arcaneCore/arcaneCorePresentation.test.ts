...
describe('Arcane Core V6 presentation', () => {
  it('uses exact alternating 40-degree socket sets in every Core', () => {
    ...
    for (const branch of ARCANE_CORE_BRANCHES) for (const ring of ARCANE_CORE_RING_INDICES) {
      ...
      const angles = nodes.map((node) => normalizeAngle(node.angleDeg)).sort((a, b) => a - b)
      const expected = [0, 40, 80, 120, 160, 200, 240, 280, 320].map((angle) => normalizeAngle(angle + (ring % 2 === 0 ? 20 : 0)))
      expect(angles).toEqual(expected)
      expect(nodes.find((node) => node.nodeType === 'major')?.angleDeg).toBe(ring % 2 === 0 ? 20 : 0)
  ...
  it('interleaves each even Ring exactly halfway between its odd neighbor', () => {
    for (const branch of ARCANE_CORE_BRANCHES) for (const oddRing of [1, 3, 5, 7] as const) {
      ...
      const even = branch.nodes.filter((node) => node.ring === oddRing + 1).map((node) => normalizeAngle(node.angleDeg)).sort((a, b) => a - b)

import type { ArcaneCoreBranchDefinition, ArcaneCoreModifierKey, ArcaneCoreNodeDefinition, ArcaneCoreRingIndex, ArcaneCoreResolvedEffects, ArcaneCoreState } from '../../types'
...
import { getArcaneCoreNodeRank, getArcaneCoreResolvedEffects } from '../../systems/arcaneCore/arcaneCoreProgression'
...
export const getArcaneCoreCombatModifierLabel = (key: CombatModifier['key'], actor: CombatModifier['actor'] = 'player') => `${actor === 'enemy' ? 'Enemy ' : ''}${COMBAT_LABELS[key] ?? key}`
export const formatArcaneCoreCombatModifierValue = (key: CombatModifier['key'], value: number) => `${value >= 0 ? '+' : ''}${key.includes('percent') || ['crit-chance', 'crit-damage'].includes(key) ? `${(value * 100).toFixed(2)}%` : trimNumber(value)}`

export const formatArcaneCoreCondition = (condition: NonNullable<CombatModifier['condition']>, actor: CombatModifier['actor'] = 'player'): string => {
  const subject = actor === 'enemy' ? 'the enemy' : 'you'
  switch (condition.type) {
    case 'always': return 'Always'
    case 'self-hp-below-percent': return `While ${subject} is below ${condition.percent}% Health`
    case 'self-hp-above-percent': return `While ${subject} is above ${condition.percent}% Health`
    ...
    case 'self-mana-above-percent': return `While ${subject} has more than ${condition.percent}% Mana`
    case 'self-mana-below-percent': return `While ${subject} has less than ${condition.percent}% Mana`
    case 'self-has-barrier': return `While ${subject} has Barrier`
    ...
    case 'self-negative-status-count-at-least': return `${actor === 'enemy' ? 'While the enemy has' : 'While you have'} at least ${condition.count} negative Statuses`
    case 'target-has-status-tag': return `Against targets with ${condition.tag} Status`
    case 'target-has-status': return `Against targets with ${condition.statusId} Status`
    case 'self-has-status': return `While ${subject} has ${condition.statusId} Status`
    case 'source-has-tag': return `When the source has ${condition.tag} tag`
    case 'event-status-has-tag': return `When the event Status has ${condition.tag} tag`
    case 'event-status-is': return `When the event Status is ${condition.statusId}`
    ...
    case 'event-target-is-self': return 'When you are the target'
    case 'all': return condition.conditions.map((entry) => formatArcaneCoreCondition(entry, actor)).join(' and ')
    case 'any': return condition.conditions.map((entry) => formatArcaneCoreCondition(entry, actor)).join(' or ')
    case 'not': return `Except when ${formatArcaneCoreCondition(condition.condition, actor).toLowerCase()}`
    default: return 'When its condition is met'
  }
}
...
const isMeaningful = (value: number) => Number.isFinite(value) && Math.abs(value) > 0.0000001
const conditionKey = (condition: NonNullable<CombatModifier['condition']>) => JSON.stringify(condition)
const isAlwaysOn = (condition: CombatModifier['condition']) => !condition || condition.type === 'always'
const isV6Mechanic = (effect: NonNullable<ArcaneCoreResolvedEffects['special']>[number]): effect is Extract<NonNullable<ArcaneCoreResolvedEffects['special']>[number], { type: 'v6-mechanic' }> => effect.type === 'v6-mechanic'
 ...
 */
export const getArcaneCoreResonanceSummary = (core: Pick<ArcaneCoreState, 'nodes'>): ArcaneCoreResonanceSummary => {
  const alwaysOn = new Map<string, { label: string; value: number; sources: ArcaneCoreResonanceSource[]; kind: 'stat' | 'modifier' }>()
  const conditional = new Map<string, { label: string; key: CombatModifier['key']; value: number; actor: CombatModifier['actor']; condition: NonNullable<CombatModifier['condition']>; sources: ArcaneCoreResonanceSource[] }>()
  ...
  ARCANE_CORE_NODES.forEach((node) => {
    ...
    Object.entries(effects.stats ?? {}).forEach(([key, rawValue]) => {
      ...
      const id = `stat:${key}`
      const current = alwaysOn.get(id) ?? { label: getArcaneCoreModifierLabel(key as ArcaneCoreModifierKey), value: 0, sources: [], kind: 'stat' as const }
      current.value += value
      current.sources.push({ nodeId: node.id, nodeName: node.name, value, formattedValue: formatArcaneCoreModifierValue(key as ArcaneCoreModifierKey, value) })
    ...
    ;(effects.modifiers ?? []).forEach((modifier) => {
      ...
      if (isAlwaysOn(modifier.condition)) {
        ...
        const current = alwaysOn.get(id) ?? { label: getArcaneCoreCombatModifierLabel(modifier.key, actor), value: 0, sources: [], kind: 'modifier' as const }
        current.value += modifier.value
        current.sources.push({ nodeId: node.id, nodeName: node.name, value: modifier.value, formattedValue: formatArcaneCoreCombatModifierValue(modifier.key, modifier.value) })
        alwaysOn.set(id, current)
      } else {
        const condition = modifier.condition!
        const id = `conditional:${actor}:${modifier.key}:${conditionKey(condition)}`
        const current = conditional.get(id) ?? { label: getArcaneCoreCombatModifierLabel(modifier.key, actor), key: modifier.key, value: 0, actor, condition, sources: [] }
        current.value += modifier.value
        current.sources.push({ nodeId: node.id, nodeName: node.name, value: modifier.value, formattedValue: formatArcaneCoreCombatModifierValue(modifier.key, modifier.value) })
    ...
    ;(effects.special ?? []).filter(isV6Mechanic).forEach((effect) => {
      if (effect.category === 'STATIC_STAT' || effect.category === 'STATIC_MODIFIER') return
      mechanics.push({ id: node.id, label: node.name, formattedValue: getArcaneCoreNodeEffectTexts(node, rank).filter((text) => text !== 'Inactive').join(' · '), kind: 'mechanic', sourceNodeIds: [node.id], sourceNodeNames: [node.name] })
    })
  })

  const toEntry = (id: string, value: { label: string; value: number; sources: ArcaneCoreResonanceSource[]; kind: 'stat' | 'modifier' }) => ({ id, label: value.label, formattedValue: value.kind === 'stat' ? formatArcaneCoreModifierValue(id.slice(5) as ArcaneCoreModifierKey, value.value) : formatArcaneCoreCombatModifierValue(id.split(':')[2] as CombatModifier['key'], value.value), numericValue: value.value, kind: value.kind, sourceNodeIds: value.sources.map((source) => source.nodeId), sourceNodeNames: value.sources.map((source) => source.nodeName), sources: value.sources })
  return {
    alwaysOn: [...alwaysOn.entries()].filter(([, value]) => isMeaningful(value.value)).map(([id, value]) => toEntry(id, value)),
    conditional: [...conditional.entries()].filter(([, value]) => isMeaningful(value.value)).map(([id, value]) => ({ id, label: value.label, formattedValue: formatArcaneCoreCombatModifierValue(value.key, value.value), numericValue: value.value, kind: 'conditional' as const, conditionText: formatArcaneCoreCondition(value.condition, value.actor), sourceNodeIds: value.sources.map((source) => source.nodeId), sourceNodeNames: value.sources.map((source) => source.nodeName), sources: value.sources })),
    mechanics,
  }
}
...
export const getArcaneCoreNodeEffectTexts = (node: ArcaneCoreNodeDefinition, rank = 1) => {
  ...
  const modifiers = (effects.modifiers ?? []).map((value) => `${getArcaneCoreCombatModifierLabel(value.key, value.actor)} ${formatArcaneCoreCombatModifierValue(value.key, value.value)}${value.condition && value.condition.type !== 'always' ? ` · ${formatArcaneCoreCondition(value.condition, value.actor)}` : ''}`)

...
export const formatCombatEffect = (effect: CombatEffect, source: CombatSource): CombatEffectPresentation => {
  ...
  if (effect.type === 'apply-status') {
    ...
    const value = periodicDamage?.type === 'deal-damage' && periodicDamage.magnitude.type === 'flat' && status?.periodic
      ...
      : undefined
    return { kind: 'status', label: `Applies ${status?.name ?? capitalize(effect.statusId)}`, value, detail: `Target: ${target}`, statusId: effect.statusId, targetLabel: target, timeLabel: duration === null || duration === undefined ? undefined : formatTime(duration) }
  }

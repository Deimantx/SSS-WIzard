...
const input = (changes: Partial<CombatFlowRuntimeInput> = {}): CombatFlowRuntimeInput => ({
  ...
  enemyCurrentStepId: pattern.steps[2].id, enemyCurrentActionPatternId: 'default', enemyActionPatternId: 'default',
...
describe('getCombatFlowPresentation', () => {
  ...
  it('does not show a player timeline when no Spell cast is committed', () => {
    expect(getCombatFlowPresentation(input({ playerSpellCast: null })).playerTimeline).toBeNull()
  })

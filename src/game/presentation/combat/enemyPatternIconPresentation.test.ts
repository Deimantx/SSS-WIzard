...
import {
  ...
  gainBarrier,
  flatHeal,
} from "../../content/monsters/monsterTypes";
...
describe("classifyEnemyActionPatternIcon", () => {
  it("classifies universal effect categories without content IDs", () => {
    ...
    expect(
      classifyEnemyActionPatternIcon(
        action([applyStatus("thorn-wound", "opponent")]),
      ),
    ).toBe("dot-damage");
    expect(
      classifyEnemyActionPatternIcon(
        action([gainBarrier({ type: "flat", value: 30 })]),
      ),
    ).toBe("barrier");
    expect(classifyEnemyActionPatternIcon(action([flatHeal(20)]))).toBe("heal");
    expect(
      classifyEnemyActionPatternIcon(action([delayCurrentAction(500)])),
    ).toBe("control");
    expect(
      classifyEnemyActionPatternIcon(
        action([applyStatus("vulnerable", "opponent")]),
      ),
    ).toBe("debuff");
  });
  ...
  it("uses the obvious primary effect for mixed actions", () => {
    expect(
      classifyEnemyActionPatternIcon(
        action([
          gainBarrier({ type: "flat", value: 30 }),
          applyStatus("haste", "self"),
        ]),
      ),
    ).toBe("barrier");
    expect(
      classifyEnemyActionPatternIcon(
        action([flatHeal(20), applyStatus("haste", "self")]),
      ),
    ).toBe("heal");
    expect(
      classifyEnemyActionPatternIcon(
    ...
    expect(
      classifyEnemyActionPatternIcon(
        action([
          { type: "remove-status", target: "opponent", statusId: "burning" },
          {

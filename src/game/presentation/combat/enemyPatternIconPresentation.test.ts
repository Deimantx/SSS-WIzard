...
import { applyStatus, delayBasicAttack, directDamage, gainBarrier, heal } from '../../content/monsters/monsterTypes'
...
describe('classifyEnemyActionPatternIcon', () => {
  it('classifies universal effect categories without content IDs', () => {
    expect(classifyEnemyActionPatternIcon(action([directDamage('physical', 10)]))).toBe('direct-damage')
    ...
    expect(classifyEnemyActionPatternIcon(action([heal(20)]))).toBe('heal')
  ...
  it('uses the obvious primary effect for mixed actions', () => {
    ...
    expect(classifyEnemyActionPatternIcon(action([heal(20), applyStatus('haste', 'self')]))).toBe('heal')
    expect(classifyEnemyActionPatternIcon(action([directDamage('physical', 10), delayBasicAttack(500)]))).toBe('direct-damage')

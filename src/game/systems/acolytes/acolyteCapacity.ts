...
export const getAcolyteCapacityBreakdown = (state: Pick<GameState, 'tower'> & Partial<Pick<GameState, 'debug'>>): AcolyteCapacityBreakdown => {
  ...
  const total = override ?? base + permanentBonuses + developerBonus
  return { base, permanentBonuses, developerBonus, override, total }
}

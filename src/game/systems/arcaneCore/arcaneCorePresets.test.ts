...
describe('Arcane Core V7 runtime presets', () => {
  ...
  it('validates rank, Ring gates, and depth-based point cost', () => { expect(validateArcaneCorePreset({ nodes: { 'power-r1-arcane-force': { rank: 5 } } })).toBe(true); expect(validateArcaneCorePreset({ nodes: { 'power-r2-opening-blast': { rank: 1 } } })).toBe(false); expect(getArcaneCorePresetSummary(stateWithStarter())).toMatchObject({ nodes: 1, ranks: 1, points: 5, majors: 0 }) })
  it('validates multi-Ring gates, Major gates, caps, and weighted Major points', () => {
    ...
    expect(getArcaneCorePresetSummary({ nodes: { 'power-r1-overwhelming-force': { rank: 1 } } })).toMatchObject({ nodes: 1, ranks: 1, points: 12, majors: 1 })
    expect(applyArcaneCorePreset({ totalPointsEarned: 5, nodes: {} }, multiRing)).toEqual({ ok: false, reason: 'not-enough-core-points' })
  })
  it('loads a valid preset and rejects obsolete IDs', () => { const result = create('Power route', stateWithStarter()); expect(result).toMatchObject({ ok: true }); if (!result.ok) return; const snapshot = getArcaneCorePresetSnapshot(result.presetId); expect(applyArcaneCorePreset({ totalPointsEarned: 100, nodes: {} }, snapshot ?? createInitialArcaneCoreState())).toEqual({ ok: true, state: stateWithStarter() }); expect(applyArcaneCorePreset(createInitialArcaneCoreState(), { nodes: { 'power-a1': { rank: 1 } } })).toEqual({ ok: false, reason: 'invalid-preset' }) })
})

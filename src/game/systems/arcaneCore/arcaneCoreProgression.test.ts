...
describe('Arcane Core V8 progression', () => {
  ...
  it('completes the exact 288-node tree and exposes percentage stat inputs', () => {
    ...
    const mana = ARCANE_CORE_BRANCHES.find((branch) => branch.id === 'mana')!.nodes.find((node) => node.ring === 1 && node.nodeType !== 'major')!
    const stats = getArcaneCoreStaticStats({ nodes: { [vitality.id]: { rank: 5 }, [mana.id]: { rank: 5 } } })

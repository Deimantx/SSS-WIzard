...
const copyState = (state: ArcaneCoreState): ArcaneCoreState => {
  ...
  return { totalXp: safeXp(state.totalXp), nodes }
}
...
export const getArcaneCorePointsSpent = (state: Pick<ArcaneCoreState, 'nodes'>) => Object.entries(state.nodes ?? {}).reduce((sum, [id, progress]) => { const node = getArcaneCoreNode(id); return sum + (node ? getArcaneCoreNodeRank(state, id) * node.rankCost : 0) }, 0)
...
export const getArcaneCoreRingStandardPointsSpent = (state: Pick<ArcaneCoreState, 'nodes'>, branchId: ArcaneCoreBranchId, ring: ArcaneCoreRingIndex) => ARCANE_CORE_NODES.filter((node) => node.branchId === branchId && node.ring === ring && node.nodeType !== 'major').reduce((sum, node) => sum + getArcaneCoreNodeRank(state, node.id) * node.rankCost, 0)
...
export const isArcaneCoreMajorUnlocked = (state: Pick<ArcaneCoreState, 'nodes'>, node: ArcaneCoreNodeDefinition, ignorePrerequisites = false) => node.nodeType !== 'major' || ignorePrerequisites || getArcaneCoreRingStandardPointsSpent(state, node.branchId, node.ring) >= ARCANE_CORE_MAJOR_GATES[node.ring]
...
export const grantArcaneCoreXp = (state: ArcaneCoreState, amount: number) => {
  ...
  return { state: next, requested: granted, granted: xpGranted, xpGranted, levelBefore: beforeLevel, previousLevel: beforeLevel, levelAfter: afterLevel, newLevel: afterLevel, levelsGained: afterLevel - beforeLevel, pointsGained: afterLevel - beforeLevel, reachedMaxLevel: afterLevel === ARCANE_CORE_MAX_LEVEL }
}
...
const cascadeAfterRemoval = (state: ArcaneCoreState, nodeId: string) => {
  ...
  const ringsRelocked = new Set<string>()
  let changed = true
  while (changed) {
    changed = false
    ARCANE_CORE_NODES.forEach((candidate) => {
      if (!isArcaneCoreNodePurchased(next, candidate.id)) return
      if (isArcaneCoreRingUnlocked(next, candidate.branchId, candidate.ring)) return
      removed.add(candidate.id)
      ringsRelocked.add(`${candidate.branchId}-${candidate.ring}`)
      delete next.nodes[candidate.id]
      changed = true
    })
    ARCANE_CORE_NODES.forEach((candidate) => {
  ...
  const directRankRemoved = 1
  const cascadeRanksRemoved = removedIds.reduce((sum, id) => sum + (id === nodeId ? 0 : getArcaneCoreNodeRank(state, id)), 0)
  const ranksAffected = directRankRemoved + cascadeRanksRemoved
  const corePointsReturned = removedIds.reduce((sum, id) => {
    const removedRank = id === nodeId ? 1 : getArcaneCoreNodeRank(state, id)
    return sum + removedRank * (getArcaneCoreNode(id)?.rankCost ?? 0)
  }, 0)
  const majorsAffected = removedIds.filter((id) => getArcaneCoreNode(id)?.nodeType === 'major').length
  return { next, removed: removedIds, ringsRelocked: [...ringsRelocked], corePointsReturned, ranksAffected, majorsAffected }
}
...
export const getArcaneCoreBranchResetPreview = (state: ArcaneCoreState, branchId: ArcaneCoreBranchId): ArcaneCoreBranchResetPreview | { ok: false; reason: ArcaneCoreFailureReason } => {
  ...
  const ranksAffected = nodeIds.reduce((sum, id) => sum + getArcaneCoreNodeRank(state, id), 0)
  const corePointsReturned = nodeIds.reduce((sum, id) => sum + getArcaneCoreNodeRank(state, id) * (getArcaneCoreNode(id)?.rankCost ?? 0), 0)
  return { ok: true, state: next, branchId, nodeIds, nodesAffected: nodeIds.length, corePointsReturned, ranksAffected, majorsAffected: nodeIds.filter((id) => getArcaneCoreNode(id)?.nodeType === 'major').length, ringsRelocked: [...new Set(branch.nodes.filter((node) => nodeIds.includes(node.id)).map((node) => `${branchId}-${node.ring}`))] }
}
...
export const setArcaneCoreNodeRank = (state: ArcaneCoreState, nodeId: string, rank: number): ArcaneCoreActionResult => {
  ...
  if (!node) return { ok: false, reason: 'unknown-node' }
  const next = copyState(state)
  const safe = safeRank(node, rank)
  if (safe > 0) next.nodes[nodeId] = { rank: safe }
  else delete next.nodes[nodeId]
  return { ok: true, state: next }
}
...
export const maxArcaneCoreRing = (state: ArcaneCoreState, branchId: ArcaneCoreBranchId, ring: ArcaneCoreRingIndex) => {
  let current = copyState(state)
  ARCANE_CORE_NODES.filter((node) => node.branchId === branchId && node.ring === ring).forEach((node) => {
    const result = setArcaneCoreNodeRank(current, node.id, node.maxRank)
...
export const resetArcaneCoreRing = (state: ArcaneCoreState, branchId: ArcaneCoreBranchId, ring: ArcaneCoreRingIndex): ArcaneCoreActionResult => {
  const next = copyState(state)
  ARCANE_CORE_NODES.filter((node) => node.branchId === branchId && node.ring >= ring).forEach((node) => delete next.nodes[node.id])
  return { ok: true, state: next }
}
...
export const getArcaneCoreResolvedEffects = (state: Pick<ArcaneCoreState, 'nodes'> | undefined, node: ArcaneCoreNodeDefinition): ArcaneCoreResolvedEffects => {
  const rank = getArcaneCoreNodeRank(state, node.id)
  return rank > 0 ? node.resolveEffects(rank) : {}
}

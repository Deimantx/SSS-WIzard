...
import { validateArcaneCoreMechanicCoverage } from '../../content/arcaneCore/arcaneCoreMechanics'
import { validateArcaneCoreMechanicRuntimeCoverage } from './arcaneCoreMechanicRuntime'
...
export const validateArcaneCoreCatalog = () => {
  ...
  ARCANE_CORE_BRANCHES.forEach((branch) => {
    ...
    for (const ring of ARCANE_CORE_RING_INDICES) {
      ...
      ringNodes.forEach((node) => {
        ...
        ids.add(node.id)
        if (node.branchId !== branch.id || node.ring !== ring || !Number.isFinite(node.angleDeg) || typeof node.resolveEffects !== 'function') errors.push(`${node.id}: invalid Arcane Core node shape`)
      ...
      ringNodes.filter((node) => node.nodeType !== 'major').forEach((node) => {
        if (!node.name.trim() || !node.description.trim()) errors.push(`${node.id}: Arcane Core node requires a player-facing name and description`)
        if (/Basic Attack|Block Chance|Shield/i.test(`${node.name} ${node.description}`)) errors.push(`${node.id}: Arcane Core node references removed player mechanics`)
      })
      const major = ringNodes.find((node) => node.nodeType === 'major')
      if (!major || !major.name.trim() || !major.description.trim()) errors.push(`${branch.id}/ring-${ring}: Arcane Core Major requires a player-facing name and description`)
      if (major && /Basic Attack|Block Chance|Shield/i.test(`${major.name} ${major.description}`)) errors.push(`${major.id}: Arcane Core node references removed player mechanics`)
    }
  })
  if (JSON.stringify(ARCANE_CORE_RING_INDICES.map((ring) => ARCANE_CORE_FULL_RING_COST_BY_RING[ring])) !== JSON.stringify([212, 1344, 6048, 24416, 86240, 198912, 311808, 1250816]) || ARCANE_CORE_TOTAL_COST_PER_CORE !== 1879796 || ARCANE_CORE_TOTAL_TREE_COST !== 7519184) errors.push('Arcane Core cost capacities are invalid')
  ...
  errors.push(...validateArcaneCoreMechanicCoverage())
  errors.push(...validateArcaneCoreMechanicRuntimeCoverage())
  return errors
}

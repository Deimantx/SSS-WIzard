...
export const getArtificingCraftIngredients = (id: ArtificingRecipeId | ArtifactId) => isArtifactId(id) ? ARTIFACTS[id].forge.ingredients : ARTIFICING_RECIPES[id]?.ingredients
const artifactAlreadyOwned = (state: Pick<GameState, 'inventory' | 'artifactProgress'>, id: ArtificingRecipeId) => isArtifactId(id) && ((state.inventory[id] ?? 0) > 0 || Boolean(state.artifactProgress?.[id]))
...
export const startArtificingCraft = (state: GameState, id: ArtificingRecipeId): ArtificingCraftResult => {
  ...
  if (!recipe || ITEMS[recipe.output.itemId]?.kind !== 'equipment') return { ok: false, reason: 'Only Equipment recipes can be forged.' }
  ...
  if (isArtifactId(id) && (state.inventory[id] ?? 0) > 0) return { ok: false, reason: 'This Artifact can only be forged once.' }
  ...
  state.activities.artificing = { activeJob: isArtifactId(id) ? { kind: 'artifact-forge', artifactId: id } : { kind: 'recipe', recipeId: id }, activeRecipeId: isArtifactId(id) ? null : id, progressMs: 0 }

...
import { completeArtifactForge } from '../artifacts/artifactProgression'
...
export const getArtificingCraftIngredients = (id: ArtificingRecipeId | ArtifactId) => ARTIFACTS[id].forge.ingredients
const artifactAlreadyOwned = (state: Pick<GameState, 'inventory' | 'artifactProgress'>, id: ArtificingRecipeId) => (state.inventory[id] ?? 0) > 0 || Boolean(state.artifactProgress?.[id])
...
export const startArtificingCraft = (state: GameState, id: ArtificingRecipeId): ArtificingCraftResult => {
  ...
  if ((state.inventory[id] ?? 0) > 0) return { ok: false, reason: 'This Artifact can only be forged once.' }
  ...
  state.activities.artificing = { activeJob: { kind: 'artifact-forge', artifactId: id }, activeRecipeId: null, progressMs: 0 }

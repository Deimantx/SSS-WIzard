...
export const startArtifactUpgrade = (state: GameState, id: ArtifactId): ArtificingCraftResult => {
  const progress = getArtifactProgress(state, id); const upgrade = getArtifactUpgrade(id, progress.level); const cap = getArtifactLevelCap(state, id)
  if (!ARTIFACTS[id] || (state.inventory[id] ?? 0) < 1) return { ok: false, reason: 'Artifact is not owned.' }
  if (active(state)) return { ok: false, reason: 'Another Artificing job is already in progress.' }
...
export const advanceArtificing = (state: GameState, deltaMs: number, onComplete?: (itemId: ItemId) => void) => {
  ...
  let itemId: ItemId
  if (job.kind === 'artifact-forge') { if (!completeArtifactForge(state, job.artifactId)) { clear(state); return null }; itemId = job.artifactId }
  else if (job.kind === 'artifact-upgrade') { const progress = state.artifactProgress[job.artifactId]; if (!progress || progress.level !== job.fromLevel) { clear(state); return null }; progress.level = job.toLevel; itemId = job.artifactId }
  else { itemId = ARTIFICING_RECIPES[job.recipeId].output.itemId; grantItem(state, itemId, 1) }
  clear(state); onComplete?.(itemId); return { ok: true, itemId } as const
}

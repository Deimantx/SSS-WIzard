...
import {
  consumeBarrier,
  gainBarrierResult,
  gainBarrier as gainBarrierRuntime,
  getActiveBarrier,
} from "./barrierRuntime";
import {
  getCombatModifiers,
...
const targetActor = (
  ...
  target === "self"
    ...
    : source.actor === "player"
      ? "enemy"
      : "player";
const sourceTags = (source: CombatSource, effectTags?: CombatTag[]) => [
  ...new Set<CombatTag>([...(source.tags ?? []), ...(effectTags ?? [])]),
];
const logSource = (state: GameState, source: CombatSource) => {
  ...
  const monsterId =
    source.sourceMonsterId ?? root.originMonsterId ?? root.sourceMonsterId;
  ...
  return monsterId
    ? { kind: "enemy" as const, monsterId }
    : { kind: "system" as const };
};
const logCategory = (
  source: CombatSource,
  tags: CombatTag[],
  fallback: CombatLogCategory,
): CombatLogCategory =>
  source.kind === "spell"
    ? "spell"
    : source.kind === "action"
...
const eventFields = (
  ...
  return {
    ...
    statusId:
      source.statusId ??
      (source.kind === "status" ? (source.sourceId as StatusId) : undefined),
    itemId:
      source.kind === "equipment" || source.kind === "weapon"
        ? (source.sourceId as import("../../types").ItemId)
        : undefined,
  };
};
...
const isDirectHit = (tags: CombatTag[]) =>
  tags.includes("direct") && !tags.includes("dot");

const emptyDamageBreakdown = (
  raw: number,
  resistance: number,
  direct: boolean,
  critChance: number,
...
const calculateCombatDamageWithRolls = (
  state: GameState,
  raw: number,
...
export const calculateCombatDamage = (
  ...
  source: CombatSource,
  target: CombatActor,
  tags: CombatTag[] = source.tags ?? [],
): DamageBreakdown =>
  calculateCombatDamageWithRolls(state, raw, damageType, source, target, tags);

const applyDamage = (
  state: GameState,
  components: Array<{ raw: number; damageType: DamageType }>,
  source: CombatSource,
  target: CombatActor,
  tags: CombatTag[],
  execute: ExecuteCombatEffects,
  depth: number,
  ...
  if (isDirectHit(tags)) {
    rolls.critical =
      nextCombatRandom(state) < getCritChance(state, source.actor, source);
    rolls.blocked =
      nextCombatRandom(state) < getBlockChance(state, target, source);
  }
  ...
  const preClampComponentEvents: CombatDamageComponentEvent[] = breakdowns.map(
    (breakdown, index) => {
      const barrierAbsorbed = Math.min(
        remainingBarrier,
        breakdown.resolvedBeforeBarrier,
      );
      ...
      return {
        ...
        healthDamage: Math.max(
          0,
          breakdown.resolvedBeforeBarrier - barrierAbsorbed,
        ),
        barrierAbsorbed,
        immune: breakdown.immune,
      };
    },
  );
  const totalBarrierAbsorbed = preClampComponentEvents.reduce(
    (sum, component) => sum + component.barrierAbsorbed,
    0,
  );
  consumeBarrier(state, target, totalBarrierAbsorbed);
  const attemptedHealthDamage = preClampComponentEvents.reduce(
    (sum, component) => sum + component.healthDamage,
    0,
  );
  const immortal =
    target === "player"
      ? state.debug.playerImmortal
      : state.debug.enemyImmortal;
  ...
  if (target === "player") {
    survivedLethal =
      ...
      previousHp > 0 &&
      nextHealth <= 0 &&
      tryConsumeArcaneCoreSurvival(state);
    state.player.health =
      immortal && previousHp > 0
        ? Math.max(1, nextHealth)
        : survivedLethal
          ? 1
          : nextHealth;
    if (survivedLethal) {
      ...
      if (
        state.combat.arcaneCoreRuntime.immortalGuardUntilMs ===
        state.combat.arcaneCoreRuntime.elapsedMs + 6_000
      ) {
        cleanseStatuses(state, "player", "all", undefined, {
      ...
      uiEvents?.push({
        ...
        category: "system",
        sourceId: "arcane-core-survival-instinct",
        amount: 1,
        effectiveAmount: 1,
      });
    }
    ...
    state.combat.enemyHp =
      immortal && previousHp > 0 ? Math.max(1, nextHealth) : nextHealth;
  ...
  const componentEvents: CombatDamageComponentEvent[] =
    preClampComponentEvents.map((component) => {
      const healthDamage = Math.min(
        remainingHealthDamage,
        component.healthDamage,
      );
  ...
  if (source.actor === "player" && source.kind === "spell")
    cascade.spellHealthDamageTotal =
      (cascade.spellHealthDamageTotal ?? 0) + dealt;
  if (source.actor === "player")
    state.combat.lastDamageDealt =
      source.kind === "spell"
        ? (cascade.spellHealthDamageTotal ?? dealt)
        : dealt;
  ...
  uiEvents?.push({
    ...eventFields(state, source, target),
    category: logCategory(source, tags, "damage"),
    damageType: damageTypes.length === 1 ? damageTypes[0] : undefined,
    damageTypes,
    damageComponents: componentEvents,
    hitId: `${cascade.cascadeId}:hit:${cascade.hitSequence}`,
    amount: resolvedBeforeBarrier,
    healthDamage: dealt,
    barrierAbsorbed: totalBarrierAbsorbed,
    barrierBefore,
    barrierAfter: getActiveBarrier(state, target),
    critical: first?.critical ?? false,
    critChance: first?.critChance ?? 0,
    critMultiplier: first?.critMultiplier ?? 1,
    blocked: first?.blocked ?? false,
    blockChance: first?.blockChance ?? 0,
    blockReduction: first?.blockReduction ?? 0,
    blockedAmount,
  });
  const context: CombatEventContext = {
    ...
    damageType: damageTypes.length === 1 ? damageTypes[0] : undefined,
    damageTypes,
    previousBarrier: barrierBefore,
    currentBarrier: getActiveBarrier(state, target),
    previousHp,
    currentHp,
    previousHpPercent: (previousHp / Math.max(1, maxHp)) * 100,
    currentHpPercent: (currentHp / Math.max(1, maxHp)) * 100,
    critical: first?.critical ?? false,
    blocked: first?.blocked ?? false,
  };
  if (
    ...
    barrierBefore > 0 &&
    getActiveBarrier(state, target) === 0
  ) {
  ...
  const hitEvent = tags.includes("basic-attack")
    ? "on-basic-attack-hit"
    : source.kind === "spell"
      ? "on-spell-hit"
      : null;
  ...
  if (previousHp > 0 && currentHp <= 0 && dealt > 0)
    runCombatTriggers(
      state,
      source.actor,
      "on-kill",
  ...
  if (dealt > 0) {
    ...
    thresholdActors.forEach((thresholdActor) =>
      runCombatTriggers(
        state,
        thresholdActor,
        "on-hp-threshold",
...
const applyHealing = (
  ...
  const amount = Math.max(
    ...
    raw *
      (1 +
        getCombatModifiers(state, source.actor, "healing-done-percent", {
          source,
          sourceTags: tags,
        })),
  );
...
const executeResource = (
  ...
  effect: Extract<
    CombatEffect,
    { type: "restore-resource" | "drain-resource" }
  >,
  source: CombatSource,
  uiEvents?: CombatEventSink,
) => {
...
export const executeCombatEffect = (
  state: GameState,
  effect: CombatEffect,
  source: CombatSource,
  depth = 0,
  execute: ExecuteCombatEffects = executeCombatEffects,
  uiEvents?: CombatEventSink,
  resolution?: CombatResolutionContext,
) => {
  ...
  switch (effect.type) {
    case "deal-damage": {
      const effectSource = effect.school
        ? { ...source, school: effect.school }
        : source;
      const components = effect.components.map((component) => ({
        raw: resolveMagnitude(state, component.magnitude, effectSource, target),
    ...
    case "heal":
      applyHealing(
        state,
        resolveMagnitude(state, effect.magnitude, source, target),
        source,
        target,
        tags,
        execute,
    ...
    case "gain-barrier": {
      ...
      const result = gainBarrierResult(
        state,
        resolveMagnitude(state, effect.magnitude, source, target),
        source,
        target,
        tags,
        {
          mode,
          durationMs:
            effect.durationMs === undefined ? null : effect.durationMs,
        },
      );
      if (result.current > 0 && (result.gained > 0 || mode === "replace")) {
        const granted =
          mode === "replace" || mode === "replace-if-stronger"
            ? result.current
            : result.gained;
        uiEvents?.push({
          ...eventFields(state, source, target),
          category: "barrier",
          amount: granted,
          barrierGranted: granted,
          barrierMode: mode,
          barrierBefore: result.previous,
          barrierAfter: result.current,
        ...
        if (result.gained > 0)
          runCombatTriggers(
            ...
            "on-barrier-gained",
            {
              ...
              changedActor: target,
              sourceTags: tags,
              previousBarrier: result.previous,
              currentBarrier: result.current,
              barrierGained: result.gained,
    ...
    case "apply-status": {
      ...
      if (target === "enemy" && effect.statusId === "frozen") {
        removeStatus(state, target, "chilled", {
          executeEffects: execute,
          source,
          depth,
          uiEvents,
      ...
      if (
        target === "enemy" &&
        effect.statusId === "chilled" &&
        targetStatuses.some((status) => status.statusId === "frozen")
      )
        break;
      const statusSource = { ...source, tags };
      const active = applyStatus(state, target, effect.statusId, statusSource, {
        durationMs: effect.durationMs,
        stacks: effect.stacks,
        periodicEffects: effect.periodicEffects,
        statusSourceKey: effect.statusSourceKey,
        modifierOverrides: effect.modifierOverrides,
      });
      if (active) {
        ...
        uiEvents?.push({
          ...
          statusPhase: "apply",
          durationMs: active.remainingMs,
          stacks: active.stacks,
          statusInstanceKey: active.instanceKey,
        });
        ...
        const statusContext = {
          ...
          sourceTags: tags,
          statusId: effect.statusId,
          eventStatusTags: definition.tags,
        };
    ...
    case "remove-status":
      if (
        removeStatus(state, target, effect.statusId, {
          executeEffects: execute,
          source,
          depth,
          uiEvents,
        ...
        appendLog(
          state,
          `${STATUS_DEFINITIONS[effect.statusId]?.name ?? effect.statusId} removed.`,
        );
      break;
    case "detonate-status": {
      ...
      statuses
        .filter(
          (active) =>
            active.statusId === effect.statusId &&
            active.source.actor === source.actor,
        )
        .slice()
        .forEach((active) => {
          ...
          const ticks = Math.max(
            0,
            Math.ceil((active.remainingMs ?? 0) / interval),
          );
          const periodic =
            active.periodicEffects ?? definition?.periodic?.effects ?? [];
          periodic.forEach((periodicEffect) => {
            if (periodicEffect.type !== "deal-damage") return;
            const scaled = periodicEffect.components.map((component) => ({
              ...component,
              magnitude: scaleMagnitude(
                component.magnitude,
                ticks * Math.max(0, effect.multiplier),
              ),
            }));
            execute(
              state,
              [
                {
                  ...
                  components: scaled,
                  target: "opponent",
                  tags: [...(periodicEffect.tags ?? []), "direct"],
                },
              ],
              { ...source, tags: [...(source.tags ?? []), "direct"] },
              depth + 1,
              uiEvents,
              cascade,
            );
    ...
    case "cleanse":
      cleanseStatuses(state, target, effect.mode, effect.tag, {
        executeEffects: execute,
        source,
        depth,
        uiEvents,
      ...
      break;
    case "dispel":
      dispelStatuses(state, target, effect.mode, effect.tag, {
        executeEffects: execute,
        source,
        depth,
        uiEvents,
    ...
    case "modify-action-timer": {
      ...
      const adjustWork = (value: number) =>
        Math.max(
          0,
          Math.min(
            MAX_ACTION_WORK_MS,
            (Number.isFinite(value) ? value : 0) + effect.amountMs,
          ),
        );
      let applied = false;
      if (target === "player") {
        if (
          effect.action === "current" &&
          state.combat.pendingPlayerSpellCast
        ) {
          state.combat.pendingPlayerSpellCast.remainingWorkMs = adjustWork(
        ...
        if (state.combat.enemyCurrentStepId) {
          state.combat.enemyActionTimerMs = adjustWork(
      ...
      appendLog(
        state,
        `${effect.amountMs >= 0 ? "Action delayed" : "Action timer changed"} by ${Math.abs(effect.amountMs)}ms.`,
      );
      uiEvents?.push({
        ...eventFields(state, source, target),
        category: "system",
        sourceId: "action-timer",
        amount: Math.abs(effect.amountMs),
        durationMs: effect.amountMs,
      });
      break;
    }
    case "modify-cooldown": {
      if (target === "player" && Number.isFinite(effect.amountMs)) {
        const resolvedSpellId =
          effect.spellId === "source"
            ? (source.eventSource?.sourceId ?? source.sourceId)
            : effect.spellId;
        const ids = resolvedSpellId
          ? [resolvedSpellId]
          : Object.keys(state.combat.spellCooldowns);
        ids.forEach((id) => {
          if (id in state.combat.spellCooldowns)
            state.combat.spellCooldowns[
              id as keyof typeof state.combat.spellCooldowns
            ] = Math.max(
              0,
              Math.min(
                MAX_ACTION_WORK_MS,
    ...
    case "set-action-pattern":
      if (target === "enemy" && isCombatActorAlive(state, target))
        setEnemyActionPattern(state, effect.patternId, uiEvents);
      break;
  }
};

export const executeCombatEffects: ExecuteCombatEffects = (
  state,
  effects,
  source,
  depth = 0,
...
/** Compatibility source factory for deterministic test/DevTools damage helpers. */
const legacyDamageSource = (
  source: "basic" | "spell" | "status",
  actor: "player" | "enemy",
): CombatSource => ({
  actor,
  kind:
...
export const damageEnemy = (
  state: GameState,
  raw: number,
  source: "basic" | "spell" | "status" = "spell",
) => {
...
export const damagePlayer = (
  state: GameState,
  raw: number,
  source: CombatSource = legacyDamageSource("basic", "enemy"),
) =>
  applyDamage(
    state,
    [{ raw, damageType: "physical" }],
...
export const getCombatDamagePreview = (
  state: GameState,
  raw: number,
  source: CombatSource,
  target: CombatActor,
  ...
  const breakdown = calculateCombatDamage(
    state,
    raw,
    damageType,
    source,
  ...
  return {
    ...breakdown,
    modified: breakdown.resolvedBeforeBarrier,
    barrier: getActiveBarrier(state, target),
  };
};

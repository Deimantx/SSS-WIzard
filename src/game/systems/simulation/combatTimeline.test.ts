...
const comparableStatus = (status: GameState['combat']['playerStatuses'][number] | GameState['combat']['enemyStatuses'][number]) => {
  const { appliedAt: _appliedAt, ...rest } = status
  const normalized = JSON.parse(JSON.stringify(rest)) as typeof rest
  if (typeof normalized.nextTickMs === 'number') normalized.nextTickMs = Math.round(normalized.nextTickMs * 1_000_000_000) / 1_000_000_000
  return normalized
}
...
const combatSnapshot = (state: GameState) => ({
  ...
  pendingPlayerSpellCast: state.combat.pendingPlayerSpellCast ? {
    ...
    remainingWorkMs: state.combat.pendingPlayerSpellCast.remainingWorkMs,
    castWorkMs: state.combat.pendingPlayerSpellCast.castWorkMs,
    castOrigin: state.combat.pendingPlayerSpellCast.castOrigin,
  } : null,
  ...
  playerStatuses: state.combat.playerStatuses.map(comparableStatus),
  enemyStatuses: state.combat.enemyStatuses.map(comparableStatus),
})
...
describe('shared combat timeline', () => {
  ...
  it('snapshots Haste for the next Action before Haste expires later in the same quantum', () => {
    ...
    expect(state.combat.enemyCurrentStepId).toBe('crushing-maul-step-1')
  ...
  it('does not make Auto-Cast ready before its internal cooldown boundary', () => {
    ...
    state.combat.activeSpellLoadout!.slots = [{ spellId: 'searing-touch', autoCast: true }]

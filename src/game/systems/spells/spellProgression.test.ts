...
describe('Spell progression foundation', () => {
  ...
  it('defines exactly the current 12-spell tutorial roster with validated content', () => {
    ...
    expect(SPELLS['water-ward']).toMatchObject({ cooldownMs: 8000, manaCost: 15, effects: [expect.objectContaining({ type: 'gain-barrier', magnitude: { type: 'spell-power', coefficient: 0.7 } })] })
    expect(SPELLS.stoneguard).toMatchObject({ cooldownMs: 18000, manaCost: 22, effects: [expect.objectContaining({ type: 'gain-barrier', magnitude: { type: 'spell-power', coefficient: 1.3 } })] })
  })
})

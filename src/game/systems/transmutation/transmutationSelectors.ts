...
export type TransmutationStatus = 'paused' | 'active' | 'waiting-mana' | 'waiting-materials' | 'locked'
...
export function getRecipeStatus(state: Pick<GameState, 'activities' | 'inventory' | 'protectedItems' | 'equipment' | 'player' | 'progress'>, recipe: RecipeDefinition): TransmutationStatus {
  ...
  if (progress >= recipe.baseDurationMs) {
    if (!hasRecipeMaterials(state, recipe)) return 'waiting-materials'
    if (state.player.mana < recipe.manaCost) return 'waiting-mana'
  }
  return 'active'
}

...
describe('combat telemetry foundation', () => {
  ...
  it('keeps Equipment proc damage, healing, and barriers out of Basic Attack analytics', () => {
    const source = { source: { kind: 'player' } as const, sourceKind: 'equipment' as const, sourceId: 'tideglass-wand', providerInstanceKey: 'weapon', ruleId: 'flame-retaliation' }
    ...
    expect(getCombatMetricSourceKey(damage)).toBe('player:equipment:tideglass-wand:provider:weapon:rule:flame-retaliation')
    ...
    const contribution = scope.player.damageDone.bySource['player:equipment:tideglass-wand:provider:weapon:rule:flame-retaliation']
    expect(contribution.kind).toBe('equipment')
    expect(contribution.total).toBe(30)
    expect(scope.player.healingDone.bySource['player:equipment:tideglass-wand:provider:weapon:rule:flame-retaliation'].effectiveHealing).toBe(4)
    expect(scope.player.barrierGrantedBySource['player:equipment:tideglass-wand:provider:weapon:rule:flame-retaliation'].barrierGranted).toBe(5)
  ...
  it('keeps duplicate Equipment providers and Status origins distinct without changing totals', () => {
    const weapon = { source: { kind: 'player' } as const, sourceKind: 'equipment' as const, sourceId: 'tideglass-wand', providerInstanceKey: 'weapon', ruleId: 'burn' }
    const head = { ...weapon, providerInstanceKey: 'head' }
    expect(getCombatMetricSourceKey({ ...weapon, category: 'damage' as const, amount: 1 })).not.toBe(getCombatMetricSourceKey({ ...head, category: 'damage' as const, amount: 1 }))
    const status1 = { source: { kind: 'player' } as const, sourceKind: 'status' as const, sourceId: 'burning', statusId: 'burning' as const, originSourceKind: 'equipment' as const, originSourceId: 'tideglass-wand', providerInstanceKey: 'weapon', ruleId: 'burn', category: 'damage' as const, amount: 5 }
    const status2 = { ...status1, providerInstanceKey: 'head' }

...
describe('combat telemetry foundation', () => {
  ...
  it('keeps Equipment proc damage, healing, and barriers out of Basic Attack analytics', () => {
    const source = { source: { kind: 'player' } as const, sourceKind: 'equipment' as const, sourceId: 'tideglass-wand', providerInstanceKey: 'ring1', ruleId: 'flame-retaliation' }
    ...
    expect(getCombatMetricSourceKey(damage)).toBe('player:equipment:tideglass-wand:provider:ring1:rule:flame-retaliation')
    ...
    const contribution = scope.player.damageDone.bySource['player:equipment:tideglass-wand:provider:ring1:rule:flame-retaliation']
    expect(contribution.kind).toBe('equipment')
    expect(contribution.total).toBe(30)
    expect(scope.player.healingDone.bySource['player:equipment:tideglass-wand:provider:ring1:rule:flame-retaliation'].effectiveHealing).toBe(4)
    expect(scope.player.barrierGrantedBySource['player:equipment:tideglass-wand:provider:ring1:rule:flame-retaliation'].barrierGranted).toBe(5)
  ...
  it('keeps duplicate Equipment providers and Status origins distinct without changing totals', () => {
    const ring1 = { source: { kind: 'player' } as const, sourceKind: 'equipment' as const, sourceId: 'tideglass-wand', providerInstanceKey: 'ring1', ruleId: 'burn' }
    ...
    const status1 = { source: { kind: 'player' } as const, sourceKind: 'status' as const, sourceId: 'burning', statusId: 'burning' as const, originSourceKind: 'equipment' as const, originSourceId: 'tideglass-wand', providerInstanceKey: 'ring1', ruleId: 'burn', category: 'damage' as const, amount: 5 }

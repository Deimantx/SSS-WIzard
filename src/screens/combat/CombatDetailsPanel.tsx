...
function CombatDetailsRow({ mode, row }: { mode: CombatDetailsMode; row: CombatDetailsRowPresentation }) {
  ...
  return <GameTooltip block wide content={<CombatDetailsTooltip mode={mode} row={row} />}><div className={`combat-details-row metric-source-accent-${row.source.accent}`} tabIndex={0} style={{ '--details-row-percent': `${Math.max(row.total > 0 ? 2 : 0, row.percent)}%` } as CSSProperties} aria-label={`${row.rank}. ${row.source.name}, ${percent(row.percent)}, ${exact(row.total)}`}><span className="combat-details-rank">{row.rank}.</span><span className="combat-details-source-icon">{icon}</span><span className="combat-details-source"><strong>{row.source.name}</strong>{row.source.subtitle && <small>{row.source.subtitle}</small>}</span><span className="combat-details-bar" aria-hidden="true" /><span className="combat-details-row-total">{row.compactTotal}</span><span className="combat-details-row-percent">{percent(row.percent)}</span></div></GameTooltip>
}

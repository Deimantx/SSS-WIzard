...
export function CombatDetailsPanel() {
  const telemetrySignal = useCombatTelemetryStore(useShallow((state) => {
    const scope = state.run ?? state.lastRun
    return {
      scopeId: scope?.scopeId ?? null,
      aggregateRevision: scope?.aggregateRevision ?? 0,
      engagedBucket: Math.floor((scope?.engagedMs ?? 0) / 250),
      resetMeasurement: state.resetMeasurement,
    }
  }))

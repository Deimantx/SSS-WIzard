...
export function CombatResource({ icon, label, value, percent, tone, currentValue, maxValue, previousValue, feedback }: CombatResourceProps) {
  ...
  useEffect(() => {
    ...
    if (previous !== measuredValue && nextFeedback) {
      ...
      setTrailPercent(Math.max(0, Math.min(100, previous / normalizedMax * 100)))
      const timer = window.setTimeout(() => { setActiveFeedback(null); setTrailPercent(null) }, feedbackDurationMs)

...
export function CombatRunBar({ selectedDungeonId, onOpenAtlas, onRequestLeave }: { selectedDungeonId: DungeonId; onOpenAtlas: () => void; onRequestLeave: () => void }) {
  ...
  const bossStatus = mode === 'boss-fight' ? 'FIGHTING' : mode === 'boss-queued' ? 'QUEUED' : mode === 'boss-ready' ? 'READY' : mode === 'tower' ? bossKills ? `${bossKills} ${bossKills === 1 ? 'CLEAR' : 'CLEARS'}` : 'NOT CLEARED' : nearBoss ? 'BOSS APPROACHING' : 'INCOMING'

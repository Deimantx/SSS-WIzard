...
export function CombatSpellTile({ spellId, presentationState, globalBlocker }: { spellId: SpellId; presentationState: SpellPresentationState; globalBlocker?: 'inactive' | 'no-target' | 'stunned' | null }) {
  ...
  return <div className={`spell-combat-tile${active ? ' is-auto' : ''}${cooldown > 0 ? ' is-cooldown' : ''}${failure === 'mana' ? ' is-mana-starved' : ''}`} style={{ '--spell-school-color': SCHOOLS[spell.school].color, '--cooldown-percent': `${Math.max(0, Math.min(100, cooldown / Math.max(1, spell.cooldownMs) * 100))}%` } as React.CSSProperties}>

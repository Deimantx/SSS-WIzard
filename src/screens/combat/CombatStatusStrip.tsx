...
export function CombatStatusStrip({ statuses, label }: { statuses: ActiveStatus[]; label: string }) {
  return <section className="combat-status-strip" aria-label={label}><div className="combat-subsection-label">{label}</div>{statuses.length ? <div className="combat-status-list">{statuses.map((status) => <CombatStatusChip key={status.statusId} status={status} />)}</div> : <span className="combat-status-empty">None active</span>}</section>
}

export function CombatStatusChip({ status }: { status: ActiveStatus }) {
  const definition = STATUS_DEFINITIONS[status.statusId]
  if (!definition) return null
  const category = definition.tags.includes('dot') ? 'Damage over time' : definition.tags.includes('control') ? 'Control' : definition.classification === 'buff' ? 'Buff' : definition.classification === 'debuff' ? 'Debuff' : 'Status'
  ...
  return <GameTooltip block accent={accent} content={<TooltipContent title={definition.name} description={definition.description}><div className="tooltip-section"><small>TYPE</small><p>{category}</p></div><div className="tooltip-section"><small>REMAINING</small><p>{duration}</p></div><div className="tooltip-section"><small>STACKS</small><p>{status.stacks}</p></div>{source && <div className="tooltip-section"><small>SOURCE</small><p>{source}</p></div>}</TooltipContent>}><span className={`combat-status-chip status-kind-${definition.classification}`} tabIndex={0} aria-label={`${definition.name}, ${category}, ${status.stacks} stack${status.stacks === 1 ? '' : 's'}, ${duration} remaining`}><span className="combat-status-icon"><StatusIcon status={status} /></span><strong>{definition.name}</strong>{status.stacks > 1 && <b>×{status.stacks}</b>}<small>{duration}</small></span></GameTooltip>
}

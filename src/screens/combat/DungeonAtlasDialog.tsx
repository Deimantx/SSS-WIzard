...
import { buildCombatRegionMap, buildWorldRegionMap, getRegionForDungeon, isWorldMapUnlocked } from './combatNavigationReadModel'
import type { CombatNavigationMode, CombatRegionMapNode, WorldRegionId, WorldRegionMapNode } from './combatNavigationTypes'
import { RegionConstellationView } from './atlas/RegionConstellationView'
import { WorldAtlasView } from './atlas/WorldAtlasView'
...
export function DungeonAtlasDialog({ selectedDungeonId, onSelect, onClose }: { selectedDungeonId: DungeonId; onSelect: (id: DungeonId) => void; onClose: () => void }) {
  ...
  const [mode, setMode] = useState<CombatNavigationMode>('region')
  const currentDungeonId = combat.active ? combat.dungeonId ?? selectedDungeonId : selectedDungeonId
  const [selectedRegionId, setSelectedRegionId] = useState<WorldRegionId>(() => getRegionForDungeon(currentDungeonId) as WorldRegionId)
  const regionMap = buildCombatRegionMap(progress, combat)
  const worldMap = buildWorldRegionMap(progress)
  const selectedNode = regionMap.nodes.find((node) => node.id === currentDungeonId) ?? regionMap.nodes[0]
  ...
  useEffect(() => {
    setSelectedRegionId(getRegionForDungeon(currentDungeonId) as WorldRegionId)
  }, [currentDungeonId])

  const handleEnterRegion = (regionId: WorldRegionId = selectedRegion.id): boolean => {
    const region = worldMap.nodes.find((node) => node.id === regionId)
    if (!region?.linkedRegionMapId) return false
  ...
  const handleEnterDungeon = (dungeonId: DungeonId): boolean => {
    const node = regionMap.nodes.find((entry) => entry.id === dungeonId)
    if (!node || node.status === 'locked' || combat.active) return false
    onSelect(dungeonId)
    enter(dungeonId)
    onClose()
    return true
  ...
  return <ModalPortal open onClose={onClose} backdropClassName="combat-modal-backdrop" surfaceClassName="dungeon-atlas-dialog combat-navigation-dialog" ariaLabelledBy="combat-navigation-title">
    <header className="combat-modal-head combat-navigation-head"><div><h2 id="combat-navigation-title">{mode === 'world' ? 'THE WORLD MAP' : `${regionMap.name.toUpperCase()} ROUTES`}</h2><p>{mode === 'world' ? worldMap.description : regionMap.description}</p></div><div className="combat-navigation-head-actions"><div className="combat-navigation-mode-switch" role="tablist" aria-label="Combat map mode"><button type="button" role="tab" aria-selected={mode === 'region'} data-ui-sound="click" className={mode === 'region' ? 'is-selected' : ''} onClick={() => setMode('region')}><Map size={13} aria-hidden="true" /> REGION</button>{worldUnlocked && <button type="button" role="tab" aria-selected={mode === 'world'} data-ui-sound="click" className={mode === 'world' ? 'is-selected' : ''} onClick={() => setMode('world')}><Globe size={13} aria-hidden="true" /> WORLD</button>}</div><Button icon variant="ghost" ariaLabel="Close combat navigation" onClick={onClose}><X size={17} aria-hidden="true" /></Button></div></header>
    <div className={`combat-navigation-body combat-navigation-view-${mode}`}>{mode === 'region' ? <><section className="combat-navigation-map-panel"><RegionConstellationView key="deep-woods-constellation" map={regionMap} selectedId={selectedNode.id} onSelect={(id) => onSelect(id as DungeonId)} onEnter={handleEnterDungeon} /></section><RegionInspector node={selectedNode} combat={combat} progress={progress} onEnter={() => handleEnterDungeon(selectedNode.targetAreaId)} /></> : <><section className="combat-navigation-map-panel"><WorldAtlasView key="world-atlas" map={worldMap} selectedId={selectedRegion.id} onSelect={(id) => setSelectedRegionId(id as WorldRegionId)} onEnter={handleEnterRegion} /></section><WorldInspector node={selectedRegion} onEnter={() => handleEnterRegion(selectedRegion.id)} /></>}</div>

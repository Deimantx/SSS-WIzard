...
export function DungeonAtlasDialog({ selectedDungeonId, onSelect, onClose, initialMode = 'region' }: { selectedDungeonId: DungeonId; onSelect: (id: DungeonId) => void; onClose: () => void; initialMode?: CombatNavigationMode }) {
  ...
  return <ModalPortal open onClose={onClose} backdropClassName="combat-modal-backdrop" surfaceClassName="dungeon-atlas-dialog combat-navigation-dialog" ariaLabelledBy="combat-navigation-title">
    ...
    <div className={`combat-navigation-body combat-navigation-view-${mode}`}>{mode === 'region' ? <><section className="combat-navigation-map-panel"><div className="combat-navigation-panel-head"><div><span className="combat-subsection-label">{regionMap.subtitle}</span><strong>Choose a combat destination</strong></div><small>{regionMap.nodes.length} WAYPOINTS · CONSTELLATION</small></div><RegionConstellationView key="deep-woods-constellation" map={regionMap} selectedId={selectedNode.id} onSelect={(id) => onSelect(id as DungeonId)} /></section><RegionInspector node={selectedNode} combat={combat} progress={progress} onEnter={() => { enter(selectedNode.targetAreaId); onClose() }} /></> : <><section className="combat-navigation-map-panel"><div className="combat-navigation-panel-head"><div><span className="combat-subsection-label">{worldMap.subtitle}</span><strong>Arcane territory atlas</strong></div><small>{worldMap.nodes.length} REGION PLATES · DRAG TO EXPLORE</small></div><WorldAtlasView key="world-atlas" map={worldMap} selectedId={selectedRegion.id} onSelect={(id) => setSelectedRegionId(id as WorldRegionId)} /></section><WorldInspector node={selectedRegion} onEnter={handleEnterRegion} /></>}</div>

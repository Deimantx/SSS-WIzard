...
function DropsMode({ presentation }: { presentation: ReturnType<typeof getDungeonStatisticsPresentation> }) {
  return <div className="dungeon-statistics-content dungeon-statistics-drops-content"><div className="dungeon-statistics-summary-grid dungeon-statistics-drops-summary"><Statistic label="ITEMS" value={presentation.totalDropsLabel} icon={<Package size={13} aria-hidden="true" />} /><Statistic label="ITEMS / HOUR" value={formatStatisticsRate(presentation.dropsPerHour)} icon={<Gauge size={13} aria-hidden="true" />} /></div><div className="dungeon-statistics-list-label">DROPS</div>{presentation.dropRows.length ? <div className="dungeon-statistics-drops-list">{presentation.dropRows.map((row) => <GameTooltip block key={row.itemId} content={<TooltipContent title={ITEMS[row.itemId].name.toUpperCase()} description={ITEMS[row.itemId].description}><div className="tooltip-row"><span>EXACT QUANTITY</span><b>{row.quantity.toLocaleString()}</b></div><div className="tooltip-row"><span>RATE</span><b>{formatStatisticsRate(row.perHour)}</b></div></TooltipContent>}><div className="dungeon-statistics-drop-row" tabIndex={0} aria-label={`${row.name}, ${row.quantity.toLocaleString()} dropped, ${formatStatisticsRate(row.perHour)}`}><span className="dungeon-statistics-drop-icon"><ItemIcon itemId={row.itemId} size="tiny" /></span><strong>{row.name}</strong><span>{formatNumber(row.quantity)}</span><small>{formatStatisticsRate(row.perHour)}</small></div></GameTooltip>)}</div> : <div className="dungeon-statistics-empty dungeon-statistics-empty-inline"><span>No drops recorded yet.</span></div>}</div>
}

...
export function EnemyContextWindow({ mode, anchorRef, triggerRef, selectedDungeonId, onModeChange, onClose }: EnemyContextWindowProps) {
  ...
  useLayoutEffect(() => {
    const updatePosition = () => {
      ...
      const left = right + width <= window.innerWidth - margin ? right : anchorRect.left - width - gap >= margin ? anchorRect.left - width - gap : Math.max(margin, (window.innerWidth - width) / 2)

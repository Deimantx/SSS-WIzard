...
export function EnemyPatternRail({ pattern, enemy, currentIndex, activeStepId, activeAction, activeOriginMatchesCurrent }: { pattern?: ActionPattern; enemy?: MonsterDefinition | null; currentIndex: number; activeStepId: string | null; activeAction: string | null; activeOriginMatchesCurrent: boolean }) {
  ...
  return <div className="combat-pattern-rail combat-flow-pattern-rail" aria-label="Enemy pattern">{pattern.steps.map((step, index) => {
    ...
    return <span className="combat-pattern-node-wrap combat-flow-pattern-node-wrap" key={step.id}><GameTooltip block accent={current ? 'warning' : 'neutral'} content={tooltip}><span className={`combat-pattern-node combat-flow-pattern-node${current ? ' is-current' : complete ? ' is-complete' : ''}`} aria-current={current ? 'step' : undefined} tabIndex={0}><i>{complete ? <Check size={12} aria-hidden="true" /> : current ? '→' : '·'}</i><strong>{name}</strong></span></GameTooltip>{index < pattern.steps.length - 1 && <span className="combat-pattern-arrow combat-flow-pattern-arrow" aria-hidden="true">→</span>}</span>
  })}</div>
}

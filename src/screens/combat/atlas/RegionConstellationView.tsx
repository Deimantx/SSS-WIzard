...
function DungeonWaypoint({ node, selected, onSelect }: { node: CombatRegionMapNode; selected: boolean; onSelect: (id: string) => void }) {
  ...
  return <GameTooltip block accent={node.status === 'locked' ? 'neutral' : 'mana'} content={<TooltipContent title={node.name} description={tooltipDescription} />}><button type="button" data-atlas-interactive="true" data-ui-sound="click" className={`atlas-waypoint is-${node.status}${selected ? ' is-selected' : ''}`} style={{ left: `${node.x}px`, top: `${node.y}px` }} onClick={() => onSelect(node.id)} aria-pressed={selected} aria-label={`${node.name}, ${statusLabel}`}><span className="atlas-waypoint-halo" aria-hidden="true" /><span className="atlas-waypoint-core" aria-hidden="true"><span>{getWaypointGlyph(node.status)}</span></span><span className="atlas-waypoint-label"><strong>{node.name}</strong><small>{statusLabel}</small></span></button></GameTooltip>
}

...
function WorldRegionPlate({ node, selected, onSelect, onEnter }: { node: WorldRegionMapNode; selected: boolean; onSelect: (id: string) => void; onEnter: (id: WorldRegionMapNode['id']) => boolean }) {
  ...
  return <GameTooltip block accent={node.status === 'locked' ? 'neutral' : 'mana'} content={<TooltipContent title={node.name} description={tooltipDescription} />}><button type="button" data-atlas-interactive="true" data-ui-sound="click" className={`atlas-world-plate is-${node.status}${selected ? ' is-selected' : ''}`} style={{ left: `${position.x}px`, top: `${position.y}px` }} onClick={() => onSelect(node.id)} onDoubleClick={(event) => { event.preventDefault(); if (node.status === 'locked' || !onEnter(node.id)) return; emitGameFeelEvent({ type: 'success', x: event.clientX, y: event.clientY, color: 'var(--ui-accent)', intensity: 1.05 }) }} aria-pressed={selected} aria-label={`${node.name}, ${statusLabel}`}><span className="atlas-world-plate-rune" aria-hidden="true"><Globe size={25} /></span><span className="atlas-world-plate-copy"><strong>{node.name}</strong><em>{statusLabel}</em></span><span className="atlas-world-plate-corner atlas-world-plate-corner-one" aria-hidden="true" /><span className="atlas-world-plate-corner atlas-world-plate-corner-two" aria-hidden="true" /></button></GameTooltip>
}

...
describe('getAdaptiveCombatLayout', () => {
  const base = [
    ...
    { i: 'combat-details', x: 7, y: 23, w: 5, h: 8 },
  ]
  ...
  it('expands the stage and pushes the following panels down without shrinking', () => {
    ...
    expect(layout).toEqual([
      ...
      { i: 'combat-details', x: 7, y: 28, w: 5, h: 8 },
    ])
  })
  ...
  it('adapts the Deck to one or two useful rows and collapses the Log to a real compact height', () => {
    ...
    expect(compact.find((item) => item.i === 'combat-details')).toMatchObject({ y: 23, h: 8 })
  })
  ...
  it('caps three-plus Spell rows at the useful two-row Deck height and restores expanded Log geometry', () => {
    ...
    expect(adaptive.find((item) => item.i === 'combat-details')).toMatchObject({ y: 23, h: 8 })
  })

  it('keeps Details visible at its own height while the Log collapses', () => {
    const collapsed = getAdaptiveCombatLayout(base, { logCollapsed: true, logExpandedH: 10 })
    expect(collapsed.find((item) => item.i === 'combat-log')).toMatchObject({ y: 23, h: 2 })
    expect(collapsed.find((item) => item.i === 'combat-details')).toMatchObject({ y: 23, h: 8 })
  })
})

...
export function CombatActNavigationDialog({ selectedDungeonId, onSelect, onClose }: { selectedDungeonId: DungeonId; onSelect: (id: DungeonId) => void; onClose: () => void }) {
  ...
  return (
    <ModalPortal open onClose={onClose} backdropClassName="combat-modal-backdrop" surfaceClassName="combat-act-navigation-dialog" ariaLabelledBy="combat-act-navigation-title">
      <header className="combat-act-navigation-header">
        <div>
          <span className="combat-subsection-label">CAMPAIGN NAVIGATION</span>
          <h2 id="combat-act-navigation-title">Follow the tower's frontier.</h2>
          <p>Choose an Act, inspect its route, and step into the next encounter.</p>
        </div>
        <Button icon variant="ghost" ariaLabel="Close campaign navigation" onClick={onClose}><X size={17} aria-hidden="true" /></Button>
      </header>
      <CombatActSelector acts={viewModel.acts} selectedActId={viewModel.selectedAct.id} onSelect={selectAct} />
      <div className="combat-act-navigation-body">
        <section className="combat-act-tree-panel">
          <div className="combat-act-tree-head">
            <div>
              <span className="combat-subsection-label">ACT PROGRESSION</span>
              <h3>{viewModel.selectedAct.title}</h3>
              <p>{viewModel.selectedAct.definition.description}</p>
            </div>
            <Status tone={viewModel.selectedAct.status === 'completed' ? 'success' : 'active'}>{viewModel.selectedAct.statusLabel}</Status>
          </div>
          <CombatActTree key={viewModel.selectedAct.id} act={viewModel.selectedAct} selectedNodeId={selectedNode.id} onSelect={setSelectedNodeId} onEnter={handleEnter} />
          {viewModel.selectedAct.id === 'act-0' && <div className="combat-act-tree-hint"><Route size={13} aria-hidden="true" /><span>Drag empty space to pan · Mouse wheel to zoom · Select a node to inspect · Double-click to enter</span></div>}
        </section>

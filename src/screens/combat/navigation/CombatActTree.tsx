...
const getNodeCoreRadius = (node: CombatActNodeViewModel) => (node.kind === 'final' ? 76 : 58) * (node.kind === 'branch' ? .88 : 1) / 2 + 6
...
const getConnectionPath = (from: CombatActNodeViewModel, to: CombatActNodeViewModel) => {
  const fromRadius = getNodeCoreRadius(from)
  const toRadius = getNodeCoreRadius(to)
  const vertical = Math.abs(to.x - from.x) < 150
  if (vertical) {
    const direction = to.y >= from.y ? 1 : -1
    const startY = from.y + direction * fromRadius
    const endY = to.y - direction * toRadius
...
function CombatActNode({ node, selected, onSelect, onEnter, onHover }: { node: CombatActNodeViewModel; selected: boolean; onSelect: (id: string) => void; onEnter: (id: string) => boolean; onHover: (id: string | null) => void }) {
  ...
  return <GameTooltip block accent={node.state === 'locked' ? 'neutral' : node.state === 'prototype' ? 'mana' : 'success'} content={<TooltipContent title={node.name} description={tooltipDescription} />}><button type="button" data-combat-progression-interactive="true" data-ui-sound="click" draggable={false} className={`combat-progression-node is-${node.state} is-${node.kind}${selected ? ' is-selected' : ''}`} style={{ left: `${node.x}px`, top: `${node.y}px`, '--node-accent': getNodeAccent(node) } as CSSProperties} aria-pressed={selected} aria-label={`${node.name}, ${nodeStateLabel}`} onClick={() => onSelect(node.id)} onMouseEnter={() => onHover(node.id)} onMouseLeave={() => onHover(null)} onDoubleClick={handleDoubleClick} onDragStart={(event) => event.preventDefault()}><span className="combat-progression-node-halo" aria-hidden="true" /><span className="combat-progression-node-core"><Icon size={node.kind === 'final' ? 21 : 18} strokeWidth={1.25} aria-hidden="true" /></span><span className="combat-progression-node-copy"><strong>{node.name}</strong><em>{nodeStateLabel}</em></span></button></GameTooltip>
}

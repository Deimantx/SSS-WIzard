...
function LootSource({ encounter, progress, inventory }: { encounter: CombatActNodeViewModel['encounters'][number] | NonNullable<CombatActNodeViewModel['boss']>; progress: ReturnType<typeof useGameStore.getState>['progress']; inventory: ReturnType<typeof useGameStore.getState>['inventory'] }) {
  ...
  return <section className={`combat-area-loot-source${encounter.role === 'boss' ? ' is-boss' : ''}`}><div className="combat-area-loot-source-head"><span className="combat-area-loot-source-glyph">{encounter.role === 'boss' ? '♛' : '◇'}</span><div><strong>{monster.name}</strong><small>{encounter.role === 'boss' ? 'BOSS LOOT' : 'NORMAL ENCOUNTER'}</small></div><Status tone="success">DISCOVERED</Status></div><div className="combat-area-loot-drop-grid">{monster.loot.map((drop) => <LootRewardTile key={drop.itemId} drop={drop} sourceName={monster.name} inventory={inventory} />)}</div></section>
}

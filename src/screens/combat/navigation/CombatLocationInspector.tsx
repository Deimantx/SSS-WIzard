...
function EncounterMonsterTooltip({ monster, worldTier, powerRating }: { monster: MonsterDefinition; worldTier?: import('../../../game/types').WorldTierId; powerRating?: number }) {
  ...
  return <TooltipContent title={monster.name} description={monster.subtitle}><div className="tooltip-section"><small>{worldTier ? `COMBAT STATS · WT${worldTier}` : 'COMBAT STATS'}</small>{powerRating !== undefined && <><span className="tooltip-row"><span>POWER</span><b>{formatNumber(powerRating)}</b></span><span className="tooltip-row"><span>THREAT / KILL</span><b>{formatNumber(powerRating)}</b></span><small>Baseline stats only; mechanics, Traits, resistances, and Zone Affixes can make an encounter more dangerous.</small></>}<span className="tooltip-row"><span>HP</span><b>{formatNumber(profile?.maxHealth ?? stats.maxHealth)}</b></span><span className="tooltip-row"><span>BASIC DAMAGE</span><b>{formatNumber(profile?.basicAttackDamage ?? stats.basicAttackDamage)}</b></span><span className="tooltip-row"><span>ATTACK TIME</span><b>{formatTime(stats.basicAttackIntervalMs)}</b></span><span className="tooltip-row"><span>DEFENSE</span><b>{formatNumber(profile?.defense ?? stats.defense)}</b></span></div></TooltipContent>
}

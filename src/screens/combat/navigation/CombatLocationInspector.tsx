...
import { ELITE_MINOR_AFFIXES } from '../../../game/content/elite-affixes'
...
function CombatZoneBody({ type, targeting, selectedTargetEnemyId, activeTargetEnemyId, onSelectTarget, boss }: { type: CombatLocationViewModel['type']; targeting: NonNullable<CombatLocationViewModel['targeting']>; selectedTargetEnemyId: MonsterId | null; activeTargetEnemyId: MonsterId | null; onSelectTarget: (enemyId: MonsterId) => void; boss: CombatEncounterViewModel | null }) {
  return <>
    ...
    {boss && <section className="combat-location-section combat-location-zone-boss"><span className="combat-location-section-label">{type === 'elite-zone' ? 'ELITE BOSS' : 'ZONE BOSS'}</span><EncounterTile encounter={boss} /></section>}
  </>
}
...
function TargetCard({ target, selected, hunting, onSelect }: { target: CombatTargetViewModel; selected: boolean; hunting: boolean; onSelect: () => void }) {
  ...
  const affix = target.minorAffixId ? ELITE_MINOR_AFFIXES[target.minorAffixId] : null
  const tooltip = target.known && monster ? <EncounterMonsterTooltip monster={monster} worldTier={target.worldTier} powerRating={target.powerRating} affix={affix} /> : affix ? <TooltipContent title={target.name} description="Discover this creature in combat to reveal its full Bestiary dossier."><div className="tooltip-section"><small>MINOR AFFIX</small><strong>{affix.name}</strong><span>{affix.description}</span></div></TooltipContent> : <TooltipContent title={target.name} description="Discover this creature in combat to reveal its full Bestiary dossier." />
  const marker = hunting ? 'HUNTING' : selected ? 'SELECTED' : null
  return <GameTooltip block accent={hunting ? 'warning' : selected ? 'elemental' : 'mana'} content={tooltip}><button type="button" className={`combat-target-card${selected ? ' is-selected' : ''}${hunting ? ' is-hunting' : ''}`} aria-pressed={selected} onClick={onSelect}><span className="combat-target-card-portrait">{target.known && monster ? <MonsterPortrait monster={monster} /> : <span aria-hidden="true">?</span>}</span><span className="combat-target-card-copy"><strong>{target.name}</strong><small>{target.difficulty.toUpperCase()} · POWER {formatNumber(target.powerRating)}</small>{affix && <span className="combat-target-card-affix">AFFIX: {affix.name.toUpperCase()}</span>}</span>{marker && <b className="combat-target-card-marker">{marker}</b>}</button></GameTooltip>
}
...
function EncounterMonsterTooltip({ monster, worldTier, powerRating, affix }: { monster: MonsterDefinition; worldTier?: import('../../../game/types').WorldTierId; powerRating?: number; affix?: { name: string; description: string } | null }) {
  ...
  return <TooltipContent title={monster.name} description={monster.subtitle}><div className="tooltip-section"><small>{worldTier ? `COMBAT STATS · WT${worldTier}` : 'COMBAT STATS'}</small>{powerRating !== undefined && <span className="tooltip-row"><span>POWER</span><b>{formatNumber(powerRating)}</b></span>}<span className="tooltip-row"><span>HP</span><b>{formatNumber(profile?.maxHealth ?? stats.maxHealth)}</b></span><span className="tooltip-row"><span>BASIC DAMAGE</span><b>{formatNumber(profile?.basicAttackDamage ?? stats.basicAttackDamage)}</b></span><span className="tooltip-row"><span>ATTACK TIME</span><b>{formatTime(stats.basicAttackIntervalMs)}</b></span><span className="tooltip-row"><span>DEFENSE</span><b>{formatNumber(profile?.defense ?? stats.defense)}</b></span>{affix && <div className="tooltip-section"><small>MINOR AFFIX</small><strong>{affix.name}</strong><span>{affix.description}</span></div>}</div></TooltipContent>
}

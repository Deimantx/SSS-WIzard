...
function CombatZoneBody({ type, targeting, selectedTargetEnemyId, activeTargetEnemyId, onSelectTarget, boss, zoneAffix, bossHunt }: { type: CombatLocationViewModel['type']; targeting: NonNullable<CombatLocationViewModel['targeting']>; selectedTargetEnemyId: MonsterId | null; activeTargetEnemyId: MonsterId | null; onSelectTarget: (enemyId: MonsterId) => void; boss: CombatEncounterViewModel | null; zoneAffix: CombatLocationViewModel['zoneAffix']; bossHunt: CombatBossHuntPresentation | null }) {
  return <>
    ...
    {boss && <section className="combat-location-section combat-location-zone-boss"><span className="combat-location-section-label">{type === 'elite-zone' ? 'ELITE BOSS' : 'ZONE BOSS'}</span><EncounterTile encounter={boss} /><BossRequirement bossHunt={bossHunt} /></section>}
  </>
}
...
function EncounterMonsterTooltip({ monster, worldTier, powerRating }: { monster: MonsterDefinition; worldTier?: import('../../../game/types').WorldTierId; powerRating?: number }) {
  ...
  return <TooltipContent title={monster.name} description={monster.subtitle}><div className="tooltip-section"><small>{worldTier ? `COMBAT STATS · WT${worldTier}` : 'COMBAT STATS'}</small>{powerRating !== undefined && <span className="tooltip-row"><span>POWER</span><b>{formatNumber(powerRating)}</b></span>}<span className="tooltip-row"><span>HP</span><b>{formatNumber(profile?.maxHealth ?? stats.maxHealth)}</b></span><span className="tooltip-row"><span>BASIC DAMAGE</span><b>{formatNumber(profile?.basicAttackDamage ?? stats.basicAttackDamage)}</b></span><span className="tooltip-row"><span>ATTACK TIME</span><b>{formatTime(stats.basicAttackIntervalMs)}</b></span><span className="tooltip-row"><span>DEFENSE</span><b>{formatNumber(profile?.defense ?? stats.defense)}</b></span></div></TooltipContent>
}

function BossRequirement({ bossHunt }: { bossHunt: CombatBossHuntPresentation | null }) {
  if (!bossHunt) return null
  const progress = bossHunt.active && bossHunt.state !== 'fighting' ? `THREAT ${bossHunt.threatCurrent} / ${bossHunt.threatRequired}` : !bossHunt.active ? `REQUIRES ${bossHunt.threatRequired} THREAT` : 'BOSS FIGHT'
  const detail = !bossHunt.active ? null : bossHunt.state === 'building' ? `${bossHunt.remainingThreat} MORE KILLS TO BOSS` : bossHunt.state === 'ready' ? 'READY' : bossHunt.state === 'queued' ? 'AUTO HUNT QUEUED' : 'FIGHTING'
  return <div className={`combat-location-boss-requirement is-${bossHunt.state}`}><strong>{progress}</strong>{detail && <span>{detail}</span>}</div>
}

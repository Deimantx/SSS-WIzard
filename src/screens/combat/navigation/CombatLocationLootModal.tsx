...
export function CombatLocationLootModal({ location, targetMonsterId, onClose }: { location: CombatLocationViewModel; targetMonsterId?: MonsterId | null; onClose: () => void }) {
  ...
  return <ModalPortal open onClose={onClose} backdropClassName="combat-location-loot-backdrop" surfaceClassName="combat-location-loot-modal" ariaLabelledBy="combat-location-loot-title">
    <header className="combat-location-loot-head"><div><span className="combat-subsection-label">{reward ? 'TARGET LOOT' : 'LOCATION LOOT'}</span><h2 id="combat-location-loot-title">{title}</h2>{reward ? <p className="combat-location-loot-context"><span>{location.name} · {location.typeLabel} · {target?.difficulty.toUpperCase()} · POWER {reward.powerRating}</span>{location.zoneAffix && <GameTooltip content={<TooltipContent title={`Zone Affix · ${location.zoneAffix.name}`} description={location.zoneAffix.description} />}><span tabIndex={0}>ZONE AFFIX · {location.zoneAffix.name.toUpperCase()}</span></GameTooltip>}</p> : <p>{loot ? 'Rewards available from this gameplay location.' : targeted ? 'Select a target to inspect its rewards.' : 'Loot has not been authored for this location.'}</p>}</div><Button icon variant="ghost" ariaLabel="Close loot" onClick={onClose}><X size={16} aria-hidden="true" /></Button></header>

...
import { canPurchaseGuildSkillNode, getGuildPointsAvailable, getGuildRequestProgress, isGuildRequestComplete } from '../../game/systems/guild/guildSelectors'
...
export function GuildScreenV2() {
  ...
  const claimedCount = Object.values(progress.requestClaims).filter(Boolean).length
  const promotionReady = progress.guildRank === 'initiate' && claimedCount >= 3 && progress.guildReputation >= 175
  ...
  const banner = <div className="guild-banner"><div className="guild-emblem">¤</div><div><Status tone="success">Unlocked</Status><h2>Verdant Circle</h2><p>Current rank · {progress.guildRank}</p></div><div className="rank-step"><span>Outsider</span><span>→ Initiate</span><strong>→ Apprentice</strong></div></div>
  const rankAndSkills = <Card title="Guild progression" action={<span className="muted">{getGuildPointsAvailable(state)} Guild Points available</span>}><div className="rank-promotion"><div><Crown size={22} color="var(--gold)" /><strong>{progress.guildRank === 'apprentice' ? 'Apprentice achieved' : 'Initiate → Apprentice'}</strong><span>{claimedCount} / 4 unique Requests claimed · Reputation {progress.guildReputation} / 175. Claim any three to unlock promotion.</span></div><Button variant={progress.guildRank === 'apprentice' ? 'success' : 'secondary'} disabled={progress.guildRank === 'apprentice' || !promotionReady} onClick={state.promoteGuild}>{progress.guildRank === 'apprentice' ? 'Promoted' : 'Promote'}</Button></div><div className="guild-skill-tree">{GUILD_SKILL_BRANCHES.map((branch) => <SkillBranch key={branch} branch={branch} state={state} />)}</div><div className="guild-tree-actions"><GameTooltip content="Refund all Guild Skill Points outside active combat. Expanded Quarters cannot be removed while too many Acolytes are assigned."><Button variant="ghost" onClick={() => state.resetGuildSkillTree()}><RotateCcw size={14} /> Respec Tree</Button></GameTooltip><span>{Object.values(progress.guildSkillNodeRanks).filter(Boolean).length} / 9 nodes invested · Reputation is non-spendable.</span></div><div className="equipment-note"><Sparkles size={15} /><span>Guild Points come from unique Request claims and promotion. Skill effects are permanent until a free respec.</span></div></Card>

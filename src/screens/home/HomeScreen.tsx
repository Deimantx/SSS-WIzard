...
export function HomeScreenV2() {
  ...
  const fire = schools.fire
  const hasAuto = Object.values(activities.autoCast).some(Boolean)
  const hasEquipment = (['ember-staff', 'tide-focus', 'stoneweave-robe', 'windthread-charm'] as ItemId[]).some((id) => (inventory[id] ?? 0) > 0)
  const objectives = [
    ...
    { label: 'Research a Fragment', done: fire.xp > 0 },

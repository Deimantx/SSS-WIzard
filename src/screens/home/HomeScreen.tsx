...
import { getChronicleConditionProgress, getChronicleConditionValueLabel } from '../../game/presentation/chronicles/chroniclePresentation'
import { getChronicleActiveChapter, getChronicleChapterProgress, getChronicleMainObjective, getChronicleObjectiveStatus, getChronicleOverviewObjectives } from '../../game/systems/chronicles/chronicleRuntime'
...
export function HomeScreenV2() {
  ...
  const overviewObjectives = getChronicleOverviewObjectives(gameState)
  const chapterComplete = chapterProgress.completed === chapterProgress.total
  ...
  return <div className="screen-content"><div className="screen-header"><div><div className="eyebrow">WIZARD TOWER · OVERVIEW</div><h1>Good evening, apprentice.</h1><p>One wizard. One tower. Combat is yours; Acolytes keep the Tower working.</p></div></div><ScreenGrid screen="home" panels={[
    ...
    { id: 'home-chronicles', content: <Card title="Chronicles" action={<Button variant="ghost" onClick={() => setChroniclesOpen(true)}><BookOpen size={14} /> Open Chronicles</Button>}><div className="home-chronicles-summary"><div className="home-chronicles-progress"><div><span className="eyebrow">{chapter.name}</span><strong>{chapterProgress.completed} / {chapterProgress.total} goals · {chapterProgress.percent}%</strong></div><span className="home-chronicles-progress-bar"><i style={{ width: `${chapterProgress.percent}%` }} /></span></div><div className="home-chronicles-goals">{overviewObjectives.length ? overviewObjectives.map((item) => { const status = getChronicleObjectiveStatus(gameState, item); const condition = getChronicleConditionProgress(gameState, item); return <button type="button" key={item.id} className={`home-chronicle-goal ${status === 'current' ? 'current' : ''}`} onClick={() => setChroniclesOpen(true)}><span className="home-chronicle-goal-icon">{status === 'current' ? <Target size={14} /> : <Check size={14} />}</span><span><small>{trackLabel(item.track)} · {getChronicleConditionValueLabel(condition)}</small><strong>{item.title}</strong></span><ChevronRight size={14} /></button> }) : <div className="home-chronicles-complete"><Check size={16} /><span><strong>Every visible goal is complete.</strong><small>Open Chronicles to review the chapter history.</small></span></div>}</div><div className="home-chronicles-footer"><span>Complete {chapter.name}</span><Button variant="secondary" onClick={() => setChroniclesOpen(true)}>View Full Chronicle <ChevronRight size={14} /></Button></div></div></Card> },

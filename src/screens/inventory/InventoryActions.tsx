...
export function InventoryActions({ itemId, inventory, protectedItems, equipment, activities, toggleProtection, equipItem, sellItem, destroyItem, contextActions }: InventoryActionsProps) {
  ...
  return <div className="inventory-actions-content">
    ...
    <div className="inventory-quantity-control">
      <div className="inventory-quantity-head">
        <span>QUANTITY</span>
        <GameTooltip block content={quantityTooltip}><div className="inventory-action-quantity-edit"><input className="inventory-action-quantity-input" type="number" min={1} max={Math.max(1, maximum)} step={1} inputMode="numeric" value={quantityEditing ? quantityDraft : quantity} aria-label="Selected quantity" onChange={(event) => { setQuantityDraft(event.currentTarget.value); setQuantityEditing(true) }} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); commitQuantityEdit() } if (event.key === 'Escape') { event.preventDefault(); cancelQuantityEdit() } }} onBlur={quantityEditing ? commitQuantityEdit : undefined} /><span>/ {maximum}</span></div></GameTooltip>
      </div>
      {maximum > 0 && <div className="inventory-quick-actions" aria-label="Quick quantity">{[1, 10, 100].map(value => <Button key={value} variant="ghost" disabled={maximum < 1} onClick={() => updateQuantity(value)}>{value}</Button>)}<Button variant="ghost" disabled={maximum < 1} onClick={() => updateQuantity(maximum)}>MAX</Button></div>}
    ...
    </div>    {reason && <p className="inventory-action-note">{manuallyProtected ? 'PROTECTED · ' : ''}{reason}</p>}

...
export function InventoryActions({ itemId, inventory, protectedItems, equipment, toggleProtection, equipItem, sellItem, destroyItem }: { itemId: ItemId | null; inventory: GameState['inventory']; protectedItems: GameState['protectedItems']; equipment: GameState['equipment']; toggleProtection: (itemId: ItemId) => void; equipItem: (itemId: ItemId) => void; sellItem: (itemId: ItemId, quantity: number) => void; destroyItem: (itemId: ItemId, quantity: number) => void }) {
  ...
  return <div className="inventory-actions-content">
    {mode === 'manage' && <div className="inventory-actions-summary"><ItemIcon itemId={itemId} size="tiny" /><div><strong>{item.name}</strong><span>Owned ×{owned.toLocaleString()}{reserved > 0 ? ` · Equipped ×${reserved}` : ''}</span></div></div>}
    {mode === 'manage' ? <>
    ...
    </> : <ActionQuantityMode mode={mode} itemName={item.name} quantity={quantity} maximum={maximum} sellValue={item.sellValue} confirming={confirming} onQuantityChange={setQuantity} onCancel={() => setActionMode('manage')} onSell={() => { sellItem(itemId, quantity); setMode('manage') }} onDestroyRequest={() => setConfirming(true)} onDestroyConfirm={() => { destroyItem(itemId, quantity); setMode('manage'); setConfirming(false) }} />}
  </div>
}

function ActionQuantityMode({ mode, itemName, quantity, maximum, sellValue, confirming, onQuantityChange, onCancel, onSell, onDestroyRequest, onDestroyConfirm }: { mode: Exclude<ActionMode, 'manage'>; itemName: string; quantity: number; maximum: number; sellValue: number | null; confirming: boolean; onQuantityChange: (quantity: number) => void; onCancel: () => void; onSell: () => void; onDestroyRequest: () => void; onDestroyConfirm: () => void }) {
  ...
  return <div className={`inventory-action-mode inventory-action-mode-${mode}`}>
    ...
    {isSell ? <div className="inventory-action-preview"><span>{sellValue?.toLocaleString() ?? '0'} GOLD EACH</span><strong>YOU RECEIVE <b>{receive.toLocaleString()} GOLD</b></strong></div> : confirming ? <div className="inventory-action-preview inventory-action-confirmation" role="status"><strong>THIS CANNOT BE UNDONE</strong></div> : <div className="inventory-action-preview"><span>NO REWARD</span></div>}

...
export function InventoryDetail({ itemId, inventory, protectedItems, equipment, economyState, navigate, toggleProtection, equipItem, sellItem, destroyItem }: { itemId: ItemId | null; inventory: GameState['inventory']; protectedItems: GameState['protectedItems']; equipment: GameState['equipment']; economyState?: ItemEconomyState; navigate?: (screen: ScreenId) => void; toggleProtection?: (id: ItemId) => void; equipItem?: (id: ItemId) => void; sellItem?: (id: ItemId, quantity: number) => void; destroyItem?: (id: ItemId, quantity: number) => void }) {
  ...
  return <div ref={detailScrollRef} className={`inventory-detail-content inventory-detail-${item.inventoryCategory} smart-scroll-region`} style={{ '--detail-accent': item.color } as CSSProperties}>
    ...
    {item.kind === 'equipment' && <InventoryLoadoutComparison itemId={itemId} equippedId={equippedId} quantity={quantity} inventory={inventory} comparison={comparison} />}
...
function InventoryLoadoutComparison({ itemId, equippedId, quantity, inventory, comparison }: { itemId: ItemId; equippedId: ItemId | null; quantity: number; inventory: GameState['inventory']; comparison: ReturnType<typeof getEquipmentComparison> }) {
  return <section className="inventory-detail-section inventory-loadout-comparison"><span className="inventory-detail-label">LOADOUT COMPARISON</span><div className="artificing-output-current"><span>CURRENT</span>{equippedId ? <ItemTooltip itemId={equippedId} owned={inventory[equippedId] ?? 0} equipped><span className="artificing-comparison-icon"><ItemIcon itemId={equippedId} size="tiny" /></span></ItemTooltip> : <span className="artificing-empty-slot">EMPTY</span>}<span aria-hidden="true">→</span><span>SELECTED PREVIEW</span><ItemTooltip itemId={itemId} owned={quantity}><span className="artificing-comparison-icon"><ItemIcon itemId={itemId} size="tiny" /></span></ItemTooltip></div>{equippedId ? <div className="artificing-output-stat-list comparison">{comparison.map((row) => <div key={row.key}><span>{row.label}</span><small>{row.equippedValue} → {row.selectedValue}</small><strong className={row.direction}>{row.delta}</strong></div>)}</div> : <p className="inventory-muted-note">No item currently equipped in this slot. This item's bonuses are additions.</p>}</section>
}

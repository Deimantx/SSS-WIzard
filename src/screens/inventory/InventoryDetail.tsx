...
export function InventoryDetail({ itemId, inventory, protectedItems, equipment, economyState, navigate, toggleProtection, equipItem, sellItem, destroyItem, onOpenArtifactPath }: { itemId: ItemId | null; inventory: GameState['inventory']; protectedItems: GameState['protectedItems']; equipment: GameState['equipment']; economyState?: ItemEconomyState; navigate?: (screen: ScreenId) => void; toggleProtection?: (id: ItemId) => void; equipItem?: (id: ItemId) => void; sellItem?: (id: ItemId, quantity: number) => void; destroyItem?: (id: ItemId, quantity: number) => void; onOpenArtifactPath?: (id: ArtifactId) => void }) {
...
function InventoryLoadoutComparison({ itemId, equippedId, quantity, inventory, preview }: { itemId: ItemId; equippedId: ItemId | null; quantity: number; inventory: GameState['inventory']; preview: EquipmentPreview | null }) {
  ...
  const impactRows = Object.entries(preview.impact).flatMap(([key, value]) => key === 'resistances' && value && typeof value === 'object' ? Object.entries(value).map(([type, resistance]) => [`resistance-${type}`, Number(resistance)] as [string, number]) : [[key, Number(value)] as [string, number]]).filter(([key, value]) => !['basicDamage', 'basicAttackSpeedPct', 'blockChance'].includes(key) && Math.abs(value) > 0.0001)

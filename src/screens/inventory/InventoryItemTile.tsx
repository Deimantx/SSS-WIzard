...
export function InventoryItemTile({ itemId, inventory, protectedItems, equipment, selected, newItem = false, flow, flowDirection, onSelect }: { itemId: ItemId; inventory: GameState['inventory']; protectedItems: GameState['protectedItems']; equipment: GameState['equipment']; selected: boolean; newItem?: boolean; flow?: ItemFlow | null; flowDirection?: ItemFlowDirection; onSelect: () => void }) {
  ...
  return <ItemTooltip itemId={itemId} owned={quantity} protectedItem={protectedItem} equipped={equipped} flow={flow}>
    <button type="button" data-item-id={itemId} className={`inventory-item ${getInventoryAccentClass(itemId)} ${selected ? 'selected' : ''} ${protectedItem ? 'protected' : ''} ${equipped ? 'equipped' : ''} ${newItem ? 'is-new' : ''}`} onClick={onSelect} onContextMenu={(event) => { event.preventDefault(); event.stopPropagation(); openContextMenu({ x: event.clientX, y: event.clientY, header: { title: item.name, meta: `${item.kind.toUpperCase()} · OWNED ${quantity}` }, sections: [{ id: 'inspect', actions: [{ id: 'inspect', label: 'INSPECT', icon: Eye, onSelect: onSelect }] }] }) }} onKeyDown={(event) => { if (event.shiftKey && event.key === 'F10') { event.preventDefault(); const rect = event.currentTarget.getBoundingClientRect(); openContextMenu({ anchor: event.currentTarget, x: rect.left, y: rect.bottom, header: { title: item.name, meta: `${item.kind.toUpperCase()} · OWNED ${quantity}` }, sections: [{ id: 'inspect', actions: [{ id: 'inspect', label: 'INSPECT', icon: Eye, onSelect: onSelect }] }] }) } }} aria-label={`${item.name}, quantity ${quantity}${selected ? ', selected' : ''}${equipped ? ', equipped' : ''}${protectedItem && !equipped ? ', protected' : ''}${newItem ? ', new' : ''}`} aria-pressed={selected}>

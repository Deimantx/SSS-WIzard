...
export function InventoryRecent({ entries, inventory, protectedItems = {}, equipment = { weapon: null, armor: null, head: null, cape: null, necklace: null, earring1: null, earring2: null, ring1: null, ring2: null }, flows, onSelect }: { entries: readonly RecentAcquisition[]; inventory: GameState['inventory']; protectedItems?: GameState['protectedItems']; equipment?: GameState['equipment']; flows?: ReadonlyMap<ItemId, ItemFlow | null>; onSelect: (itemId: ItemId) => void }) {
  ...
  return <section className="inventory-recent" aria-labelledby="inventory-recent-heading">
    ...
    <div className="inventory-recent-list">{visible.map((entry) => <ItemTooltip key={entry.itemId} itemId={entry.itemId} owned={inventory[entry.itemId] ?? 0} recentlyGained={entry.amount} protectedItem={Boolean(protectedItems[entry.itemId]) || Object.values(equipment).includes(entry.itemId)} equipped={Object.values(equipment).includes(entry.itemId)} flow={flows?.get(entry.itemId)}>

...
export function InventoryScreenV2() {
  ...
  const [clearedNew, setClearedNew] = useState<Set<import('../../game/types').ItemId>>(() => new Set())
  ...
  const visibleIds = useMemo(() => selectVisibleItemIds(inventory, protectedItems, equipment, search, filter, sort, materialSubcategory, recentOrder), [inventory, protectedItems, equipment, search, filter, sort, materialSubcategory, recentOrder])
  ...
  const selectItem = (itemId: import('../../game/types').ItemId) => {
    setSelected(itemId)
  ...
  const selectRecent = (itemId: import('../../game/types').ItemId) => {
    setSearch('')
    setFilter('All')
  ...
  const noMatchText = search.trim() ? `No owned items match “${search.trim()}”.` : filter === 'Protected' ? 'No protected items are currently owned.' : 'No owned items match this view.'
  ...
  const renderGrid = () => {
    ...
    const groups = filter === 'All' || filter === 'Protected' ? groupOwnedItemIds(visibleIds) : [{ category: filteredCategory as 'material' | 'loot' | 'equipment' | 'special', ids: visibleIds }]
    return <div className="inventory-groups">{groups.map((group) => <section className="inventory-group" key={group.category}><div className="inventory-group-heading"><span>{CATEGORY_LABELS[group.category]}</span><small>{group.ids.length} {group.ids.length === 1 ? 'TYPE' : 'TYPES'}</small></div><div className="inventory-grid">{group.ids.map((id) => <InventoryItemTile key={id} itemId={id} inventory={inventory} protectedItems={protectedItems} equipment={equipment} selected={selected === id} newItem={newItems.has(id)} onSelect={() => selectItem(id)} />)}</div></section>)}</div>
  }

  const catalog = <Card title="ITEM VAULT" action={<span className="inventory-summary">{summary.types} ITEM TYPES <i>·</i> {summary.total.toLocaleString()} TOTAL ITEMS</span>}>
    <div className="inventory-toolbar"><div className="inventory-search"><Search size={16} aria-hidden="true" /><SearchInput value={search} onChange={setSearch} placeholder="Search inventory..." /></div><label className="inventory-sort">Sort<select aria-label="Sort inventory" value={sort} onChange={(event) => setSort(event.target.value as InventorySort)}>{INVENTORY_SORTS.map((value) => <option key={value}>{value}</option>)}</select></label><button type="button" className={`inventory-protected-toggle ${filter === 'Protected' ? 'active' : ''}`} aria-pressed={filter === 'Protected'} onClick={() => setCategory(filter === 'Protected' ? 'All' : 'Protected')}><LockKeyhole size={14} /> Protected {filter === 'Protected' && <Check size={13} />}</button></div>

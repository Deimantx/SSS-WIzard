...
export function getLoadoutDropDestination(fromIndex: number, targetIndex: number, position: DropPosition, slotCount: number): number | null {
  if (!Number.isInteger(fromIndex) || !Number.isInteger(targetIndex) || slotCount < 1 || fromIndex < 0 || fromIndex >= slotCount || targetIndex < 0 || targetIndex > 7) return null
  const insertionIndex = Math.min(targetIndex + (position === 'after' ? 1 : 0), slotCount)
  return Math.max(0, insertionIndex > fromIndex ? insertionIndex - 1 : insertionIndex)
}
...
function LoadoutSlot({ index, totalSlots, slot, spell, canEdit, dragging, dropTarget, registerTarget, onMove, onRemove, onSelect, onPointerDown }: { index: number; totalSlots: number; slot?: { spellId: SpellId; autoCast: boolean }; spell: typeof SPELLS[SpellId] | null; canEdit: boolean; dragging: boolean; dropTarget: DropPosition | null; registerTarget: (element: HTMLElement | null) => void; onMove: (fromIndex: number, toIndex: number) => unknown; onRemove: (spellId: SpellId) => unknown; onSelect?: (spellId: SpellId) => void; onPointerDown: (event: React.PointerEvent<HTMLElement>) => void }) {
  const school = spell ? SCHOOLS[spell.school] : null
  return <article ref={registerTarget} className={`loadout-slot${slot ? '' : ' is-empty'}${!canEdit ? ' is-readonly' : ''}${dragging ? ' is-dragging' : ''}${dropTarget ? ` is-drop-${dropTarget}` : ''}`} style={school ? { '--school-accent': school.color } as React.CSSProperties : undefined} onClick={() => spell && onSelect?.(spell.id)} onPointerDown={onPointerDown}>

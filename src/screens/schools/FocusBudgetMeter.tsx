...
export function FocusBudgetMeter({ autoCastFocus, otherFocus, totalFocus, maxFocus, freeFocus, compact = false }: FocusBudgetMeterProps) {
  ...
  return <div className={`focus-budget-meter${compact ? ' is-compact' : ''}${overCap ? ' is-over-cap' : ''}`} role="img" aria-label={meterLabel}>
    ...
    <div className="focus-budget-track" aria-hidden="true">
      <span className="focus-budget-segment is-auto" style={{ width: `${autoWidth}%` }}><GameTooltip accent="focus" content={<TooltipContent title="Auto-Cast Focus" description={`${autoCastFocus} Focus reserved by the projected Auto-Cast loadout.`} />}><span className="focus-budget-segment-hitbox" /></GameTooltip></span>
      <span className="focus-budget-segment is-other" style={{ width: `${otherWidth}%` }}><GameTooltip accent="neutral" content={<TooltipContent title="Other Systems" description={`${otherFocus} Focus reserved by non-spell systems.`} />}><span className="focus-budget-segment-hitbox" /></GameTooltip></span>
      <span className="focus-budget-segment is-free" style={{ width: `${freeWidth}%` }}><GameTooltip accent="success" content={<TooltipContent title={overCap ? 'Over Cap' : 'Free Focus'} description={overCap ? `${Math.abs(freeFocus)} Focus over the current cap.` : `${freeFocus} Focus remains available.`} />}><span className="focus-budget-segment-hitbox" /></GameTooltip></span>
    </div>

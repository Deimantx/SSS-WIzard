...
export function PresetAvailableSpellTile({ spell, rank, focusCost, added, disabled = false, onAdd }: { spell: SpellDefinition; rank: number; focusCost: number; added: boolean; disabled?: boolean; onAdd: (spellId: CanonicalSpellId) => void }) {
  ...
  return <GameTooltip block accent={added ? 'neutral' : 'elemental'} content={<TooltipContent title={spell.name} description={added ? 'Already in this preset.' : 'Add this Spell to the loadout.'} />}>
    <button type="button" disabled={unavailable} aria-label={added ? `${spell.name}, already added` : disabled ? 'Preset has eight slots' : `Add ${spell.name} to preset`} className={`spell-preset-available-tile${added ? ' is-added' : ''}${disabled ? ' is-full' : ''}`} style={{ '--spell-school-color': SCHOOLS[spell.school].color } as React.CSSProperties} onClick={() => onAdd(spell.id)}>

import { ArrowDown, ArrowUp, CircleDot, GripVertical, X } from 'lucide-react'
...
export function PresetLoadoutSpellTile({ spell, slot, rank, focusCost, index, total, dragging = false, dropTarget = false, onMove, onRemove, onToggleAutoCast, onDragStart, onDragEnd, onDragOver, onDrop }: { spell: SpellDefinition | null; slot: SpellPresetSlot | null; rank: number | null; focusCost: number | null; index: number; total: number; dragging?: boolean; dropTarget?: boolean; onMove: (index: number, direction: -1 | 1) => void; onRemove: (spellId: SpellPresetSlot['spellId']) => void; onToggleAutoCast: (spellId: SpellPresetSlot['spellId'], autoCast: boolean) => void; onDragStart?: (event: React.DragEvent<HTMLButtonElement>, spellId: SpellPresetSlot['spellId'], index: number) => void; onDragEnd?: () => void; onDragOver?: (event: React.DragEvent<HTMLElement>, index: number) => void; onDrop?: (event: React.DragEvent<HTMLElement>, index: number) => void }) {
  const available = Boolean(spell && slot)
  const name = spell?.name ?? 'Unavailable Spell'
  const school = spell?.school ?? 'fire'
  return <article className={`spell-preset-loadout-tile spell-ui-static${slot ? (available ? '' : ' is-unavailable') : ' is-empty'}${dragging ? ' is-dragging' : ''}${dropTarget ? ' is-drop-target' : ''}`} onDragOver={(event) => onDragOver?.(event, index)} onDrop={(event) => onDrop?.(event, index)}>
    <div className="spell-preset-loadout-head"><span className="spell-preset-slot-number">{String(index + 1).padStart(2, '0')}</span>{available && slot && <GameTooltip content={<TooltipContent title="Drag to reorder" description="Move this Spell to another loadout slot." />}><button type="button" className="spell-preset-drag-handle spell-ui-static" draggable aria-label={`Drag ${name} to reorder`} onClick={(event) => event.preventDefault()} onDragStart={(event) => onDragStart?.(event, slot.spellId, index)} onDragEnd={onDragEnd}><GripVertical size={14} aria-hidden="true" /></button></GameTooltip>}</div>

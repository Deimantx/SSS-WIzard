...
describe('Spell automation editor audit fixes', () => {
  ...
  it('shows the rich tooltip on the Edit Rules spell icon', () => {
    vi.useFakeTimers()
    renderAutomationModal('fire-bolt', undefined, true)

    fireEvent.pointerEnter(document.querySelector('.spell-automation-title-icon-tooltip-target') as HTMLElement)
  ...
  it('shows the rich tooltip on a prepared Combat Loadout spell icon without starting DnD', () => {
    vi.useFakeTimers()
    const current = useGameStore.getState()
    useGameStore.setState({ progress: { ...current.progress, spellRanks: { ...current.progress.spellRanks, 'fire-bolt': 1 } } })
    const id = useGameStore.getState().createSpellPreset('Tooltip Preset')

...
export function SpellBrowser({ state, filters, onFiltersChange, selectedEntryId, newSpells, onSelect, onToggleAutoCast, onOpenPresetManager }: {
  ...
  return <div className="schools-browser-panel">
    ...
    <div ref={browserGridRef} className="spell-browser-grid smart-scroll-region" aria-label="Spell catalog">
      {entries.map((entry: SpellBrowserEntry) => <SpellBrowserTile key={entry.id} entry={entry} state={state} selected={entry.id === selectedEntryId} newSpell={entry.kind === 'spell' && newSpells?.has(entry.spellId)} onSelect={onSelect} onToggleAutoCast={onToggleAutoCast} onOpenPresetManager={onOpenPresetManager} />)}

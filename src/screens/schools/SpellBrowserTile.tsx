...
export function SpellBrowserTile({ entry, selected, onSelect }: { entry: SpellBrowserEntry; selected: boolean; onSelect: (id: SpellId | string) => void }) {
  ...
  const content = unlocked
    ? <TooltipContent title="Spell" description={`Select to inspect this Spell. ${formatSpellRank(entry.rank ?? 1)} · ${SPELLS[entry.spellId].manaCost} Mana · ${formatTime(SPELLS[entry.spellId].cooldownMs)} cooldown.`} />
  ...
  const rankLabel = unlocked && entry.kind === 'spell' ? `R ${formatSpellRank(entry.rank ?? 1).replace(/^Rank /, '')}` : '?'
  return <GameTooltip block accent={unlocked ? 'elemental' : 'warning'} content={content}>
    <button type="button" style={{ '--spell-school-color': school.color } as React.CSSProperties} className={`spell-browser-tile${selected ? ' is-selected' : ''}${unlocked ? ' is-unlocked' : ' is-locked'}`} aria-label={visibleLabel} aria-pressed={selected} onClick={() => onSelect(entry.id)}>
      <div className="spell-browser-tile-top"><span className="spell-browser-icon-frame"><SpellIcon school={entry.school} locked={!unlocked} size="large" /></span><span className="spell-browser-rank-badge">{rankLabel}</span></div>

...
export function SpellInspector({ entry, state, rankPathOpen, equippedSlotIndex, canEdit, onEquip, onRemove, onToggleRankPath }: {
  ...
  if (!entry) return <Card className="schools-inspector-panel"><div ref={inspectorScrollRef} className="spell-inspector-scroll smart-scroll-region"><div className="spell-inspector-empty"><InspectorEyebrow>SELECT A SPELL</InspectorEyebrow><h2>SELECT A SPELL</h2><p>Known Spells will appear here when learned. Choose a known Spell from the Spellbook to inspect its mechanics.</p></div></div></Card>
  const school = SCHOOLS[entry.school]
  if (entry.kind === 'placeholder') return <Card className="schools-inspector-panel" style={{ '--spell-school-color': school.color, borderTopColor: school.color } as React.CSSProperties}><div ref={inspectorScrollRef} className="spell-inspector-scroll smart-scroll-region"><InspectorEyebrow>UNKNOWN SPELL</InspectorEyebrow><div className="spell-inspector-unknown"><SpellIcon school={entry.school} locked size="large" /><h2>?</h2><Status tone="locked">UNDISCOVERED</Status><p>{school.name.toUpperCase()} SCHOOL · Requires Level {entry.unlockLevel}</p><p className="muted">This future catalog slot has no authored mechanics yet.</p></div></div></Card>
  if (!entry.unlocked) {
    const lockedSpell = entry.kind === 'spell' ? SPELLS[entry.spellId] : null
    const currentSchoolLevel = state.schools[entry.school]?.level ?? 1
    return <Card className="schools-inspector-panel" style={{ '--spell-school-color': school.color, borderTopColor: school.color } as React.CSSProperties}><div ref={inspectorScrollRef} className="spell-inspector-scroll smart-scroll-region"><InspectorEyebrow>LOCKED SPELL</InspectorEyebrow><div className="spell-inspector-unknown"><SpellIcon school={entry.school} locked size="large" /><h2>{lockedSpell?.name ?? 'Undiscovered Spell'}</h2><Status tone="locked">LOCKED</Status><p>{school.name.toUpperCase()} SCHOOL · Requires Level {entry.unlockLevel}</p><div className="spell-lock-requirement"><span>Current {school.name} Level</span><strong>{currentSchoolLevel}</strong></div><p className="muted">Reach School Level {entry.unlockLevel} to reveal this spell and make it available for combat.</p></div></div></Card>
  }
...
function Metric({ icon, label, value, description, semantic }: { icon: React.ReactNode; label: string; value: string; description: string; semantic: 'mana' | 'time' | 'cast-time' | 'cooldown' | 'neutral' }) { return <GameTooltip block delay={180} accent={semantic === 'mana' ? 'mana' : semantic === 'cast-time' ? 'warning' : 'neutral'} content={<TooltipContent title={label} description={description} />}><div className="spell-core-metric"><span className={`spell-core-metric-icon ui-${semantic}`} aria-hidden="true">{icon}</span><small>{label}</small><strong className={`ui-${semantic}`}>{value}</strong></div></GameTooltip> }

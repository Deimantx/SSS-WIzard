...
import type { CanonicalSpellId, SpellId } from '../../game/types'
...
import { buildCombatSpellTilePresentation } from '../../game/presentation/spells/combatSpellTilePresentation'
...
export function SpellRow({ entry, state, selected, equipped, newSpell, canEdit, onSelect, onEquip, onRemove, onConfigureAutomation }: { entry: SpellBrowserEntry; state: SpellPresentationState; selected: boolean; equipped: boolean; newSpell: boolean; canEdit: boolean; onSelect: (id: SpellId | string) => void; onEquip: (spellId: SpellId) => void; onRemove: (spellId: SpellId) => void; onConfigureAutomation: (spellId: SpellId) => void }) {
  ...
  const compact = unlocked ? buildCombatSpellTilePresentation(state, entry.spellId as CanonicalSpellId, entry.rank ?? 1) : null
  ...
  return <article className={`spell-row${selected ? ' is-selected' : ''}${!unlocked ? ' is-locked' : ''}`} style={{ '--school-accent': school.color } as React.CSSProperties} onContextMenu={openSpellContextMenu}>
    <button type="button" className="spell-row-main" aria-label={label} aria-pressed={selected} onPointerDown={event => { if (unlocked && entry.kind === 'spell') beginDrag({ source: 'library', spellId: entry.spellId }, event) }} onClick={() => onSelect(entry.id)}>
      {compact && entry.kind === 'spell' ? <GameTooltip wide placement="right" delay={200} content={<LiveSpellCardTooltip spellId={entry.spellId} />}><span className="spell-row-icon spell-row-icon-tooltip-target" data-no-drag="true"><SpellIcon school={entry.school} spellId={entry.spellId} size="medium" /></span></GameTooltip> : <span className="spell-row-icon"><SpellIcon school={entry.school} spellId={unlocked ? entry.spellId : undefined} locked={!unlocked} size="medium" /></span>}
      ...
      {compact && entry.kind === 'spell' ? <span className="spell-row-stats"><CompactStat semantic="mana" icon={<Droplet size={13} />} label="Mana Cost" value={formatResourceAmount(compact.manaCost)} /><CompactStat semantic="neutral" icon={<CircleDot size={13} />} label="Auto-Cast" value="Independent" /><CompactStat semantic="cast-time" icon={<Clock3 size={13} />} label="Cast Time" value={compact.castTimeLabel} /><CompactStat semantic="cooldown" icon={<Clock3 size={13} />} label="Cooldown" value={compact.cooldownLabel} /></span> : <span className="spell-row-locked-copy"><LockKeyhole size={13} aria-hidden="true" />Requires {school.name} Level {entry.unlockLevel}</span>}
    </button>
...
function CompactStat({ semantic, icon, label, value }: { semantic: string; icon: React.ReactNode; label: string; value: string }) {
  const descriptions: Record<string, string> = {
    ...
    neutral: 'Auto-Cast is available without a separate capacity cost.',

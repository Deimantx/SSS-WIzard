...
const INLINE_ROW_PRIORITY: Record<string, number> = {
  ...
  'Action Speed': 10,
  'Air Damage Taken': 10,
  'Current Base Preview': 20,
  'School Scaling': 30,
  'Current School Level': 35,
  'Duration': 40,
  'Tick Interval': 45,
  'Applied Stacks': 50,
  'Max Stacks': 55,
...
const inlinePriority = (row: SpellEffectTooltipRow) => {
  if (INLINE_ROW_PRIORITY[row.label] !== undefined) return INLINE_ROW_PRIORITY[row.label]
  if (row.label.endsWith('Damage Taken')) return INLINE_ROW_PRIORITY['Damage Taken']
  return 90
}

...
describe('spell effect tooltip models', () => {
  ...
  it('exposes canonical status mechanics for buffs, control, and damage over time', () => {
    const state = createInitialState()
    ...
    expect(fortified).toMatchObject({ category: 'BUFF', title: 'Hardened' })
    expect(row(fortified, 'Damage Taken')?.value).toBe('-25%')
    expect(row(fortified, 'Duration')?.value).toBe('5.0s')
    expect(chilled).toMatchObject({ category: 'CONTROL', title: 'Chilled' })
    expect(row(chilled, 'Basic Attack Speed')?.value).toBe('-25%')
    expect(row(chilled, 'Action Speed')?.value).toBe('-25%')
    expect(row(chilled, 'Duration')?.value).toBe('5.0s')
    expect(burning).toMatchObject({ category: 'DOT', title: 'Burning' })
    expect(row(burning, 'Damage Per Tick')?.value).toBe('4.2')
  ...
  it('shows barrier duration, mode, target, source, and safe barrier equipment contribution', () => {
    const state = createInitialState()
    state.equipment.weapon = 'ember-staff'
    const model = buildSpellEffectTooltipModel(state, 'earthen-barrier', 0)

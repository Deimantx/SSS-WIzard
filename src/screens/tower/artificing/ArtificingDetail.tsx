...
function ArtifactArtificingDetail({ recipe }: { recipe: ArtificingRecipeDefinition }) {
  ...
  const activeForThis = Boolean(active && ((active.kind === 'recipe' && active.recipeId === recipe.id) || (active.kind !== 'recipe' && active.artifactId === recipe.id)))
  const sourceDrops = sources ? getItemDropSources(sources) : []
  const sourceTransmutation = sources ? getItemSources(sources).find((relation) => relation.kind === 'recipe' && relation.detail === 'Transmutation output') : undefined
  const unlocked = isRecipeUnlocked(state, recipe)
  const openSourceTransmutation = () => { if (!sourceTransmutation) return; setNavigationIntent({ transmutationRecipeId: sourceTransmutation.id as never }); state.setScreen('tower-transmutation') }
  ...
  return <><Card className="artificing-detail" title="ARCANE FORGE"><div className="artificing-detail-content"><ItemTooltip itemId={item.id} owned={state.inventory[item.id] ?? 0}><div className="artificing-detail-hero" tabIndex={0}><ItemIcon itemId={item.id} size="large" /><div><span className="eyebrow">TIER {artifactState.tier} ARTIFACT</span><h2>{item.name}</h2><EquipmentMetadata item={item} /><span className="artificing-owned">OWNED {(state.inventory[item.id] ?? 0).toLocaleString()}</span></div></div></ItemTooltip><p className="artificing-description">{item.description}</p><ActiveArtificingCraft /><section className="artificing-section"><span className="eyebrow">TIER {artifactState.tier} ARTIFACT</span><strong>{modeLabel}</strong>{artifactState.mode === 'level-cap' && <span>{artifactState.reason}</span>}{artifactState.mode === 'upgrade' && <span>NEXT UPGRADE · LEVEL {artifactState.level} → {artifactState.level + 1}</span>}{artifactState.mode === 'forge' && <span>Forge this unique Artifact once. It cannot be crafted again after acquisition.</span>}{artifactState.ingredients.length > 0 && <div className="artificing-material-list">{artifactState.ingredients.map((ingredient) => <ArtificingMaterialRow key={ingredient.itemId} itemId={ingredient.itemId} available={getConsumableQuantity(state, ingredient.itemId)} required={ingredient.quantity} missing={getConsumableQuantity(state, ingredient.itemId) < ingredient.quantity} onWhereToGet={() => setSources(ingredient.itemId)} />)}</div>}<div className="button-row">{artifactState.owned && <Button variant="secondary" onClick={() => setPathOpen(true)}>ARTIFACT PATH</Button>}{(artifactState.mode === 'forge' || artifactState.mode === 'upgrade') && <Button className="artificing-craft-button" variant="success" disabled={!unlocked || !artifactState.canStart || activeForThis || Boolean(active)} tooltip={artifactState.reason ?? (!unlocked ? getRecipeUnlockRequirement(recipe) ?? undefined : undefined)} onClick={() => artifactState.mode === 'forge' ? state.craftArtificingRecipe(recipe.id) : state.upgradeArtifact(recipe.id)}>{actionLabel}</Button>}</div></section><EquipmentInspection recipe={recipe} />{!unlocked && <div className="artificing-locked-banner"><LockKeyhole size={15} /><span>{getRecipeUnlockRequirement(recipe)}</span></div>}{sources && <div className="artificing-material-sources"><strong>{ITEMS[sources].name}</strong>{sourceDrops.map((drop) => <span key={drop.monsterId}>{drop.monsterName} · {drop.dungeonName} · {(drop.chance * 100).toFixed(1)}% · {drop.min}–{drop.max}</span>)}{sourceTransmutation && <button type="button" onClick={openSourceTransmutation}>OPEN TRANSMUTATION</button>}{sourceDrops.length > 0 && <button type="button" onClick={openSourceCombat}>OPEN COMBAT</button>}</div>}</div></Card>{pathOpen && artifactState.owned && <ArtifactPathModal artifactId={item.id} onClose={() => setPathOpen(false)} />}</>
}

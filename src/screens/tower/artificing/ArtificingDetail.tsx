...
export function ArtificingDetail({ recipe, inspection }: { recipe: ArtificingRecipeDefinition | null; inspection?: React.ReactNode }) {
  ...
  return <Card className="artificing-detail" title="ARCANE FORGE">
    <div className="artificing-detail-content">
      ...
      <p className="artificing-description">{item.description}</p>{inspection}
      ...
      <button type="button" className="artificing-pin-button" onClick={() => setUiPreferences({ screenState: { artificing: { pinnedRecipeId: uiPreferences.pinnedRecipeId === recipe.id ? null : recipe.id } } })}>{uiPreferences.pinnedRecipeId === recipe.id ? 'UNPIN RECIPE' : 'PIN RECIPE'}</button><div className={`artificing-craft-actions ${crafted ? 'craft-success' : ''}`}><Button className="artificing-craft-button" variant="success" disabled={!craftable} tooltip={reason ?? undefined} onClick={() => { if (state.craftArtificingRecipe(recipe.id)) setCrafted(item.name) }}>CRAFT</Button><Status>{unlocked ? 'ONE ITEM PER CRAFT' : 'LOCKED'}</Status></div>{crafted && <div className="artificing-crafted-result"><strong>CRAFTED · {crafted}</strong><div><Button variant="secondary" onClick={() => state.equipItem(item.id)}>EQUIP</Button><Button variant="ghost" onClick={() => state.setScreen('inventory')}>VIEW INVENTORY</Button></div></div>}
    </div>
  </Card>
}

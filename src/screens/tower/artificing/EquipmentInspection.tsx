...
function RingChoice({ position, itemId, selected, onClick }: { position: 'ring1' | 'ring2'; itemId: GameState['equipment']['ring1']; selected: boolean; onClick: () => void }) {
  return <button type="button" className={`artificing-ring-choice ${selected ? 'selected' : ''}`} aria-pressed={selected} onClick={onClick}><span>{position === 'ring1' ? 'RING 1' : 'RING 2'}</span><strong>{itemId ? ITEMS[itemId].name : 'Empty'}</strong></button>
}

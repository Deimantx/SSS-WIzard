...
export function ArcaneEchoPanel({ details, onToggle }: { details: boolean; onToggle: () => void }) {
  ...
  return <Card title="Arcane Echo Channeling" action={<Status tone={echoes > BALANCE.channeling.maxEchoes ? 'warning' : echoes === 0 ? 'neutral' : 'active'}>{status}</Status>}>
    ...
    <Button variant="ghost" className="channeling-breakdown-toggle" onClick={onToggle}>{details ? 'Hide Echo Modifiers' : 'View Echo Modifiers'}</Button>

...
export function ChannelingScreen() {
  ...
  return <div className="screen-content channeling-screen">
    <div className="screen-header"><div><div className="eyebrow">WIZARD TOWER · CHANNELING</div><h1>Channeling Chamber</h1><p>Assign Acolytes to draw Arcane Flux from the leyline while the Wizard fights.</p></div></div>
    <ArcaneDiscoveriesStrip completed={completed} total={CHANNELING_DISCOVERIES.length} onOpen={() => setDiscoveriesOpen(true)} />
    <ScreenGrid screen="tower-channeling" panels={[{ id: 'channeling-mana-core', content: <ArcaneFluxCorePanel /> }, { id: 'channeling-echoes', content: <AcolyteChannelingPanel /> }, { id: 'channeling-breakdown', content: <FluxBreakdownPanel /> }, { id: 'channeling-pillars', content: <ManaPillarsPanel /> }]} />
...
function ArcaneFluxCorePanel() {
  ...
  return <Card className="arcane-flux-core-panel" title="ARCANE FLUX CORE"><div className="flux-core-readout"><strong>{Math.floor(flux)} <small>/ {capacity.total}</small></strong><Progress value={capacity.total ? flux / capacity.total * 100 : 0} tone="mana" /></div><div className="flux-core-stats"><div><span>TOTAL FLUX FLOW</span><strong>+{production.total.toFixed(1)}/s</strong></div><div><span>ASSIGNED ACOLYTES</span><strong>{production.assignedAcolytes}</strong></div><div><span>CAPACITY</span><strong>{capacity.total}</strong></div></div></Card>
}
...
function AcolyteChannelingPanel() {
  ...
  return <Card className="acolyte-channeling-panel" title="ACOLYTE CHANNELING"><div className="channeling-stepper"><GameTooltip content={<TooltipContent title="Remove Channeling Acolyte" description="Release one worker. Partial Tower work remains intact." />}><Button variant="ghost" icon ariaLabel="Remove Channeling Acolyte" onClick={state.removeChannelingAcolyte} disabled={assigned <= 0}>-</Button></GameTooltip><strong>{assigned}</strong><GameTooltip content={<TooltipContent title="Assign Channeling Acolyte" description={addDescription} />}><Button variant="secondary" icon ariaLabel="Assign Channeling Acolyte" onClick={state.assignChannelingAcolyte} disabled={free <= 0}>+</Button></GameTooltip></div><p>{assigned} assigned · {free} available for other Tower work</p><div className="channeling-rate-summary"><span>Effective / Acolyte</span><strong>+{production.assignedAcolytes ? (production.total / production.assignedAcolytes).toFixed(1) : '0.0'} Flux/s</strong><span>Total</span><strong>+{production.total.toFixed(1)} Flux/s</strong></div></Card>
}

...
export function ManaCorePanel() {
  ...
  return <Card title="Mana Core" action={<Status tone="active">+{formatChannelingRate(regen.total)}/s</Status>}>
    <div className="channeling-mana-hero"><div><span className="eyebrow">CURRENT MANA</span><strong>{manaLabel} <small>/ {maxManaLabel}</small></strong>{overCap && <Status tone="warning">OVER CAP</Status>}</div><span className="channeling-mana-orb">✦</span></div>
    <Progress value={player.maxMana ? player.mana / player.maxMana * 100 : 0} tone="orange" label="Mana reserves" right={`${manaLabel} / ${maxManaLabel}`} />

...
export function ManaCorePanel() {
  ...
  return <Card title="Mana Core" action={<Status tone="active">+{rate(regen.total)}/s</Status>}>
    ...
    <Button variant="ghost" className="channeling-breakdown-toggle" onClick={() => setExpanded((value) => !value)}>{expanded ? 'Hide Detailed Breakdown' : 'View Detailed Breakdown'}</Button>

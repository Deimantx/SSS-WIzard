...
export function PreparedResearchRow({ slotId, expanded, onToggleExpanded }: { slotId: ResearchSlotId; expanded: boolean; onToggleExpanded: () => void }) {
  ...
  return <article className={`prepared-research-row status-${status} ${expanded ? 'is-expanded' : ''}`} tabIndex={0} aria-label={`${itemName} research batch for ${schoolName}, ${statusLabel}`} onContextMenu={(event) => { event.preventDefault(); event.stopPropagation(); openResearchMenu(event.clientX, event.clientY) }} onKeyDown={(event) => { if (event.shiftKey && event.key === 'F10') { event.preventDefault(); const rect = event.currentTarget.getBoundingClientRect(); openResearchMenu(rect.left, rect.bottom, event.currentTarget) } }}>
    ...
    <div className="prepared-research-row-progress"><div className="prepared-research-compact-progress"><div><span>ITEM</span><strong>{Math.round(itemProgress)}%</strong></div><Progress value={itemProgress} tone="violet" /></div><div className="prepared-research-compact-progress" style={{ '--school-color': SCHOOLS[job.targetSchoolId]?.color } as CSSProperties}><div><span>{schoolName} <b>LV {schoolProgress.level} / {schoolProgress.cap}</b></span><strong>{schoolProgressPercent}%</strong></div><Progress value={schoolProgressPercent} tone={job.targetSchoolId} /></div><div className="prepared-research-rates" aria-label={`${formatNumber(getResearchItemsPerHour(job))} items per hour, minus ${formatResourceRate(getResearchManaPerSecond(job))} Mana per second, ${formatNumber(getResearchXpPerHour(job))} XP per hour`}><span>{formatNumber(getResearchItemsPerHour(job))}/h</span><span>−{formatResourceRate(getResearchManaPerSecond(job))} Mana/s</span><span>{formatNumber(getResearchXpPerHour(job))} XP/h</span></div><GameTooltip content={<TooltipContent title={expanded ? 'Hide Research batch details' : 'Show Research batch details'} description="View exact school XP, next-level estimate, XP remaining and reserved Focus." />}><Button variant="ghost" icon className="prepared-research-expand" ariaLabel={expanded ? 'Hide Research batch details' : 'Show Research batch details'} ariaPressed={expanded} onClick={onToggleExpanded}>{expanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}</Button></GameTooltip></div>

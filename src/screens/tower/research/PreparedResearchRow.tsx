...
export function PreparedResearchRow({ slotId }: { slotId: ResearchSlotId }) {
  ...
  const openResearchMenu = (x: number, y: number, anchor?: HTMLElement) => openContextMenu({ x, y, anchor, header: { title: ITEMS[job.itemId]?.name ?? job.itemId, meta: `RESEARCH · ${statusLabel}` }, sections: [{ id: 'inspect', actions: [{ id: 'inspect', label: 'INSPECT ITEM', icon: Eye, onSelect: () => state.setScreen('inventory') }] }, { id: 'echoes', actions: [{ id: 'assign', label: 'ASSIGN +1 ECHO', disabled: !canAdd, disabledReason: echoHelp, onSelect: () => addEcho(slotId) }, { id: 'remove-echo', label: 'REMOVE 1 ECHO', disabled: echoes <= 0, onSelect: () => removeEcho(slotId) }] }, { id: 'remove', actions: [{ id: 'remove-research', label: 'REMOVE RESEARCH', tone: 'warning', onSelect: () => removeJob(slotId) }] }] })

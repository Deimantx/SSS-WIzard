import { BookOpen, Minus, Plus, ShoppingBag, X } from 'lucide-react'
...
export function PreparedResearchRow({ slotId }: { slotId: ResearchSlotId }) {
  ...
  const openResearchMenu = (x: number, y: number, anchor?: HTMLElement) => openContextMenu({ x, y, anchor, header: { title: ITEMS[job.itemId]?.name ?? job.itemId, meta: `RESEARCH · ${statusLabel}` }, sections: [{ id: 'echoes', actions: [{ id: 'assign', label: 'ASSIGN +1 ECHO', disabled: !canAdd, disabledReason: echoHelp, onSelect: () => addEcho(slotId) }, { id: 'assign-max', label: 'ASSIGN MAX AVAILABLE ECHOES', disabled: !canAdd, disabledReason: echoHelp, onSelect: () => state.assignMaxResearchEchoes(slotId) }, { id: 'remove-echo', label: 'REMOVE 1 ECHO', disabled: echoes <= 0, onSelect: () => removeEcho(slotId) }, { id: 'pause', label: 'PAUSE RESEARCH', disabled: echoes <= 0, onSelect: () => state.pauseResearch(slotId) }] }, { id: 'links', actions: [{ id: 'school', label: 'OPEN TARGET SCHOOL', icon: BookOpen, onSelect: () => { setUiPreferences({ screenState: { research: { selectedItemId: job.itemId, targetSchoolId: job.targetSchoolId } } }); setNavigationIntent({ schoolId: job.targetSchoolId }); state.setScreen('schools') } }, { id: 'inventory', label: 'OPEN ITEM IN INVENTORY', icon: ShoppingBag, onSelect: () => { setNavigationIntent({ inventoryItemId: job.itemId }); state.setScreen('inventory') } }] }, { id: 'remove', actions: [{ id: 'remove-research', label: 'REMOVE RESEARCH', tone: 'warning', onSelect: () => removeJob(slotId) }] }] })
  return <div className={`prepared-research-row status-${status}`} onContextMenu={(event) => { event.preventDefault(); event.stopPropagation(); openResearchMenu(event.clientX, event.clientY) }} onKeyDown={(event) => { if (event.shiftKey && event.key === 'F10') { event.preventDefault(); const rect = event.currentTarget.getBoundingClientRect(); openResearchMenu(rect.left, rect.bottom, event.currentTarget) } }}>
    <div className="prepared-research-main"><ItemIcon itemId={job.itemId} size="tiny" /><div className="prepared-research-copy"><strong>{ITEMS[job.itemId]?.name ?? job.itemId} &rarr; {SCHOOLS[job.targetSchoolId]?.name ?? job.targetSchoolId}</strong><GameTooltip content={<TooltipContent title={statusLabel} description={statusDescription} />} accent={status === 'running' || status === 'prepared' ? 'neutral' : 'warning'}><Status tone={status === 'running' ? 'active' : status === 'mana-limited' || status === 'waiting-mana' || status === 'level-cap' || status === 'protected' || status === 'missing-item' ? 'warning' : 'neutral'}>{statusLabel}</Status></GameTooltip></div><div className="prepared-research-echo-control"><GameTooltip content="Remove one Research Echo. Progress is preserved." accent="focus"><Button variant="ghost" ariaLabel={`Remove Research Echo from ${ITEMS[job.itemId]?.name ?? job.itemId}`} onClick={() => removeEcho(slotId)} disabled={echoes <= 0}><Minus size={12} /></Button></GameTooltip><strong>{echoes}</strong><GameTooltip content={<TooltipContent title="Assign Research Echo" description={echoHelp} />} accent={canAdd ? 'focus' : 'warning'}><Button variant="secondary" ariaLabel={`Assign Research Echo to ${ITEMS[job.itemId]?.name ?? job.itemId}`} onClick={() => addEcho(slotId)} disabled={!canAdd}><Plus size={12} /></Button></GameTooltip></div><GameTooltip content={canRemove} accent="warning"><Button variant="ghost" className="prepared-research-remove" ariaLabel="Remove prepared Research batch" onClick={() => removeJob(slotId)}><X size={13} /></Button></GameTooltip></div>
...
function ResearchMetric({ label, value, className = '' }: { label: string; value: string; className?: string }) { return <span className={`prepared-research-metric ${className}`}><small>{label}</small><strong>{value}</strong></span> }

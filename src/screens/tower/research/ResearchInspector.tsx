...
export function ResearchInspector({ itemId }: { itemId: ItemId | null }) {
  ...
  const tooltip = uniqueSlotFull
    ...
    : disabled ? <TooltipContent title="Cannot prepare" description={available < 1 ? 'No unreserved quantity is available.' : 'Choose a valid quantity and target school.'} />
      : <TooltipContent title="Prepare Research" description="Reserve this quantity without consuming it. Assign a Research Acolyte in Prepared Research." />
  ...
  return <Card className="research-inspector" title="ITEM INSPECTION">
    ...
    <section className="research-inspector-section"><span className="eyebrow">RESEARCH VALUE</span><div className="research-value-grid"><Metric label="XP / ITEM" value={String(xpPerItem)} /><Metric label="BASE TIME" value={formatTime(BALANCE.research.durationPerItemMs)} /><Metric label="FLUX / ITEM" value={String(BALANCE.research.arcaneFluxPerItem)} /></div></section>
    ...
    <div className="research-inspector-total"><Metric label="TOTAL XP" value={formatNumber(safeQuantity * xpPerItem)} /><Metric label="BASE TIME" value={formatTime(safeQuantity * BALANCE.research.durationPerItemMs)} /><Metric label="TOTAL FLUX" value={formatNumber(safeQuantity * BALANCE.research.arcaneFluxPerItem)} /></div>
    <section className="research-projection" style={{ '--school-color': SCHOOLS[targetSchoolId].color } as CSSProperties}><div className="research-projection-heading"><div><span className="eyebrow">PROJECTED SCHOOL · {SCHOOLS[targetSchoolId].name}</span><strong className="research-projection-school">{projection.current.atCap ? `LV ${projection.current.level} / ${projection.current.cap}` : projection.current.level === projection.projected.level ? `LV ${projection.current.level}` : `LV ${projection.current.level} → LV ${projection.projected.level}`}</strong></div><span className="research-projection-state">{projection.projected.atCap ? 'CAP' : projection.levelsGained > 0 ? `+${projection.levelsGained} LEVEL${projection.levelsGained === 1 ? '' : 'S'}` : 'PROGRESS'}</span></div><div className="research-projection-progress"><span>{projection.current.atCap ? 'CAP' : `${Math.round(projection.current.progress * 100)}%`}</span><Progress value={projection.projected.atCap ? 100 : projection.projected.progress * 100} tone={targetSchoolId} /><strong>{projection.projected.atCap ? 'CAP' : `${Math.round(projection.projected.progress * 100)}%`}</strong></div><div className="research-projection-grid"><Metric label="XP AFTER" value={formatNumber(projection.projected.xp)} /><Metric label="EFFECTIVE XP" value={formatNumber(projection.addedXp)} /></div><p>{targetAtCap ? 'No School XP can be gained at the current cap.' : 'Based on this quantity and the selected target. Preparing does not grant XP.'}</p></section>

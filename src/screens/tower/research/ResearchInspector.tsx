...
export function ResearchInspector({ itemId }: { itemId: ItemId | null }) {
  ...
  if (!itemId || !item) return <Card className="research-inspector" title="ITEM INSPECTION"><div className="empty-state small">Select a Researchable Item to inspect its affinity, target school value, and quantity.</div></Card>

  return <Card className="research-inspector" title="ITEM INSPECTION">
    <div className="research-inspection-hero"><div className="research-inspection-icon"><ItemIcon itemId={itemId} size="large" /></div><div><span className="eyebrow">{item.researchSchool ? SCHOOLS[item.researchSchool].name.toUpperCase() : 'RESEARCH' } AFFINITY</span><h2>{item.name}</h2><span className="muted">Owned ×{formatNumber(state.inventory[itemId] ?? 0)} · Available ×{formatNumber(available)} · Reserved ×{formatNumber(reserved)}</span></div></div>
    ...
    <section className="research-inspector-section"><span className="eyebrow">TARGET SCHOOL</span><div className="research-target-grid">{SCHOOL_ORDER.map((schoolId) => <GameTooltip key={schoolId} block content={<TooltipContent title={SCHOOLS[schoolId].name} description={`${getResearchXp(itemId, schoolId)} XP per researched ${item.name}.`} />}><button type="button" className={`research-target-button ${targetSchoolId === schoolId ? 'selected' : ''}`} style={{ '--school-color': SCHOOLS[schoolId].color } as CSSProperties} onClick={() => chooseTarget(schoolId)}>{SCHOOLS[schoolId].name.toUpperCase()}</button></GameTooltip>)}</div>{targetAtCap && <div className="research-cap-note"><Status tone="warning">LEVEL CAP</Status><span>This batch can be prepared, but cannot progress until the cap increases.</span></div>}</section>

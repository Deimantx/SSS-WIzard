...
export function ResearchScreen() {
  ...
  const intentItemId = navigationIntent.researchItemId && ITEMS[navigationIntent.researchItemId]?.researchSchool ? navigationIntent.researchItemId : null
  ...
  const selectedItemId = intentItemId ?? (storedItemId && visibleIds.includes(storedItemId) ? storedItemId : visibleIds[0] ?? null)

  useEffect(() => {
    if (intentItemId) {
      setUiPreferences({ screenState: { research: { selectedItemId: intentItemId, affinityFilter: 'all', ...(navigationIntent.researchSchoolId ? { targetSchoolId: navigationIntent.researchSchoolId } : {}) } } })
      setNavigationIntent({ researchItemId: null, researchSchoolId: null })
      return
    }

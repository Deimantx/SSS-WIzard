...
export function FocusAssignment({ selectedRecipeId, onSelect }: { selectedRecipeId: RecipeId; onSelect: (recipeId: RecipeId) => void }) {
  ...
  return <Card className="transmutation-focus" title="FOCUS ASSIGNMENT" action={<Status tone={totalEchoes ? 'active' : 'neutral'}>{totalEchoes} / {capacity} ECHOES</Status>}>
    ...
    {!locked && selectedEchoes > 0 && <small className="transmutation-focus-effective-time">Effective cycle {formatTime(getRecipeCurrentEffectiveDuration(recipe, selectedEchoes) ?? recipe.baseDurationMs)} · Mana demand {formatSignedRate(-getRecipeManaDemandPerSecond(recipe, selectedEchoes))}/s</small>}

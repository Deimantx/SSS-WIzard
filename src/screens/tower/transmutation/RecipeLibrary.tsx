  ...
  return <Card className="transmutation-library" title="RECIPE LIBRARY" action={<span className="transmutation-count">{visible.length} / {recipes.length}</span>}>
    ...
    <div className="transmutation-library-list">
      {visible.length === 0 ? <div className="empty-state small">No recipes match this filter.</div> : groups.map((category) => {
        ...
        const collapsed = collapsedCategories[category] && !shouldRevealCategory(filter, query)
        const contentId = `transmutation-${category}-recipes`
        return <section className="transmutation-recipe-group" key={category}><button type="button" className="transmutation-group-heading" aria-label={`${CATEGORY_LABELS[category]}, ${categoryRecipes.length} recipes`} aria-expanded={!collapsed} aria-controls={contentId} onClick={() => toggleCategory(category)}><span className="transmutation-group-heading-label">{CATEGORY_LABELS[category]}</span><span className="transmutation-group-heading-meta"><small>{categoryRecipes.length} RECIPES</small>{collapsed ? <ChevronRight size={15} aria-hidden="true" /> : <ChevronDown size={15} aria-hidden="true" />}</span></button><div id={contentId} className="transmutation-recipe-grid" hidden={collapsed}>{categoryRecipes.map((recipe) => <RecipeTile key={recipe.id} state={state} recipe={recipe} selected={recipe.id === selectedRecipeId} onSelect={onSelect} />)}</div></section>
      })}
    </div>
  </Card>
}

...
export const setBossKillsAction = (state: GameState, bossId: 'grove-sentinel' | 'forest-heart', amount: number) => { state.progress.bossKillsByBoss[bossId] = Math.max(0, amount); state.progress.requestProgress['sentinel-breaker'] = state.progress.bossKillsByBoss['grove-sentinel'] ?? 0 }

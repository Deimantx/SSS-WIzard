...
export const useGameStore = create<GameStore>()(immer((set, get) => ({
  ...
  setEnemyHealthPercent: (percent) => set((state) => { if (state.combat.enemyId) { state.combat.enemyHp = Math.max(0, Math.min(state.combat.enemyMaxHp, state.combat.enemyMaxHp * clamp(percent, 0, 100) / 100)); if (state.combat.enemyHp > 0) runCombatTriggers(state, 'enemy', 'on-hp-threshold', { source: { actor: 'player', kind: 'system', sourceId: 'developer-health-control' }, target: 'enemy', sourceTags: [] }, executeCombatEffects) } return state }),
  ...
  setPlayerBarrier: (amount) => set((state) => { state.combat.playerBarrier = Math.max(0, Math.floor(sanitizeDebugNumber(amount))); return state }),
  setEnemyBarrier: (amount) => set((state) => { state.combat.enemyBarrier = Math.max(0, Math.floor(sanitizeDebugNumber(amount))); return state }),
  clearPlayerBarrier: () => set((state) => { state.combat.playerBarrier = 0; return state }),
  clearEnemyBarrier: () => set((state) => { state.combat.enemyBarrier = 0; return state }),

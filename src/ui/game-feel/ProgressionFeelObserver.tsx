...
export function ProgressionFeelObserver({ profileKey }: { profileKey: string | null }) {
  ...
  useEffect(() => {
    ...
    previous.current = current
    newRecipes.forEach((recipeId) => { const recipe = RECIPES[recipeId as keyof typeof RECIPES]; const item = recipe && ITEMS[recipe.output.itemId as keyof typeof ITEMS]; if (!recipe || !item) return; enqueueMilestone({ kind: 'recipe', eyebrow: recipe.kind === 'artificing' ? 'ARTIFICING RECIPE UNLOCKED' : 'TRANSMUTATION RECIPE UNLOCKED', title: item.name, detail: recipe.kind === 'artificing' ? 'New blueprint available in Artificing' : 'New recipe available in Transmutation' }); markAttention(profileKey, 'recipe', recipeId) })

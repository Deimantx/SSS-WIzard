...
export function ProgressionFeelObserver({ profileKey }: { profileKey: string | null }) {
  ...
  useEffect(() => {
    ...
    previous.current = current
    newRecipes.forEach((recipeId) => { const recipe = RECIPES[recipeId as keyof typeof RECIPES]; const item = recipe && ITEMS[recipe.output.itemId as keyof typeof ITEMS]; if (!recipe || !item) return; enqueueMilestone({ kind: 'recipe', eyebrow: 'TRANSMUTATION RECIPE UNLOCKED', title: item.name, detail: 'New recipe available in Transmutation' }); markAttention(profileKey, 'recipe', recipeId) })

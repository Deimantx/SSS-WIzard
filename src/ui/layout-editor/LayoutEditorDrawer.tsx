...
function ShellLayoutControls() {
  ...
  return <>
    ...
    <section className="layout-editor-section shell-layout-section"><div className="layout-editor-section-title">REGION WIDTHS <span>Desktop pixels</span></div><p className="layout-editor-help">Drag resource grips in the topbar to reorder HP, Mana, and Focus. Drag the left or right edge of a resource to resize it.</p><div className="shell-width-list">{(['topbar-breadcrumb', ...TOPBAR_RESOURCE_IDS] as TopbarRegionId[]).map((id) => <label key={id}>{names[id]}<input type="number" min={TOPBAR_WIDTH_LIMITS[id].min} max={TOPBAR_WIDTH_LIMITS[id].max} value={layout.widths[id]} onChange={(event) => updateTopbarWidth(id, Number(event.target.value))} /><small>{TOPBAR_WIDTH_LIMITS[id].min}–{TOPBAR_WIDTH_LIMITS[id].max}px</small></label>)}</div><p className="layout-editor-help">Utilities remain content-sized as one grouped block. Narrow layouts temporarily use responsive shell rules and restore these values when wide.</p></section>
  </>
}

...
export function UiEditorInteractionLayer() {
  ...
  return <div className="ui-editor-selection-layer" data-ui-editor-ignore="true" aria-hidden="true">{entries.map((entry) => { const rect = entry.element.getBoundingClientRect(); if (rect.width <= 0 || rect.height <= 0) return null; const isFocused = focused?.id === entry.id; return <div key={entry.id} className={`ui-editor-selection-box ${isFocused && selectedUiId === entry.id ? 'selected' : isFocused ? 'hovered' : 'id-only'}`} style={{ left: rect.left, top: rect.top, width: rect.width, height: rect.height }}><span className="ui-editor-selection-label">{entry.label}{showIds && <small>{entry.id}</small>}<em>{Math.round(rect.width)} × {Math.round(rect.height)}</em></span>{isFocused && selectedUiId === entry.id && <><i className="ui-editor-resize-handle nw" /><i className="ui-editor-resize-handle ne" /><i className="ui-editor-resize-handle sw" /><i className="ui-editor-resize-handle se" /></>}</div> })}</div>
}

...
const labels: Record<string, string> = {
  ...
  'combat-stage': 'Combat Stage', 'combat-spell-deck': 'Spell Deck', 'combat-details': 'Combat Details',
...
export const PANEL_REGISTRY: PanelDefinition[] = (Object.entries(DEFAULT_LAYOUTS) as [ScreenId, Record<string, PanelDefinition['defaultLayout']>][]) .flatMap(([screen, layouts]) => Object.entries(layouts).map(([id, defaultLayout]) => ({
  ...
  label: labels[id] ?? id,
  defaultLayout,
  minW: id === 'combat-stage' ? 6 : id === 'combat-spell-deck' ? 6 : id === 'combat-details' ? 4 : id === 'focus-summary' ? 8 : id === 'focus-reservations' ? 5 : id === 'focus-improvement' ? 4 : id === 'research-school-mastery' || id === 'home-school-mastery' || id === 'home-arcane-work' ? 6 : id === 'inventory-actions' || id.includes('request') ? 3 : 2,

...
describe('Loot Reveal store', () => {
  ...
  it('batches one combat result and retains discovery through coalescing', () => {
    enqueueCombatLootReveal({ sourceLabel: 'Whispering Woods', sourceDetail: 'Grove Sentinel', items: [{ itemId: 'life-essence', quantity: 2, isNewDiscovery: true }, { itemId: 'artifact-essence', quantity: 1, isNewDiscovery: false }], now: 1000 })
    ...
    expect(getLootReveals()[0]?.items).toEqual(expect.arrayContaining([{ itemId: 'life-essence', quantity: 5, isNewDiscovery: true }, { itemId: 'artifact-essence', quantity: 1, isNewDiscovery: false }]))
  })
